...
<script>
  ...
  <title><s:text name="general.generalClaimRegain"/></title><%--通赔收回 --%>
</head>
...
<input type="hidden" name="pageFlag">
    <table id="queryTable" width="100%" border="0" align="center" cellpadding="4" cellspacing="1"  class="common">
      <tr>
	    <td colspan="4" class="formtitle"><s:text name="general.claimRegainSerach"/></td><%--查询可收回通赔任务 --%>
        </tr>
      <tr>
        <td class='title' ><s:text name="prompt.queRegist.RegistNo"/>：</td> <!--报案号 -->
        <td class='input' >
            <input type=text name="RegistNo" class="query" onchange="clickable()" onkeypress="KeyDown()">
        </td>
        <td class='title' ><s:text name="db.prpCmain.policyNo"/>:</td><!--保单号码 -->
        <td class='input' >
            <input type=text name="PolicyNo" class="query" onchange="clickable()" onkeypress="KeyDown()">
        </td>
      </tr>
      <tr>
        <td class='title' ><s:text name="workflow.receivePatty"/>:</td><%--接收机构 --%>
        <td class='input' >
        <input class='query' type='hidden' name='ReceiveComCode' value="">
    			   <input  class='codecode' type='text' name='ReceiveComname' value="" 
...
function init(){
	YAHOO.widget.DataTable.formatLink = function(elCell, oRecord, oColumn, oData) {
		if(oColumn.key =="registNo"){
			elCell.innerHTML="<a  href=\"${ctx}/generalClaim.do?actionType=prepareRegainInsert&registNo="+oRecord.getData("registNo") + "\">"+ oData+"</a>";
		}else if(oColumn.key =="givetime") {
			if(oData!=null){
				var date = new Date(oData.time);
				elCell.innerHTML = formatDate(date,'yyyy-MM-dd');
			}
	...
	contentColumnHeaders =[
		...
		{key:"receivecomname",label:"<s:text name="workflow.receivePatty" />",width:"40em",sortable:true},<%--接收机构--%>

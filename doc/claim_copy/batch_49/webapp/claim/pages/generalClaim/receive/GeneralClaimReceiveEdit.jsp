...
<head>
    <!--对title处理-->
    <title>
    	<s:text name="general.claimReceive"/>
    </title><%-- 通赔接收处理 --%>
  <%-- 页面样式  --%>
 	...
 	<div style="width:100%;height:320px;background-color:#ffffff;overflow:auto;">
         <table class="common" style="width:100%" cellspacing="1" cellpadding="5">
         	<thead>
                ...
                <tr>
       	  		   <td class="centertitle" ><s:text name="db.prpLrepairFee.registNo"/></td><%-- 报案号 --%>
       	  		   <td class="centertitle" ><s:text name="db.prpLclaimpolicy.policyNo"/></td><%-- 保单号 --%>
       	  		   <td class="centertitle" ><s:text name="db.prpLprop.riskCode"/></td><%-- 险种 --%>
  	   ...
  	   <TABLE class="common"  cellpadding="3" cellspacing="1" >
		  ...
		  <TR>
		    ...
		    <TD class='left'><s:text name="db.prpLregist.damageCode"/>：</td><%-- 出险原因 --%>
		    <TD class='right'>
		  ...
		  <TR>
		    <TD class='left'><s:text name="db.prpDagent.linkerName"/>：</td><%-- 联系人 --%>
		    <TD class='right' >
		...
		<TABLE cellpadding="3" cellspacing="0"   class="common" id=GeneralClaimTaskTable>
			<thead>
				<tr>
      			 	<TD class=formtitle><s:text name="db.prpCmain_coeff.remark"/></td><%-- 备注 --%>
      			 </tr>
			</thead>
    		<tbody>
  				<TR align=center>
		...
		<table class="common" style="width:100%" cellspacing="1" cellpadding="5">
         	<thead>
                ...
                <tr>
       	  		   <td class="centertitle" ><s:text name="db.prpCname.serialNo"/></td><%-- 序号 --%>
       	  		   <td class="centertitle" ><s:text name="general.nodeName"/></td><%-- 当前环节 --%>
       	  		   <td class="centertitle" ><s:text name="general.handleDept"/></td><%-- 处理机构代码 --%>
       	  		   <td class="centertitle" ><s:text name="general.receiveComcodeName"/></td><%-- 处理机构名称 --%>
       	  		   <td class="centertitle" ><s:text name="db.prpCmain.operatorCode"/></td><%-- 操作员代码 --%>
           ...
           <tbody> 
              <tr>
                ...
                <td class="input" align="center" style="width:15%"><%-- 选择处理机构 --%>
    			   ...
    			   <input  title='选择处理机构' class='codecode' type='text' name='comcode' value="" 
          				  ondblclick="code_CodeSelect(this,'queryProvince','-1,0,1','Y','','${prplgeneralclaimtask.receivecomcode}');" 
    	  				  onchange="code_CodeSelect(this,'queryProvince','-1,0,1','Y','','${prplgeneralclaimtask.receivecomcode}');">
    	  			<img src="/claim/images/bgMarkMustInput.jpg">
                </td>
                <td class="input" align="center" style="width:15%"><%-- 选择处理机构 --%>
                <input  title='选择处理机构' class='codecode' type='text' name='comname' value="" 
          				  ondblclick="code_CodeSelect(this,'queryProvince','-2,-1,0','Y','','${prplgeneralclaimtask.receivecomcode}');" 
    	  				  onchange="code_CodeSelect(this,'queryProvince','-2,-1,0','Y','','${prplgeneralclaimtask.receivecomcode}');">
    	  			<img src="/claim/images/bgMarkMustInput.jpg">
                </td>
                <td class="input" align="center" style="width:15%"><%-- 选择具有处理本任務权限的人员 --%>

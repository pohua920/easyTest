	...
	<tr>
		<td>
			<table class=common cellpadding="0" cellspacing="1" width="100%">
				...
				<input type="hidden" name="prpLcompensateStartDate"
					value="<fmt:formatDate value='${requestScope.prpLcompensate.startDate}' pattern='yyyy-MM-dd'/> <s:text name='endcase.dayStart'/><fmt:formatDate value='${requestScope.prpLcompensate.endDate}' pattern='yyyy-MM-dd'/><s:text name='endcase.dayEnd'/>"><%--日 0 时 至--%><%--日 24 时止--%>
				...
				<input type="hidden" name="prpLcompensateDamageStartDate"
					value="<fmt:formatDate value='${requestScope.prpLcompensate.damageStartDate}' pattern='yyyy-MM-dd'/> <s:text name='regist.prpLregist.date'/> <c:out value='${requestScope.prpLcompensate.damageStartHour}' /> <s:text name='regist.prpLregist.hour'/><c:out value='${requestScope.prpLcompensate.damageStartMinute}' /> <s:text name='regist.prpLregist.minute'/>"><%--日--%><%--时--%><%--分--%>
				...
				<tr>
					...
					<td class="left"　>同險編號</td>
					<td class="right"　><input type="text" name="prpLcompensateSameAddressNo" value="<c:out value='${prpLcompensate.sameAddressNo}'/>" class="readonly" readonly="true" /></td>
					<td class="left"　>
					<%-- <s:text name="db.prpLcompensate.earthquake" /> --%>
					</td>
					<td class="right"　></td>
				</tr>
				</tr>
				<tr>
					...
					<td class="left"><s:text name="db.prpLcompensate.otherPolicyNo" />：</td>
					<td class="right">
						<input type=text name="prpLcompensateOtherPolicyNo" title="<s:text name="db.prpLcompensate.otherPolicyNo" />" class="input" value="${prpLcompensate.otherPolicyNo}">
					</td>
				</tr>
				<tr>
					<td class="left">
					...
					<td class="left"><s:text name="db.prpLcompensate.otherClaimNo" />：</td>
					<td class="right"><input type=text name="prpLcompensateOtherClaimNo" title="<s:text name="db.prpLcompensate.otherClaimNo" />" class="input" value="${prpLcompensate.otherClaimNo}"></td>
				</tr>
			</table>
		</td>
	</tr>
		...
		<td>
			<table class=common cellpadding="0" cellspacing="1" width="100%">
				<tr>
					...
					<%-- 出险时间 --%>
					<td class="right">
<!-- 						<input type=text name="prpLcompensateDamageStartDate" class="readonly" readonly="true" maxlength="10" -->
<%-- 							value="<fmt:formatDate value='${requestScope.prpLcompensate.damageStartDate}' pattern='yyyy-MM-dd'/>  --%>
						<rc:rcDate name="prpLcompensateDamageStartDate" style="width:80" class="readonly" wdatePicker="false"  value='${requestScope.prpLcompensate.damageStartDate}'/>
					...
					<%-- 出险原因 --%>
					<td class="right">
						<c:out value='${requestScope.prpLcompensate.damageCode}' />
						<c:out value='${requestScope.prpLcompensate.damageName}' />
					</td>
				</tr>
			</table>
		</td>
			...
			<table class=common cellpadding="0" cellspacing="1" width="100%">
				<tr>
					...
					<c:choose>
						<c:when test="${requestScope.recaseFlag=='0' || param.editType=='SHOW'}">
							...
							<td class="right">
								<select id="prpLcompensateFinallyFlag" name="prpLcompensateFinallyFlag" style="width: 110px;" onchange="return changeFinallyFlagAndLltextContent();">
									<option value="0" <c:if test="${requestScope.prpLcompensate.finallyFlag=='0'}"><c:out value="selected"/></c:if>>
										<s:text name="compensate.partiallyOpen" /><%-- 部分未结 --%>
									</option>
									<option value="1" <c:if test="${requestScope.prpLcompensate.finallyFlag=='1'}"><c:out value="selected"/></c:if>>
										<s:text name="claim.case" /><%-- 结案 --%>
									</option>
									<option value="2" <c:if test="${requestScope.prpLcompensate.finallyFlag=='2'}"><c:out value="selected"/></c:if>>
										<s:text name="claim.rejectClaim" /><%-- 拒赔 --%>
									</option>
								</select>
							</td>
						</c:when>
				...
				<tr>
					...
					<td class="left">
						...
						<rc:rcDate name="startApplyPayDate" style="display: none;width:50%;" value="${requestScope.prpLclaim.startApplyPayDate}" class="query" />

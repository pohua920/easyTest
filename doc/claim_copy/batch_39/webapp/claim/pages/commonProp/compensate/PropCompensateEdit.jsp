...
<script src="${ctx}/pages/commonProp/compensate/js/PropCompensateEdit.js"></script>
<script src="${ctx}/pages/commonProp/compensate/js/PropPersonLossEdit.js"></script>
<script src="${ctx}/pages/commonProp/compensate/js/PropCompensateEditAdd.js"></script>
...
<DIV id="mainLayer" class="mainLayer">
	<form name=fm action="${ctx}/compensate/compensateSave.do" method="post" onsubmit="return validateForm(this);">
		...
		<mpc:container ID="oMPC" style="width:830px;height:420px;visibility:hidden">
			<mpc:page ID="tabMain" TABTITLE="main" TABTEXT="<s:text name='regist.prpLregist.registMain'/>">
				<%--基本讯息--%>
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%-- 1.报案主信息 --%>
						<%@include file="/pages/commonProp/compensate/PropCompensateMainHeadEdit.jsp"%>
						<%-- 2.特别约定 --%>
						<jsp:include page="/pages/DAA/compensate/DAACompensateCengage.jsp" />
						<%-- 2.赔偿限额/免赔额 --%>
						<%--@include file="/pages/commonProp/compensate/PropCompensatePrpDlimit.jsp"--%>
						<%-- 4.付款说明 --%>
			...
			<mpc:page ID="tabMain" TABTITLE="<s:text name='button.PayoutInformation.value'/>" TABTEXT="<s:text name='button.PayoutInformation.value'/>">
				...
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%-- 1.赔付标的信息 --%>
						<%@include file="/pages/commonProp/compensate/PropCompensateLlossEdit.jsp"%>
						<%-- 2.赔付人员信息  --%>
						<%@include file="/pages/commonProp/compensate/PropCompensatePersonLossEdit.jsp"%>
						<%-- 3.赔款费用 --%>
						<%@include file="/pages/commonProp/compensate/PropCompensateChargeEdit.jsp"%>
						<%-- 支付帳户信息 --%>
						<jsp:include page="/pages/commonProp/compensate/EditPrpdpaymentaccountPage.jsp" />
						<%-- 4.联共保赔款费用分摊信息  --%>
						<c:choose>
						...
						<%-- 5.主信息 --%>
						<%@include file="/pages/commonProp/compensate/PropCompensateMainTailEdit.jsp"%>
					</DIV>
				</CENTER>
			</mpc:page>
			<mpc:page ID="tabMain" TABTITLE="<s:text name='compensate.adjustReport'/>" TABTEXT="<s:text name='compensate.adjustReport'/>">
				...
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%--增加定损说明--%>
						<%@include file="/pages/commonProp/compensate/PropEndCaseCompensateTextEdit.jsp"%>
						<%-- 4.理算报告 --%>
						<%@include file="/pages/commonProp/compensate/PropCompensateTextEdit.jsp"%>
						<%-- 3.發送再保通知 --%>
						<%@include file="/pages/common/compensate/CompensateNotice.jsp"%>
					</DIV>
				</CENTER>
			</mpc:page>
			<mpc:page ID="tabMain" TABTITLE="地震基金訊息" TABTEXT="地震基金訊息">
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%@include file="/pages/commonProp/compensate/PropCompEarthquakeFund.jsp"%>
					</DIV>
				</CENTER>
			</mpc:page>
		</mpc:container>
		<TABLE id="btnCommon" class="common">
			<TR>
				<TD align="center">
					<c:if test="${param.editTypeOther!='SHOWTASK'}">
						<%-- 保存通用按钮 --%>
						<%@include file="/pages/commonProp/compensate/PropCompensateSave.jsp"%>
					</c:if>
				</TD>
			</TR>
		</TABLE>

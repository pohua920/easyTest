...
<script src="${ctx }/pages/commonProp/regist/js/PropRegistEdit.js"></script>
			...
			<TR>
				<TD align="left">
					<c:if test="${param.editTypeOther!='SHOWTASK'}">
						...
						<c:if test="${editType=='ADD'}">
							<input type="button" disabled name="messageSave" class="bigbutton" value="<s:text name='button.claimsProcessingRecords.value'/>"
								onclick="openWinSave('${prpLregist.registNo}','${prpLregist.policyNo}','${prpLregist.riskCode}','regis','')">
							<%--赔案处理记录  --%>
						</c:if>
					</c:if>
		...
		<mpc:container ID="oMPC">
			<c:choose>
				<c:when test="${prpLregist.cancelDate!=null&&prpLregist.cancelDate!=''}">
					<mpc:page ID="tabMain" TABTITLE="<s:text name="menu.regist.edit"/>" TABTEXT="<s:text name="menu.regist.edit"/><s:text name="claim.cancelled"/>"><%-- 已注销 --%>
				</c:when>
				<c:otherwise>
				...
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%-- 1.报案主信息头 --%>
						<%@include file="/pages/commonProp/regist/PropRegistMainHeadEdit.jsp"%>
						<%-- 2.报案主信息中 --%>
						<%@include file="/pages/commonProp/regist/PropRegistMainMiddleEdit.jsp"%>
						<%-- 3.报案主信息结尾 --%>
						<%@include file="/pages/commonProp/regist/PropRegistMainTailEdit.jsp"%>
		...
		<TABLE id="btnCommon" class="common">
			<TR>
				<TD align="center">
					<%-- 6.保存通用按钮 --%>
					<c:if test="${param.editTypeOther!='SHOWTASK'}">
						<%@include file="/pages/commonProp/regist/PropRegistSave.jsp"%>
					</c:if>
				</TD>
			</TR>
		</TABLE>

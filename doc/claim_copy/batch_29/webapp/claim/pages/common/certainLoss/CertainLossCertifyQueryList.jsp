<%--
****************************************************************************
* DESC       ：定损结果
* AUTHOR     ：理赔组
* CREATEDATE ：2004-07-13
* MODIFYLIST ：   id       Date            Reason/Contents
*          ------------------------------------------------------
****************************************************************************/
--%>

<%@ page contentType="text/html; charset=GBK" %>
<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html" %>
<%@ taglib uri="/WEB-INF/claim-app.tld" prefix="app" %>
<%@ include file="/common/taglibs.jsp"%>

<html:html locale="true">
<head>
    <app:css />
<title><s:text name="title.claimBeforeEdit.queryClaim" /></title>
<script src="/claim/common/js/showpage.js">
</script>
  <html:base/>
</head>

<body  >

<table  class="common" cellpadding="5" cellspacing="1" >
		<tr>
			<td colspan=5 class="formtitle">
				<s:text name="certainLoss.queryCertainLoss" />
			</td>
		</tr>
		<%--查询定损信息--%>
   <tr>
			<td class="centertitle">
				<s:text name="certainLoss.prpLscheduleMainWF.caseState" />
			</td>
			<%--案件状态--%>
			<td class="centertitle">
				<s:text name="regist.prpLregist.registNo" />
			</td>
			<%--报案号--%>
			<td class="centertitle">
				<s:text name="db.prpLclaim.policyNo" />
			</td>
			<td class="centertitle">
				<s:text name="certainLoss.feeCode" />
			</td>
			<%--定损人代码--%>
			<td class="centertitle">
				<s:text name="certainLoss.feeDateGeneration" />
			</td>
			<%--定损/代定损日期--%>
   </tr>
		<%
			int index = 0;
		%>
   <c:if test="${prpLverifyLoss.verifyLossList!==null}">
   <c:forEach var="prpLverifyLoss1" items="${prpLverifyLoss.verifyLossList}">
<%
          if(index %2== 0)
               out.print("<tr class=listodd>");
          else
               out.print("<tr class=listeven>");
				%>
				<tr class=common>
					<td align="center">
	
        <c:if test="${prpLverifyLoss1.status=='1'}">
							<s:text name="common.status.untreated" />
							<%--未处理--%>
        </c:if>
         <c:if test="${prpLverifyLoss1.status=='2'}">
							<s:text name="common.status.intreating" />
							<%--正处理--%>
        </c:if>
         <c:if test="${prpLverifyLoss1.status=='3'}">
							<s:text name="common.status.treated" />
							<%--已处理--%>
        </c:if>
         <c:if test="${prpLverifyLoss1.status=='4'}">
							<s:text name="common.status.submited" />
							<%--已提交--%>
        </c:if>
         <c:if test="${prpLverifyLoss1.status=='5'}">
							<s:text name="common.status.revoked" />
							<%--已撤消--%>
          </c:if>
      </td>
					<td align="center">
						<a href="/claim/certifyFinishQueryList.do?prpLcertifyCertifyNo=${prpLverifyLoss1.registNo}&editType=EDIT&nodeType=certa">${prpLverifyLoss1.registNo}</a>
					</td>
      <td align="center">${prpLverifyLoss1.policyNo}</td>
      <td align="center">${prpLverifyLoss1.handlerCode}</td>
      <td align="center">${prpLverifyLoss1.defLossDate}</td>
    </tr>
				<%
					index++;
				%>
    </c:forEach>
    </c:if>
      <tr class="listtail">
	      <td colspan="5">
				<s:text name="certainLoss.totalInquiries" />
				<%--共查询出--%><%=index%><s:text name="certainLoss.meetRecord" />
				<%--条满足条件的记录--%>
	      </td>
      </tr>
  </table>
  </tr>
</table>
<%@include file="/common/meta_js.jsp"%>
</body>
</html:html>
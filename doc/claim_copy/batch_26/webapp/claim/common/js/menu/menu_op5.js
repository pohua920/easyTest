...
function oCM(snum, cnum) {
	if (cP[snum][cnum].length > 0 && cI[snum][cnum].length > 0) {
		...
		if (cI[snum][cnum] != '') {
			miV(d.getElementById('childshadow' + snum + '-' + cnum));
		}
...
function goToUrl(cUrl, cTarg) {
	if (cellCLICK || (cellCLICK == mu)) {
		if (cTarg == "_self") {
			...
			var newwin = open(cUrl, "", "status=yes,scrollbars=yes,toolbar=yes,menubar=yes,location=yes");
		} else {
			if (top.frames[cTarg]) {
...
if (!free) {
	if (stretchMENU) {
		...
		if (showBORDERS) {
			T += '<span id="stretchnav" onmouseover="cM()" style="' + mnDisplay + '\;position:absolute; top:' + (aP[xp] + bSIZE) + '\; left:' + bSIZE + 'px\; width:' + (bW - (2 * bSIZE)) + '\;"><table cellpadding=0 cellspacing=0 border=0 ' + bks[0] + ' style="width:' + (bW - (2 * bSIZE)) + '\; height:' + (nH) + '\;"><tr><td></td></tr></table></span>';
		}
	} else if (showBORDERS) {
...
for (i = 0; i < mI.length; i++) {
	...
	if (mI[i][0] == "") {
		...
		if ((cellCLICK || (cellCLICK == mu)) && mI[i][4] == "_blank") {
			tmItem = 'href="' + mI[i][0] + '" onclick="goToUrl(mI[' + i + '][0],\'_blank\')\;return false" ';
		}
		T += '<a ' + colr + tmItem;
	}
...
for (c = 0; c < mI.length; c++) {
	...
	if ((vOFFSET + sP[c][3]) > 0) {
		M += '<div id="gridblocker' + c + '" style="width:' + sP[c][0] + 'px\; height:' + (vOFFSET + sP[c][3] + 1) + '\; visibility:hidden\; z-index:' + (zORDER + 1) + '\;position:absolute\; top:' + (aP[xp] + thisAH[c] - 1) + 'px\; left:' + acL + 'px\;" onmouseover="clearTimeout(rTimer)\; rCount=0\;"></div>';
	}
	...
	if (gblw > 0) {
		M += '<span id="gridLblocker' + c + '" style="width:' + gblw + 'px\; height:' + (((snH + bSIZE) * sI[c].length) + ((snH + 1) * Xrows[c]) + shb + shSIZE) + 'px\; visibility:hidden\; z-index:' + (zORDER + 6) + '\;position:absolute\; top:' + (aP[xp] + thisAH[c] + vOFFSET + sP[c][3]) + 'px\; left:' + gbposL + 'px\;" onmouseover="clearTimeout(rTimer)\; rCount=0\;"></span>';
	}
	...
	for (i = 0; i < sI[c].length; i++) {
		...
		if (sI[c][i][0] == "#") {
			...
			if ((cellCLICK || (cellCLICK == mu)) && sI[c][i][2] == "_blank") {
				tsItem = 'href="' + sI[c][i][0] + '" onclick="goToUrl(sI[' + c + '][' + i + '][0],\'_blank\')\;return false" ';
			}
			M += '<a ' + solr + tsItem;
		}
	...
	if (shCOLOR != "") {
		M += '<table cellpadding=0 cellspacing=0 border=0 ' + bks[6] + ' id="shadow' + c + '" style="visibility:hidden\; position:absolute; z-index:' + (zORDER + 6) + '\; top:' + (aP[xp] + thisAH[c] + vOFFSET + sP[c][3] + shSIZE - shb) + 'px\; left:' + (acL + shSIZE - shb) + 'px\; height:' + (SUBaT - (aP[xp] + thisAH[c] + vOFFSET + sP[c][3]) - svPADDING - 2 + shb - svtOFFSET) + '\; width:' + (sP[c][0] + shb) + '\;"><tr><td>&nbsp;</td></tr></table>';
	}
		...
		if (cP[c][sq].length > 0 && cI[c][sq].length > 0) {
			...
			for (cj = 0; cj < cI[c][sq].length; cj++) {
				...
				if ((cellCLICK || (cellCLICK == mu)) && cI[c][sq][cj][2] == "_blank") {
					tcItem = 'href="' + cI[c][sq][cj][0] + '" onclick="goToUrl(cI[' + c + '][' + sq + '][' + cj + '][0],\'_blank\')\;return false"';
				}
			...
			//alert("drop shadow"); 
			if (shCOLOR != "") {
				C += '<table cellpadding=0 cellspacing=0 border=0 ' + bks[6] + ' id="childshadow' + c + '-' + sq + '" style="visibility:hidden\; position:absolute; z-index:' + (zORDER + 10) + '\; top:' + (((chTop[c][sq] - sbSIZE - svPADDING - 2) - svtOFFSET) + shSIZE + chvOFFSET + cP[c][sq][3] - shb) + 'px\; left:' + (chL + shSIZE - shb) + 'px\; height:' + (((snH + sbSIZE + 1) * cI[c][sq].length) + ((snH) * XCrows[c][sq]) + sbSIZE + shb) + '\; width:' + (cP[c][sq][0] + shb) + '\;"><tr><td>&nbsp;</td></tr></table>';
			}

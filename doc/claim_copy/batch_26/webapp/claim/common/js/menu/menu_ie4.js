	...
	if (smObj[mid]) {
		...
		if (tsm == "udm") {
			...
			for (i = 0; i < sI[mid].length; i++) {
				...
				txi = '';
				if (sP[mid][2] == "left") {
					txi = 'margin-left:' + stINDENT + 'px';
				}
				if (sP[mid][2] == "right") {
					txi = 'margin-right:' + stINDENT + 'px';
				}
				txv = '';
				if (svtOFFSET > 0) {
					txv = 'margin-top:' + svtOFFSET + 'px';
				}
				if (svtOFFSET < 0) {
					txv = 'margin-bottom:' + (0 - svtOFFSET) + 'px';
				}
		...
		if (csm == "udm") {
			...
			if (!movedCM[mid][cid]) {
				...
				if (cP[mid][cid][1] == "right") {
					scObjLeft -= cP[mid][cid][0];
				}

				if (win) {
					scObjTop += scObj.offsetParent.offsetParent.offsetTop;
				}
				if (mac) {
					for (i = 0; i < cid; i++) {
			...
			for (i = 0; i < cI[mid][cid].length; i++) {
				...
				var Trows = splitT.length;
				if (Trows > 1) {
					chAry[i] = (subNavH * Trows) - (svPADDING * (Trows - 1));
				}
				...
				if (cP[mid][cid][2] == "left") {
					cxi = 'margin-left:' + stINDENT + 'px';
				}
				if (cP[mid][cid][2] == "right") {
					cxi = 'margin-right:' + stINDENT + 'px';
				}
				cxv = '';
				if (svtOFFSET > 0) {
					cxv = 'margin-top:' + svtOFFSET + 'px';
				}
				if (svtOFFSET < 0) {
					cxv = 'margin-bottom:' + (0 - svtOFFSET) + 'px';
				}
	...
	if (sI[i].length > 0) {
		...
		if (smTopPad[i] < 0) {
			...
			if (smLeftPad[i] <= mI[i][2]) {
				...
				if (sP[i][1] == "left") {
					smLeft[i] += mI[i][2];
					inLeft[i] = smLeftPad[i] - mI[i][2];
				} else {
					smLeft[i] -= mI[i][2];
					inLeft[i] = 0 - (smLeftPad[i] - mI[i][2]);
				}
			}
		} else {
			inTop[i] = smTopPad[i];

...
var ntl = 0;
for (intl = 0; intl < mI.length; intl++) {
	if (mI[intl][1] != "") {
		ntl++;
	}
}
...
function oM(n, gridTrue) {
	...
	if (sI[n] != '' && sI[n].length > 0) {
		miV(document.layers["shadow" + n]);
	}
...
// open frame
if (!free) {
	T += '<layer ' + mnDisplay + ' id="mainnav" top=' + aP[xp] + ' left=' + rL + '><table cellpadding=0 cellspacing=' + bSIZE + ' border=0><tr>';
}
...
function findWidths(lyo, lyW, lyH) {
	...
	if (mI[lc][3] == "right") {
		lyo.left = (mI[lc][2] - lyW - tINDENT);
	}
	if ((lyH < thisNH[lc])) {
		lyo.top = parseInt((thisNH[lc] - lyH) / 2) + vtOFFSET;
	}
	lc++;
	if (lc == mI.length) {
		lc = 0;
...
for (i = 0; i < mI.length; i++) {
	if (free) {
		T += '<layer ' + mnDisplay + ' id="free' + i + '" top=' + aP[i] + ' left=' + aPL[i] + ' ' + bks[1] + '><table cellpadding=0 cellspacing=' + bSIZE + ' border=0><tr>';
	}
...
for (i = 0; i < mI.length; i++) {
	...
	if ((cellCLICK || (cellCLICK == mu)) && mI[i][4] == "_blank") {
		tcItem = '<a class="translink" href="' + mI[i][0] + '" onclick="activateClick(mI[' + i + '][0])\;return false" ' + aTxt[i] + '>';
	}
	T += tcItem + '<font color="' + aRoll + '">' + mI[i][1] + '</font></a>';
	T += '</div></layer></td></tr></table></layer>';
	T += '</layer>';
	if (!free) {
		subDefLeft += (mI[i][2] + bSIZE);
	}
}
...
for (i = 0; i < mI.length; i++) {
	...
	if (mI[i][0] == "") {
		T += '<layer id="defaultLayer' + i + '" top=' + (aP[xp] + bSIZE) + ' left=' + subDefLeft + ' width=' + mI[i][2] + ' height=' + (thisNH[i]) + ' onmouseover="oM(' + i + ')"><layer width="100%" height="100%" ' + aTxt[i] + '></layer></layer>';
	}
	if (!free) {
		subDefLeft += (mI[i][2] + bSIZE);
	}
...
if (((bW > (nS + bSIZE)) || (stAdjust > 0)) && (stretchMENU || showBORDERS) && (!free)) {
	if (ralign) {
		T += gdL + ' top=' + aP[xp] + ' left=' + aL + ' ' + ogcm + ' width=' + (eS - (bSIZE * 2) - abR) + '></layer>';
	}
	if (ralign && abR > 0) {
		T += gdR + ' top=' + aP[xp] + ' left=' + (eS - abR + nS) + ' ' + ogcm + ' width=' + (abR - (bSIZE * 2)) + '></layer>';
	}
	if (lalign && abR > 0) {
		T += gdL + ' top=' + aP[xp] + ' left=' + aL + ' ' + ogcm + ' width=' + abR + '></layer>';
	}
	if (lalign) {
		T += gdR + ' top=' + aP[xp] + ' left=' + (abR + nS) + ' ' + ogcm + ' width=' + (eS - abR) + '></layer>';
	}
	if (calign) {
...
function findSubWidths(lct, slyo, slyW, slyH, thisLH) {
	...
	if (sP[lct][2] == "right") {
		slyo.left = ((sP[lct][0] - (2 * sbSIZE)) - slyW - stINDENT);
	}
	if (slyH < thisLH) {
		slyo.top = (thisLH - slyH) / 2 + svtOFFSET;
	}
	slc++;
	if (slc == sP.length) {
		slc = 0;
...
function findChildWidths(lct, cht, slyo, slyW, slyH, thisLH) {
	if (cP[lct][cht][2] == "left") {
		slyo.left = stINDENT;
	}
	if (cP[lct][cht][2] == "center") {
		slyo.left = 0;
	}
	if (cP[lct][cht][2] == "right") {
		slyo.left = ((cP[lct][cht][0] - (2 * sbSIZE)) - slyW - stINDENT);
	}
	if (slyH < thisLH) {
		slyo.top = (thisLH - slyH) / 2 + svtOFFSET;
	}
	clc++;
	if (clc == cI[lct].length) {
		clc = 0;
	}
}
...
function findLayerHeight(sn, sh) {
	if (shCOLOR != "") {
		document.layers["shadow" + sn].clip.bottom = sh + shb;
	}
}

function findChildHeight(cnn, cqn, cch) {
	if (shCOLOR != "") {
		document.layers["childshadow" + cnn + "-" + cqn].clip.bottom = cch + shb;
	}
	if ((chhOFFSET + cP[cnn][cqn][4]) > 0) {
		document.layers["childgrid" + cnn + "-" + cqn].clip.bottom = cch + shb + shSIZE;
	}
}
...
function subRollOpen(omic, omis) {
	if (sI[omic][omis][4]) {
		miV(document.layers["subroll" + omic + "-" + omis]);
	}
}
...
for (c = 0; c < mI.length; c++) {
	...
	if (shCOLOR != "") {
		M += '<layer id="shadow' + c + '" ' + bks[6] + ' visibility=hide top=' + (aP[xp] + thisAH[c] + layerVOff + shSIZE + tj - shb) + ' left=' + (acL + shSIZE - shb) + ' width=' + (sP[c][0] + shb) + ' height=10>&nbsp;</layer>';
	}
	...
	for (sq = 0; sq < sI[c].length; sq++) {
		...
		if (sI[c][sq][3] != "none") {
			if (altDISPLAY == "status") {
				sTxt[sq] = ' onmouseover="return writeStatus(sI[' + c + '][' + sq + '][3])" onmouseout="return writeStatus(altNull)" ';
			}
		}
		...
		if ((cellCLICK || (cellCLICK == mu)) && sI[c][sq][2] == "_blank") {
			tcItem = '<a class="translink" href="' + sI[c][sq][0] + '" onclick="activateClick(sI[' + c + '][' + sq + '][0])\;return false" ' + sTxt[sq] + '>';
		}
		...
		if (cP[c][sq].length > 0 && cI[c][sq].length > 0) {
			...
			// child dropshadow
			if (shCOLOR != "") {
				C += '<layer id="childshadow' + c + '-' + sq + '" ' + bks[6] + ' visibility=hide top=' + ((chTop[c][sq] + shSIZE + chvOFFSET) + cP[c][sq][3] + tj - shb) + ' left=' + (chL + shSIZE - shb) + ' width=' + (cP[c][sq][0] + shb) + ' height=10>&nbsp;</layer>';
			}
			...
			for (cj = 0; cj < cI[c][sq].length; cj++) {
				...
				if (cI[c][sq].length > 0) {
					...
					if ((cellCLICK || (cellCLICK == mu)) && cI[c][sq][cj][2] == "_blank") {
						tcItem = '<a class="translink" href="' + cI[c][sq][cj][0] + '" onclick="activateClick(cI[' + c + '][' + sq + '][' + cj + '][0])\;return false" ' + cTxt[c][sq][cj] + '>';
					}
...
function moveObject(mobj, tmob) {
	if (mobj) {
		mobj.top = eval(window.pageYOffset) + tmob;
	}
}
...
function makeStatic() {
	...
	for (i = 0; i < mI.length; i++) {
		...
		if (free) {
			moveObject(d.layers["free" + i], aP[xp]);
		}
...
function goToUrl(cUrl, cTarg) {
	if (cellCLICK || (cellCLICK == mu)) {
		if (cTarg == "_self") {
			...
			if (top.frames[cTarg]) {
				top.frames[cTarg].document.location = cUrl
			} else {
				//to support iframe

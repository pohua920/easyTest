...
if (!kde && !saf) {
	...
	if ((mz7 && ns6) && (agt.indexOf("rc1") != -1 || agt.indexOf("rv:1.2") != -1)) {
		sbaa = 19;
	}
	if (((mz7 && ns6) && win && (agt.indexOf("windows nt 5.1") != -1)) || mac || ns6) {
		if (document.width <= (innerWidth - 30)) {
			bWidth -= sbaa;
		}
	} else if ((mz7 && ns6) && win && (agt.indexOf("windows nt 5.0") != -1)) {
		if (document.width <= (innerWidth - 16)) {
			bWidth -= 16;
		}
	} else if (win) {
		if (document.width <= (innerWidth - 17)) {
			bWidth -= 17;
		}
	}
	if (lin) {
	...
	if (menuALIGN == "free" || remoteTRIGGERING || (!showBORDERS && !stretchMENU)) {
		bWidth -= sbaa;
	}
}
...
function openSubMenu(mid) {
	...
	if (kde || saf || (op7 && shType == "string")) {
		applyWidth = 'width:' + sP[mid][0] + 'px\;';
	}
	...
	for (i = 0; i < sI[mid].length; i++) {
		...
		if (!allowForSCALING || kde || saf || op7) {
			textScale = 'width:' + (sP[mid][0] - (2 * sbSIZE)) + 'px\;';
		}
	...
	if (kde || saf) {
		applyWidth = 'width:' + (sP[mid][0] + slMargin) + 'px\;';
	}
	applyWidthII = 'width:auto;min-width:' + (sP[mid][0] + swPad - sbSIZE) + 'px\;white-space:nowrap\;';
	if (kde || saf) {
		applyWidthII = 'width:' + (sP[mid][0] + swPad - sbSIZE) + 'px\;';
	}
	...
	for (i = 0; i < sI[mid].length; i++) {
		...
		stObj.style.top = stt;
		if (sP[mid][2] == "left") {
			stObj.style.left = stINDENT + "px";
		}
		if (sP[mid][2] == "right") {
			stObj.style.left = (0 - stINDENT) + "px";
		}
	}
...
function openChildMenu(mid, cid) {
	...
	if (kde || saf || (op7 && shType == "string")) {
		applyWidth = 'width:' + cP[mid][cid][0] + 'px\;';
	}
	...
	for (i = 0; i < cI[mid][cid].length; i++) {
		...
		if (!allowForSCALING || kde || saf || op7) {
			textScale = 'width:' + (cP[mid][cid][0] - (sbSIZE * 2)) + 'px\;';
		}
	...
	if (kde || saf) {
		applyWidth = 'width:' + (cP[mid][cid][0] + clMargin) + 'px\;';
	}
	applyWidthII = 'width:auto;min-width:' + (cP[mid][cid][0] + swPad - sbSIZE) + 'px\;white-space:nowrap;';
	if (kde || saf) {
		applyWidthII = 'width:' + (cP[mid][cid][0] + swPad - sbSIZE) + 'px\;';
	}
	...
	for (i = 0; i < cI[mid][cid].length; i++) {
		...
		if (cP[mid][cid][2] == "left") {
			ctObj.style.left = stINDENT + "px";
		}
		if (cP[mid][cid][2] == "right") {
			ctObj.style.left = (0 - stINDENT) + "px";
		}
	}
...
if (menuALIGN != "free") {
	...
	if (stretchMENU || showBORDERS) {
		...
		if (stretchMENU && showBORDERS) {
			mn += '<div style="margin:' + bSIZE + 'px\;width:' + (bWidth - (2 * bSIZE)) + 'px\;height:' + navH + 'px\;' + bks[0] + 'cursor:default">&nbsp;</div>';
		}
		mn += '</div>';
	}
...
function loadFunction() {
	...
	if (remoteTRIGGERING) {
		document.addEventListener("mousemove", menuListen, false);
	}
	...
	for (i = 0; i < mI.length; i++) {
		...
		if (typeof mtObj != "undefined") {
			...
			mtObj.style.top = mtt;
			if (mI[i][3] == "left") {
				mtObj.style.left = tINDENT + "px";
			}
			if (mI[i][3] == "right") {
				mtObj.style.left = (0 - tINDENT) + "px";
			}

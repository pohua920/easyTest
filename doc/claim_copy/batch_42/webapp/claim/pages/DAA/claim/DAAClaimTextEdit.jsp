...
<table class="common" align="center" style="width: 100%">
	<tr>
		<td class="common" style="text-align: left">
			<img style="cursor: hand;" src="/claim/images/butCollapseBlue.gif" name="LtextImg" onclick="showPage(this,Ltext)">
			...
			<c:if test="${prpLltext.id.textType == '3'}">
				<s:text name="db.prpLltext.text3" />
				<%--[texttype=1] 出险摘要--%>
			</c:if>
			<c:if test="${prpLltext.id.textType == '4'}">
				<s:text name="db.prpLltext.text4" />
				<%-- 立案报告--%>
			</c:if>
			<c:if test="${prpLltext.id.textType == '5'}">
				<s:text name="db.prpLltext.text5" />
				<%-- 追偿及转让文字--%>
			</c:if>
			...
			<c:if test="${prpLltext.id.textType == '09'}">
				<s:text name="db.prpLltext.text09" />
				<%-- 出险摘要 --%>
			</c:if>
			<c:if test="${prpLltext.id.textType == '10'}">
				<s:text name="db.prpLltext.text10" />
				<%-- 注销/拒赔文本 --%>
			</c:if>
			<br>
				...
				<tbody>
					<tr>
						<td class="title" style="text-align: center; width: 100%">
							<textarea style="wrap: hard" rows="15" cols="80" id="prpLltextContextInnerHTML" name="prpLltextContextInnerHTML">${prpLltext.context}</textarea>
						</td>
					</tr>
				</tbody>
			</table>

...
/*
mantis： CLM0001，處理人員：David，需求單編號：CLM0001 
     處理過程：依需求新增的西元年元件，原型是 UIRCDateTag，因某些功能不會影響，且為必免不必要的錯誤，所以不做調整。
*/
public class UIADateTag extends TagSupport {
	...
	/**
	 * <ad:date format="yy-MM-dd" value="${t1}" name="data"/> <input
	...
	private void print(Object date) throws Exception {
		...
		if (isNotEmpty(onclick) || wdatePicker) {
			...
			if(wdatePicker){
				input.append("WdatePicker({dateFmt:'" + getFormat()+"'");

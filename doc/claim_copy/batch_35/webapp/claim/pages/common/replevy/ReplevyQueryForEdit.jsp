...
<body onload="initPage();" class="yui-skin-sam">
    <form name="fm" action="${ctx}/replevyQuery.do" method="post" onsubmit="return validateForm(this);">
        <input type="hidden" name="underWriteFlag" value="2">
        ...
        <table border="0" align="center" cellpadding="5" cellspacing="1" class="common">
            ...
            <tr>
                ...
                <td class='title'>
                    <s:text name="compensate.computeBookNum" />:<%--計算書號碼 --%>
                </td>
                <td class='input'>
    ...
    function init(){
        YAHOO.widget.DataTable.formatLink = function(elCell, oRecord, oColumn, oData) {
        	if(oColumn.key =="compensateNo"){
                elCell.innerHTML="<a href='${ctx}/replevyFinishQueryList.do?editType=EDIT&claimNo="+oRecord.getData("claimNo")+"&compensateNo="+oRecord.getData("compensateNo")+"&swfLogFlowID="+oRecord.getData("flowID")+"&swfLogLogNo="+oRecord.getData("logNo")+"'>"+oData+"</a>";
            }else if(oColumn.key =="claimNo"){
                elCell.innerHTML="<a href='${ctx}/replevyFinishQueryList.do?editType=EDIT&claimNo="+oRecord.getData("claimNo")+"&compensateNo="+oRecord.getData("compensateNo")+"&swfLogFlowID="+oRecord.getData("flowID")+"&swfLogLogNo="+oRecord.getData("logNo")+"'>"+oData+"</a>";
            }else if(oColumn.key =="handlerCode"){
                if(oRecord.getData("flowID")==""){
                    elCell.innerHTML=oRecord.getData("operatorName")+"("+oRecord.getData("operatorCode")+")";
                }else if(oData !=""){
                    elCell.innerHTML=oRecord.getData("handlerName")+"("+oData+")";
                }
         ...
         contentColumnHeaders =[
            ...
            {key:"handlerCode",label:"<s:text name="db.prpLlawsuit.operatorCode" />",width:"40em",sortable:true,formatter:YAHOO.widget.DataTable.formatLink},<%--操作员--%>
            {key:"flowInTime",label:"駁回時間",width:"40em",sortable:true,formatter:YAHOO.widget.DataTable.formatLink}
            ];
    }

...
<body >
	<form name="fm" action="${ctx }/claimReplevyQuery.do" method="post" >
		...
		<table cellpadding="3" cellspacing="1"  class="common"
			<thead>
				<tr>
					<td class="formtitle" colspan="14">應追償未追償查詢</td>
				</tr>
				<tr class="tableHead">
					...
					<td class="centertitle">預估追償日期</td>
					<td class="centertitle">追償人員</td>
			...
			<c:forEach items="${requestScope.resultList}" var="mapObject" varStatus="stat">
				...
				<c:forEach items="${mapObject}" var="entry">
					<c:choose>
						<c:when test="${entry.key == 'damagestartdate' || entry.key == 'claimdate' || entry.key == 'endcasedate' || entry.key == 'replevydate' || entry.key == 'inputdate' }">
							<td align="center"><fmt:formatDate value="${entry.value}" pattern="yyyy-MM-dd"/></td>
						</c:when>
						<c:when test="${entry.key == 'replevyamount'}">

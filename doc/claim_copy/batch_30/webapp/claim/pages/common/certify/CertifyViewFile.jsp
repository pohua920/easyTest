...
<body class="interface" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onload="initPage();progressStop();">
	<form name=fm method="post"
		action="${ctx}/certify/certifyViewFile.do?isCase=${isCase }&editType=${editType}&nodeTypeUpload=${nodeTypeUpload}&directType=${directType}&itemcode=${strLossItemName}&businessNo=${businessNo}">
		...
		<input type="hidden" name="pageNo1" value="${pageNo+1 }" />
		<input type="hidden" name="BusinessNo" value="${businessNo}">
		<table border="0" cellpadding="4" cellspacing="1" bgcolor="#2D8EE1" class="title" width="100%">
			<tr>
				...
				<td class="subformtitle" style="width: 40%" align="center">
				...
				<!-- reasion: 由於display的数据，在翻页之後，边成="null" 这样的数据，所以需要增加判断条件"null".equals(display) -->
					...
					<s:else>
						<s:if test="#attr.directType=='undefined'">
							<s:if test="#prpLcertifyImg.typeCode!=null&&#prpLcertifyImg.typeCode.length()>1&&#prpLcertifyImg.typeCode.substring(0,2)=='99'">
								<s:set var="strFileName" value="%{'/claim/uiviewimg?BusinessNo='+#prpLcertifyImg.id.businessNo+'&SerialNo=' + #prpLcertifyImg.id.serialNo}" scope="page" />
								<tr>
									<td class="prompt">${prpLcertifyImg.picName }</td>
									<td class="prompt">
						...
						<s:else>
							<s:set var="strFileName" value="'/claim/uiviewimg?BusinessNo='+ #prpLcertifyImg.id.businessNo+ '&SerialNo='+ #prpLcertifyImg.gid.serialNo" scope="page" />
							<tr>
								<td class="prompt">${prpLcertifyImg.picName }</td>
								<td class="prompt">
									<s:set var="fileExt" value="" scope="page" />
			...
			<s:if test="#attr.display!=null||#attr.display==''">
				<tr>
					<td colspan=5 class="centertitle">
						<input type="button" name="buttonSave" class="button" value="<s:text name='button.confirmDelete.value' />" onclick="return submitFormli(this)">
						<%--确认删除--%>

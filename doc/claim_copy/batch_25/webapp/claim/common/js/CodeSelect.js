 ...
 * @param callBackMethod 执行完後回调的方法
 * @param getDataMethod 取值方法，当codeType为custom时取数据用（暂不支持)
 */
function code_CodeSelect(field, codeType, codeRelation, isClear,
		otherCondition, typeParam, varParamField, callBackMethod, getDataMethod) {
	if (event.type == "keyup") {
	...
	if ((varParamField != undefined && trim(varParamField) != "")
			&& (typeParam == undefined || trim(typeParam) == "")) {
		if (varParamField.indexOf(",") != -1) {
			var fields = varParamField.split(",");
			for ( var i = 0; i < fields.length; i++) {
				typeParam += document.getElementsByName(fields[i])[0].value;
				typeParam += ",";
			}
	...
	inCodeQuery = true;
	private_Code_CallServiceByDialog(field, "select", codeType, codeRelation,
			isClear, otherCondition, typeParam, callBackMethod, getDataMethod);
	inCodeQuery = false;
}
/**
 ...
 */
function code_CodeQuery(field, codeType, codeRelation, isClear, otherCondition,
		typeParam, varParamField, callBackMethod, getDataMethod) {
	if (event.type == "keyup") {
		var charCode = window.event.keyCode;
		if (!(charCode == 13 & window.event.ctrlKey)) {
			return;
	...
	inCodeQuery = true;
	private_Code_CallServiceByDialog(field, "query", codeType, codeRelation,
			isClear, otherCondition, typeParam, callBackMethod, getDataMethod);
	inCodeQuery = false;
}
 ...
 */
function private_Code_ParseParams(field, codeMethod, codeType, codeRelation,
		isClear, otherCondition, typeParam, callBackMethod, getDataMethod) {
	...
	if (otherCondition == null || otherCondition == undefined
			|| otherCondition == "null") {
		otherCondition = "";
	}
	...
	for ( var i = 0; i < conditionsCount; i++) {
		...
		if (condStatement.indexOf("[]") == -1) {
			...
			if (startPos == endPos - 1) {
				condStatement = condStatement.substring(0, startPos + 1)
						+ elementOrder + condStatement.substring(endPos);
			}
			try {
				condValue = eval(condStatement);
			} catch (E) {
...
function private_Code_CallServiceByDialog(field, codeMethod, codeType,
		codeRelation, isClear, otherCondition, typeParam, callBackMethod,
		getDataMethod) {
	private_Code_ParseParams(field, codeMethod, codeType, codeRelation,
			isClear, otherCondition, typeParam, callBackMethod, getDataMethod);
	...
	var handle = window
			.showModalDialog(
					url,
					window,
					"resizable:yes;dialogHide:yes;help:no;resizable:yes;status:no;scroll:yes;dialogWidth:350px;dialogHeight:420px");
 ...
 * @param getDataMethod get data method,when codeType is custom
 */
function code_CodeChange(field, codeType, codeRelation, isClear,
		otherCondition, typeParam, varParamField, callBackMethod, getDataMethod) {
	...
	var myAjax = new Ajax.Request(url, {
		...
		asynchronous : 'false',
		parameters : pars,
		onComplete : setFieldValueForCodeChange
	});
}
/** for query */
...
function cancelFieldValue() {
	...
	var callerWindowObj = window.dialogArguments;
	var openerFm = callerWindowObj.document.forms[0];
	if (document.forms[0].isClear.value == "Y"
			|| document.forms[0].isClear.value == "y") {
		var relations = new Array();
		if (document.forms[0].codeRelation.value.indexOf(",") > -1) {
			relations = document.forms[0].codeRelation.value.split(",");
		} else {

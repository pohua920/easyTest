 ...
 function calRealpayNew(field) {
 	if (isChange(field)) {
 		...
 		if (kindCode != '' && sumDefPay != '' && !isNaN(sumDefPay)) { //核定赔偿为0，不继续下去
 			...
 			//車損、物損賠付資訊
 			//mantis：CLM0299 ，處理人員：DP0713，需求單編號：新核心-理算任務處理賠付訊息--超額險賠付邏輯加入C1及C2險別 
 	 		if("31"==kindCode || "32"==kindCode || "C1"==kindCode || "C2"==kindCode){
 	 			cleanE3E9();
 	 		}
...
//mantis： CLM0086，處理人員：BK007 蘇哲，需求單編號：CLM0086.超額問題開發與測試 -start
 function checkExcess( kindCode , type , identifyNumber ){
	...
	if(type == "0"){
		...
		targetKindCode = "32";
		//mantis：CLM0299 ，處理人員：DP0713，需求單編號：新核心-理算任務處理賠付訊息--超額險賠付邏輯加入C1及C2險別  START
		if ($limitObject.length === 0) {
			$limitObject = $("#limitList").find(":input[name='limitKindCode'][value='C2']").parents("div[name='limitObject']");
			targetKindCode = "C2";
		}
		//mantis：CLM0299 ，處理人員：DP0713，需求單編號：新核心-理算任務處理賠付訊息--超額險賠付邏輯加入C1及C2險別  END
	}else{
		//alert("判斷人傷超額");
		...
		//mantis：CLM0299 ，處理人員：DP0713，需求單編號：新核心-理算任務處理賠付訊息--超額險賠付邏輯加入C1及C2險別  START
		if ($limitObject.length === 0) {
			$limitObject = $("#limitList").find(":input[name='limitKindCode'][value='C1']").parents("div[name='limitObject']");
			targetKindCode = "C1";
		}
		//mantis：CLM0299 ，處理人員：DP0713，需求單編號：新核心-理算任務處理賠付訊息--超額險賠付邏輯加入C1及C2險別  END
	}

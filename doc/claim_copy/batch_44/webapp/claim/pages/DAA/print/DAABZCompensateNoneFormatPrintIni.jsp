...
<%
	...
	String strDamageAddressType = "";
	String strDamageHourMS = ""; //出险时分秒
...
<%if (strRiskCode.equals(ConstantCodes.RISKCODE_DAZ)) {
				...
				lawSumPaid = prpLcompensateDto.getSumThisPaid();
		    ...
		    tdCSumQiangFee.innerHTML      = '&nbsp;本次实付赔款（人民币大写）：'+ '<%=strLawCSumPaid%> ';
    ...
    tdRPolicyNo.innerHTML =  '<%=strPolicyNo%>';
    ...
    <%strDamageHourMS = prpLclaimDto.getDamageStartHour() + "";
			...
			// new PrpLclaimDto().getMinute()//.getMinute()//.getHour()
    ...
    <%//事故责任
            ...
            <%if (prpLclaimDto != null) {
				if (!"".equals(prpLclaimDto.getEndDate().toString()) || !"".equals(prpLclaimDto.getStartDate().toString())) {
			...
			if (policyDto.getPrpCitemCarDtoList() != null) {
				...
				for (index = 0; index < intItemCarCount; index++) {
					prpItemCarDto = (PrpCitemCarDto) policyDto.getPrpCitemCarDtoList().get(0);
			...
			<%if (strRiskCode.equals(ConstantCodes.RISKCODE_DAZ)) {
				...
				if (intCtextCountTmp > 10) {
					if (strContextLaw.length() > 250) {
						countNumber1 = 250;
						countNumber = strContextLaw.indexOf("\\r\\n", countNumber1);
						// strContext = strContext.substring(0,countNumber)+"<br>计算书信息过多，请详见清单。";
						strContextLaw = "计算书信息过多，请详见清单。";
						%>
				...
				while (strContext.indexOf("\\r\\n") != -1)
				   ...
				   tdContextLaw.innerHTML=''+'<br>'+'<%=strContextLaw%>';
					<%//--------赔款费用信息表PrpLcharge*****    
			dblJudgeFee = 0;
			dblCheckFee = 0;
			dblLawFee = 0;
			...
			strDamageAddressType = uiCodeAction.translateCodeCode("DamageAddress", prpLregistDto.getDamageAddressType(), true);%>

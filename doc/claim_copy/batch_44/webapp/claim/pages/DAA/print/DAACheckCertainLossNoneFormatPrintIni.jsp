...
function loadForm() {
	...
	//*****报案信息表PrpLregist*****
	//  tdEstimateLoss.innerHTML = '&nbsp;事故损失估损金额：' + '<%=new DecimalFormat("#,##0.00").format(prpLregistDto.getEstimateLoss())%>' + ' 元';
	...
	// tdInsuredName2.innerHTML = '<%=StringConvert.encode(prpLclaimDto.getInsuredName())%>' + '（被保险人名称）：';
	...
	if (policyDto.getPrpCitemKindDtoList() != null) {
		for (index = 0; index < intItemKindCount; index++) {
			...
			if (strCalculateFlag.equals("Y1")) {
				dblSumAmount1 += prpItemKindDto.getAmount();
			}
			if (strCalculateFlag.equals("Y2")) {

			...
			<td class="subformtitle" style="width: 96%">
				<table class="common" style="width: 100%">
					...
					<tr>
						...
						<td class="input" style="width:18%">
							<s:select name="prpLpayObjectInfoAccountCurrency" class="common" listKey ="key" listValue="value" list="#request.prpLpayObjectInfoCurrencyList" onchange="getPrpLpayObjectInfoExchRate(this);"/>
					...
					<tr>
						...
						<td class="input" style="width: 18%">
							<input name="prpLpayObjectInfoOwnerName" class="input" maxlength="120" value="">
							<img src="${ctx}/images/bgMarkMustInput.jpg">
						</td>
					...
					<tr>
						...
						<%--mantis：CLM0113   傷害險增加AML功能，處理人員：BL061 張明財，需求單編號：CLM0113  傷害險增加AML功能 start--%>
						<td class="input" style="width: 15%">洗錢狀態回覆：</td>
							<td class="input" style="width: 18%">
								...
								<span name="spanCutBack" style="display: none;">取消禁背：</span>
								<span name="spanCutBack" style="display: none;"> <select name="prpLpayObjectInfoCutBack" class='common' style="width: 50%">
					...
					<!--  \webapp\claim\pages\commonAcci\compensate\EditPrpdpaymentaccountPage.jsp -->
						<td class="title" style="width: 15%">郵遞區號：</td>
						<td class="input" style="width: 18%">
							<!-- mantis：CLM0145，處理人員：DP0713，需求單編號：CLM0145，.新核心-理算任務處理賠付對象郵遞區號長度檢核 -->
							<input name="prpLpayObjectInfoAreaCode" class="input" value="${prpCinsured.postCode }" maxlength="3">
							<img src="${ctx}/images/bgMarkMustInput.jpg" complete="complete" />
						</td>
						<td class="title" style="width: 15%">郵遞地址：</td>
						<td class="input" style="width: 50%" colspan="3">
							<input name="prpLpayObjectInfoCourierAddress" class="input" value="${prpCinsured.postAddress }">
							<img src="${ctx}/images/bgMarkMustInput.jpg" complete="complete" />
						</td>
					</tr>
								...
								<td class="subformtitle" style="width: 96%">
									<table class="common" style="width: 100%">
										...
										<tr>
											...
											<%--理賠金額 --%>
											<td class="input" style="width: 18%">
												<input name="prpLpayObjectInfoPayAmount" class="readonly" readonly value="<fmt:formatNumber value="${prpLpayObject.payAmount}" pattern="#"/>" onfocus="cacheData(this);" onchange="validateMoney(this);" title="理賠金額" style="width: 80px;" />
												<img src="${ctx}/images/bgMarkMustInput.jpg">
											</td>
											<td class="title" style="width:15%">支付幣別：</td>
											<td class="input" style="width:18%">
												<s:select name="prpLpayObjectInfoAccountCurrency" class="common" value="#attr.prpLpayObject.accountCurrency" listKey ="key" listValue="value" list="#request.prpLpayObjectInfoCurrencyList" onchange="getPrpLpayObjectInfoExchRate(this);"/>
										...
										<tr>
											...
											<td class="input" style="width: 18%">
												<select name="prpLpayObjectInfoPaymentKind" style="width: 50%" onchange="changePaymentKind(this);">
													<option value="1" <c:if test="${prpLpayObject.paymentKind=='1'}"> <c:out value="selected"/></c:if>>修車廠</option>
													<option value="2" <c:if test="${prpLpayObject.paymentKind=='2'}"> selected</c:if>>材料商</option>
													<option value="3" <c:if test="${prpLpayObject.paymentKind=='3'}"> selected</c:if>>公司行號</option>
													<option value="4" <c:if test="${prpLpayObject.paymentKind=='4'}"> selected</c:if>>個人</option>
													<option value="5" <c:if test="${prpLpayObject.paymentKind=='5'}"> selected</c:if>>公證公司</option>
													<option value="6" <c:if test="${prpLpayObject.paymentKind=='6'}"> selected</c:if>>健保局</option>
												</select> <img src="${ctx}/images/bgMarkMustInput.jpg" complete="complete" />
											</td>
											<td class="input" style="width: 15%">證件類型：</td>
											<td class="input" style="width: 18%">
										...
										<tr>
											<td class="title" style="width: 15%">
												<span name="InfoUniformNo1" <c:if test="${prpLpayObject.ownerShip=='C' && prpLpayObject.paymentKind=='4'}">style="display:none;"</c:if>>統一編號/身份證號：</span> <span name="InfoUniformNo2" <c:if test="${!(prpLpayObject.ownerShip=='C' && prpLpayObject.paymentKind=='4')}">style="display:none;"</c:if>>個人身份證號：</span>
											</td>
											<td class="input" style="width: 18%">
											...
											<td class="title" style="width: 15%">
												<span name="BeneficiaryPhone1" <c:if test="${prpLpayObject.ownerShip=='C'}">style="display:none;"</c:if>>受款人電話：</span> <span name="BeneficiaryPhone2" <c:if test="${prpLpayObject.ownerShip!='C'}">style="display:none;"</c:if>>市內電話：</span>
											</td>
											<td class="input" style="width: 18%">
											...
											<td class="input" style="width: 18%">
												...
												<span name="spanCutBack" <c:if test="${prpLpayObject.ownerShip!='Q'}">style="display:none;"</c:if>>取消禁背：</span>
												<span name="spanCutBack" <c:if test="${prpLpayObject.ownerShip!='Q'}">style="display:none;"</c:if>> <select name="prpLpayObjectInfoCutBack" class='common' style="width: 50%">
														<option value="1" <c:if test="${prpLpayObject.cutBack=='1'}"> selected</c:if>>是</option>
														<option value="0" <c:if test="${prpLpayObject.cutBack=='0'}"> selected</c:if>>否</option>
												</select>
												</span>
											</td>
											<%--<td class="title" style="width: 15%">
										...
										<tr name="AreaInfo" <c:if test="${prpLpayObject.ownerShip=='C'}">style="display:none"</c:if>>
											<!-- mantis：CLM0145，處理人員：DP0713，需求單編號：CLM0145，.新核心-理算任務處理賠付對象郵遞區號長度檢核 -->
											<!--  \webapp\claim\pages\commonAcci\compensate\EditPrpdpaymentaccountPage.jsp 2-->
											<td class="title" style="width: 15%">郵遞區號：</td>
											<td class="input" style="width: 18%">
												<!-- mantis：CLM0145，處理人員：DP0713，需求單編號：CLM0145，.新核心-理算任務處理賠付對象郵遞區號長度檢核 -->

...
<body class=interface onload="initPage();" class="interface">
	<form name="fm" method="post" action="${ctx }/endcase/endcaseSave.do?step=step2">
		<h3>
			<s:text name="prompt.endcase.finalSuccessSaved" />
		</h3>
		<br>
		<%--结案登记保存成功！--%>
		<h3>
			<s:text name="prompt.endcase.pleaseDotContinueNext" />
		</h3>
		<%--要生成结案号,请点击下一步按钮！--%>
		<hr>
		<input type=hidden name="prpLendcaseClaimNo1" value="${claimNo }">
		<input type="button" name="buttonNext" value="<s:text name='button.continueNext.value' />" onclick="submitForm()">
		<%--继续下一步--%>

...
<script src="/claim/pages/commonCargo/check/js/CargoCheckEdit.js"></script>
</head>
...
<s:elseif test="#request.editType=='SHOW' || #request.editType=='DELETE'">
	<body onload="initPage();disabledAllButton('buttonArea');oMPC.style.visibility='visible';" >
</s:elseif>
<s:else>
	...
	<form name=fm action="${ctx}/check/checkEditPost.do" method="post" onsubmit="return validateForm(this);">
			...
			<mpc:page ID="tabMain" TABTITLE="基本信息" TABTEXT="基本信息">
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%-- 1.查勘/代查勘头信息 --%>
						<%@include file="/pages/commonCargo/check/CargoCheckMainHeadEdit.jsp" %>
						<%-- 3.查勘尾信息 --%>
						<%@include file="/pages/commonCargo/check/CargoCheckMainTailEdit.jsp" %>
						<%-- 4.查勘详细信息 --%>
						<%@include file="/pages/commonCargo/check/CargoCheckOtherInfor.jsp"%>
						<%-- 5.查勘备注信息 --%>
						<%@include file="/pages/commonCargo/check/CargoCheckTextEdit.jsp"%>
						<%-- 6.巨灾代码--%>
		...
		<TABLE id="btnCommon" class="common">
			<TR>
				<TD align="center">
					<%--7.保存通用按钮 --%>
					<%@include file="/pages/commonCargo/check/CargoCheckSave.jsp" %>
				</TD>
			</TR>
		</TABLE>
	</form>

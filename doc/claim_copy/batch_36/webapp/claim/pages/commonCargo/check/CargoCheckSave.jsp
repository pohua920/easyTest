...
<%-- 保存通用按钮 --%>
<table id="buttonArea" cellpadding="0" cellspacing="0" width="80%"
	style="display:">
	...
	<tr>
		<td>
			<input type="button" name="print" class='bigbutton'
				value="<s:text name='button.printInsClaimInfo.value'/>"
				onclick="printWindow('${prpLcheck.id.registNo}','列印1');">
			&nbsp;&nbsp;
			<%--- 列印承保理赔信息 --%>
		</td>
		<c:if test="${prpLcheck.status != 4}">
			<td class=button style="width: 20%" align="center">
				<!--暂存按钮-->
				<input type="button" name=buttonSave class='button'
					value="<s:text name='button.save.value'/>"
					onclick="saveForm(this,'2');">
			...
			<c:if test="${param.status == 0}">
				<td class=button align="center">
					<input type=button name=buttonGiveup class='button'
						value="<s:text name='button.giveUpTask.value'/>"
						onclick="taskCheckGiveup();">
				</td>
			</c:if>
			<!--放弃任务-->
		...
		<c:if test="${prpLcheck.status == 4}">
			<td class=button style="width: 33%" align="center">
				<!--返回按钮-->
				<input type=button name=buttonBack class='button'
					value="<s:text name='button.return.value'/>"

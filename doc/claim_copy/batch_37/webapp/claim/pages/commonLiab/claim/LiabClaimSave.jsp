...
<table  id="buttonArea" cellpadding="0" cellspacing="0" width="80%" style="display:">
	...
	<tr>
		<td>
			<input type="button" name="print" class='bigbutton' value="<s:text name='button.printInsClaimInfo.value' />" onclick="printWindow('${prpLclaim.registNo}', '列印1');">&nbsp;&nbsp;<%--列印承保理赔信息--%>
		</td>
			<!--放弃任务(只对待处理状态)-->
		<c:if test="${param.statusgiv == '0'}">
		<td class=button  align="center">
			<!--放弃按钮style="width:33%"-->
			<input type=button name=buttonGiveup class='button' value="<s:text name='button.giveUpTask.value' />" onclick="taskClaimGiveup();" ><%--放弃任务--%>
		</td>
		</c:if>
			<!--放弃任务-->
		<c:if test="${prpLclaim.status =='1'}">

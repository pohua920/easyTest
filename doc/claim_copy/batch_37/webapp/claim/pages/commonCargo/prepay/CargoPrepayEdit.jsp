...
<head>
  ...
  <script src="${ctx}/pages/commonCargo/prepay/js/CargoPrepayEdit.js"></script>
</head>
...
<body class=interface onload="initPage();InitDocument();initSet();" >
  ...
  <form name=fm action="${ctx}/prepaySave.do" method="post" onsubmit="return validateForm(this);">
    ...
    <!-- 1.预赔主信息 -->
    <%@include file="/pages/commonCargo/prepay/CargoPrepayMainEdit.jsp" %>
    <!-- 赔付支付信息   -->
    <%@include file="/pages/common/prepay/EditPrpdpaymentaccountPrePage.jsp"%>
    <!-- 4.预赔备注信息 -->
    <%@include file="/pages/commonCargo/prepay/CargoPrepayTextEdit.jsp"%>
    <!-- 5.核赔意见 -->
    <%@include file="/pages/common/pub/UndwrtTextEdit.jsp" %>
   <c:if test="${param.editTypeOther!='SHOWTASK'}">
    <!-- 保存通用按钮 -->
    <%@include file="/pages/commonCargo/prepay/CargoPrepaySave2.jsp"%>
   </c:if>
  </form>
</body>

    ...
    <tr class=mline>
      <td class="subformtitle" style="text-align:left;">
        <img style="cursor:hand;" src="/claim/images/butCollapseBlue.gif"
             ...
             <c:if test="${prpLregistText.textType=='1'}">
             	<s:text name="db.regist.registText.textType1" />
             </c:if>
             <c:if test="${prpLregistText.textType=='2'}">
             	<s:text name="db.regist.registText.textType2" />
             </c:if>
             <c:if test="${prpLregistText.textType=='3'}">
             	<s:text name="db.regist.registText.textType3" />
             </c:if>
        <br>
      <table class="common" align="center" id="RegistText" style="display:none" cellpadding="5" cellspacing="1">
          <tbody>
            <tr>
              ...
              <textarea style="wrap:hard" rows="15" cols="80" name="prpLregistTextContextInnerHTML">${ prpLregistText.context}</textarea>
                ...
                <c:if test="${prpLregistText.textType=='1'}">
                	<input type="button" name="btnRegistText"  class=bigbutton value="<s:text name='button.buildAccident.value'/>" class="bigbutton" onclick="return generateRegistText();"><%-- 生成出险摘要 --%>
                </c:if>
              </td>
            </tr>
          </tbody>

...
function printWindow(registNo,strWindowName)
{
	...
	--%>
		...
		<tr>
			<td>
				<input type="hidden" name="print" class='bigbutton' value="<s:text name='button.printInsClaimInfo.value' />" onclick="printWindow('${prpLclaim.registNo}', '列印1');">&nbsp;&nbsp;<%--列印承保理赔信息--%>
			</td>
			<s:if test="#request.prpLclaim.status == 1">
			...
			<s:if test="#request.prpLclaim.status == 2">
			...
			<s:if test="#request.prpLclaim.status == 3">
				<td class=button style="width:33%" align="center">
				<!--保存按钮-->
			...
			<!--放弃任务-->
			<s:if test="#request.prpLclaim.status == 4">
				<td class=button style="width:33%" align="center">
				<!--取消按钮-->

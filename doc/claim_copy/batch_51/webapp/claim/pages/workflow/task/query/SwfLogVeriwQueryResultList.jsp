...
* MODIFYLIST ： 人伤核损节点的工作流任务查询 (用于显示待处理、正处理、已处理、注销拒赔人伤核损任务 查询介面的查询结果)
*          ------------------------------------------------------
****************************************************************************/
--%>
...
<table class="common" cellpadding="5" cellspacing="1">
    <tr>
        <td colspan=12 class="formtitle">${pageScope.strTitle}<s:text name="prompt.personverifyLoss.message" /><%-- 人傷核損訊息 --%></td>
    </tr>
    <tr>
...
<c:forEach items="${requestScope.swfLog.swfLogList}" var="tempSwfLog" varStatus="stat">
    ...
    <c:choose>
        ...
        <c:otherwise><%-- 正处理、已处理 --%>
            <c:set var="flowStrStart" value="${ctx}/verifyLoss/verifyLossFinishQueryList.do?prpLverifyLossRegistNo=${tempSwfLog.registNo}&lossTypeFlag=${tempSwfLog.typeFlag}&insureCarFlag=${tempSwfLog.insureCarFlag}&lossItemCode=${tempSwfLog.lossItemCode}&lossItemName=${tempSwfLog.lossItemName}&flag=1" />
        </c:otherwise>
    </c:choose>
    <c:choose>

...
* MODIFYLIST ：单证节点的工作流任务查询 (用于显示待处理、正处理、已处理、注销拒赔 单证任务 查询介面的查询结果)
*          ------------------------------------------------------
****************************************************************************/
--%>
...
<table class="common" cellpadding="5" cellspacing="1">
    <tr>
        <td colspan=12 class="formtitle">${pageScope.strTitle}<s:text name="prompt.certify.message" /><%-- 單證訊息--%></td>
    </tr>
    <tr>
...
<c:forEach items="${requestScope.swfLog.swfLogList}" var="tempSwfLog" varStatus="stat">
    ...
    <c:choose>
        <c:when test="${param.status=='0' && (tempSwfLog.nodeStatus == '0' || tempSwfLog.nodeStatus == '3')}"><%-- 未处理 --%>
            <c:set var="flowStrStart" value="${ctx}/certifyBeforeEdit.do?RegistNo=${tempSwfLog.registNo}" />
        </c:when>
        ...
        <c:when test="${param.status=='-1' && param.FuncName == 'specialApply'}"><%-- 申请特殊赔案 --%>
            <c:set var="flowStrStart" value="${ctx}/specailCaseQuery.do?" />
        </c:when>
        <c:otherwise><%-- 正处理、已处理 --%>
            <c:set var="flowStrStart" value="${ctx}/certifyFinishQueryList.do?prpLcertifyCertifyNo=${tempSwfLog.registNo}" />
        </c:otherwise>
    </c:choose>
    <c:choose>
    ...
    <c:if test="${param.status=='0' || param.status=='-1'}">
        ...
        <td align="center">
            <c:choose>
                ...
                <c:when test="${param.status == '-1' && param.FuncName == 'specialApply'}"><%-- 特殊赔案申请--%>
                    <a style="display: none" href="${flowStrStart}${flowStr}" border="0" hspace="5" alt="<s:text name="specialCase.specialCas.applye"/>"><s:text name="workflow.status.applye" /></a>
                </c:when>
                <c:when test="${tempSwfLog.nodeStatus == '4'}">

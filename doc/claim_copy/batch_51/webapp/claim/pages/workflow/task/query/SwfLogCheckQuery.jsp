...
* MODIFYLIST ： 查勘节点的工作流任务查询 (用于组织待处理、正处理、已处理查勘任务、申请注销/拒赔 查询介面的查询条件)
*              调查节点的工作流任务查询 (用于组织待处理、正处理、已处理调查任务 查询介面的查询条件)
*          ------------------------------------------------------
****************************************************************************/
--%>
...
<table border="0" align="center" cellpadding="5" cellspacing="1" class="common">
	...
	<tr>
		<c:choose>
			<c:when test="${param.type == 'acci'}">
				<%-- 意健险调查 --%>
				<td class='title'>調查號碼：</td>
				<td class='input'>
	...
	<tr>
		...
		<td class='input' colspan="3" align="left">
			...
			<s:text name="prompt.to" />
			&nbsp;
			<rc:rcDate name="damageEndDate" style="width:40.5%" value="${pageScope.damageEndDate}" />
		</td>
	</tr>
	<tr>
			<%-- 工程险查勘 --%>
			<td class='title'>
				<s:text name="db.prplcheck.identifyNumber" />
				<%-- 身份證字號/統一編號 --%>
				：
			</td>
			<td class='input'>
				...
				<input type=text name="InsuredIdentifyNumber" class="query" style="width: 70%" value="${param.InsuredIdentifyNumber}">
			</td>
		<c:choose>
...
<c:if test="${not empty param.BusinessNoSign}">
	<script type="text/javascript">
		$(":input[name='BusinessNoSign']").val("${param.BusinessNoSign}");
	</script>
</c:if>

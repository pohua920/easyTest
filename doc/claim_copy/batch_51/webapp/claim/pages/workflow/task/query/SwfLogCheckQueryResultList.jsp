...
* MODIFYLIST ：查勘节点的工作流任务查询 (用于显示待处理、正处理、已处理、注销拒赔 查勘任务 查询介面的查询结果)
*          ------------------------------------------------------
****************************************************************************/
--%>
...
<table class="common" cellpadding="5" cellspacing="1">
    <tr>
        <td colspan=12 class="formtitle">${pageScope.strTitle}<s:text name="prompt.accicheck.message" /><%-- 查勘訊息--%></td>
    </tr>
    <tr>
    ...
    <c:choose>
       <c:when test="${param.type == 'acci'}"><s:text name="db.prpLacciCheck.checkNo" /><%-- 調查號 --%></c:when>
       <c:otherwise><s:text name="prpLregist.registNo" /><%-- 備案號碼 --%></c:otherwise>
    </c:choose>
        </td>
...
<c:forEach items="${requestScope.swfLog.swfLogList}" var="tempSwfLog" varStatus="stat">
    ...
    <c:choose>
        <c:when test="${param.status=='0' && tempSwfLog.nodeStatus == '0'}"><%-- 未处理 --%>
            <c:set var="flowStrStart" value="${ctx}/check/checkBeforeEdit.do?RegistNo=${tempSwfLog.registNo}&insureCarFlag=${tempSwfLog.insureCarFlag}&lossItemCode=${tempSwfLog.lossItemCode}&lossItemName=${tempSwfLog.lossItemName}" />
        </c:when>
        ...
        <c:otherwise><%-- 正处理、已处理 --%>
            <c:set var="flowStrStart" value="${ctx}/check/checkFinishQueryList.do?prpLcheckCheckNo=${tempSwfLog.registNo}&insureCarFlag=${tempSwfLog.insureCarFlag}&lossItemCode=${tempSwfLog.lossItemCode}&lossItemName=${tempSwfLog.lossItemName}" />
        </c:otherwise>
    </c:choose>
    <c:choose>
    ...
    <c:if test="${param.status=='0' || param.status=='-1'}">
        ...
        <td align="center">
            <a href="${flowStrStart}${flowStr}" title="${tempSwfLog.titleStr}" ><c:out value="${tempSwfLog.keyIn}" /></a>
        </td>
        <td align="center">

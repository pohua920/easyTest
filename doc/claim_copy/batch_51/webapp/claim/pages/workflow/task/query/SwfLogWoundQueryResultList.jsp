...
* MODIFYLIST ： 人伤定损节点的工作流任务查询 (用于显示待处理、正处理、已处理、退回人伤、注销拒赔任务 查询介面的查询结果)
*          ------------------------------------------------------
****************************************************************************/
--%>
...
<table class="common" cellpadding="5" cellspacing="1">
    <tr>
        <td colspan=12 class="formtitle">${pageScope.strTitle}<s:text name="info.personTrack" /><%-- 人傷跟蹤訊息 --%></td>
    </tr>
    <tr>
...
<c:forEach items="${requestScope.swfLog.swfLogList}" var="tempSwfLog" varStatus="stat">
    ...
    <c:choose>
        <c:when test="${param.status=='0' && tempSwfLog.nodeStatus == '0'}"><%-- 未处理 --%>
            <c:set var="flowStrStart" value="${ctx}/certainLoss/certainLossBeforeEdit.do?RegistNo=${tempSwfLog.registNo}&&lossTypeFlag=0&insureCarFlag=${tempSwfLog.insureCarFlag}&lossItemCode=${tempSwfLog.lossItemCode}&lossItemName=${tempSwfLog.lossItemName}" />
        </c:when>
        ...
        <c:when test="${tempSwfLog.nodeStatus=='3'}"><%-- 回退--%>
            <c:set var="flowStrStart" value="${ctx}/certainLoss/certainLossFinishQueryList.do?RegistNo=${tempSwfLog.registNo}&lossTypeFlag=0&insureCarFlag=${tempSwfLog.insureCarFlag}&lossItemCode=${tempSwfLog.lossItemCode}&lossItemName=${tempSwfLog.lossItemName}&flag=1" />
        </c:when>
        <c:otherwise><%-- 正处理、已处理 --%>
            <c:set var="flowStrStart" value="${ctx}/certainLoss/certainLossFinishQueryList.do?prpLverifyLossRegistNo=${tempSwfLog.registNo}&lossTypeFlag=0&insureCarFlag=${tempSwfLog.insureCarFlag}&lossItemCode=${tempSwfLog.lossItemCode}&lossItemName=${tempSwfLog.lossItemName}" />
        </c:otherwise>
    </c:choose>
    <c:choose>
        ...
        <td align="center">
            <c:choose>
                ...
                <c:when test="${param.status == '-1' && param.FuncName == 'cancelApply'}"><%-- 申请注销拒赔 --%>
                    <a href="${flowStrSt}${flowStr}"><img name=buttonDistribute src="${ctx}/images/butCancel.gif" border="0" hspace="5" alt="<s:text name='claim.cancelReject'/>"></a>
                </c:when>
                <c:when test="${tempSwfLog.nodeStatus == '4'}">

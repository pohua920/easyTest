...
* MODIFYLIST ： 备案节点的工作流任务查询 (用于组织正处理、已处理 备案任务查询介面的查询条件)
*          ------------------------------------------------------
****************************************************************************/
--%>
    ...
    <tr>
        ...
        <td class='title'>
            <c:choose>
               ...
               <c:otherwise><s:text name="guarantee.dealIime" /><%-- 處理時間 --%>：</c:otherwise>
            </c:choose>
        </td>
        <td class='input'>
            <rc:rcDate name="statStartDate" style="width:42%" value="${param.statStartDate}" />
...
<c:choose>
    <c:when test="${param.status == '2'}">
        ...
        <td class='input' colspan="3" align="left">
            <rc:rcDate name="damageStartDate" style="width:42%" value="${pageScope.damageStartDate}"/>
            &nbsp;<s:text name="prompt.to" />&nbsp;
            <rc:rcDate name="damageEndDate" style="width:41.5%" value="${pageScope.damageEndDate}"/>
        </td>
    </c:when>
    <c:otherwise>
        <td class='title'></td>

...
function checkPrpLloss() {
    ...
    $("#lLoss").find("tr[name='prpLlossObject']").each(function (i) {
        ...
        var currency = $(this).find(":input[name='prpLlossDtoCurrency']").val();
        var payObjectSerialNo = $payObjectSerialNo.val();
        if ($.trim($kindCode.val()) == '') {
            checkFlag = alertMessage($kindCode[0], "第 " + (i + 1) + " 筆賠付標的訊息‘險別’必須錄入!");
            return false; //跳出循环
        }else{
        	var sumRealPayNTD = 0;
        	if(payObjectSerialNo!=""){
	    		...
	    		for (var j = 0; j < payObjectValue.length; j++) {
	    			...
	    			if(currency!= $prpLpayObjectInfoCurrency.get(parseInt(payObjectValueTemp[0])-1).value){
 ...
 */
 function checkPrpLpersonLoss() {
    ...
    $("#Person").find("tr[name='personObject']").each(function (i, personObject) {
        ...
        if ($.trim($personName.val()) == '') {
            checkFlag = alertMessage($personName[0], "第 " + (i + 1) + " 筆人員傷亡賠付訊息‘人員姓名’必須錄入!");
            return false; //跳出each
        } else if ($.trim($birthday.val()) == '') {
            checkFlag = alertMessage($birthday[0], "第 " + (i + 1) + " 筆人員傷亡賠付訊息‘出生年份’必須錄入!");
            return false; //跳出each
        } else {
            if(certificateCode == '01' || certificateCode =='02'){//證件類型為 身份證號，統一編號時需要校驗
                if($.trim(identifyNumber).length == 0){
                    checkFlag = alertMessage($identifyNumber[0], "第 " + (i + 1) + " 筆人員傷亡賠付訊息，證件類型為‘"+$certificateCode.find("option:selected").text()+"’,‘證件號碼’必須錄入!");
                    return false; //跳出each
            ...
            if($ll.length == 0){
                ...
                $ll.each(function (j, prpLpersonLossObject) {
                    ...
                    var currency = $(this).find(":input[name='prpLpersonLossCurrency']").val();
                    var $payObjectSerialNo = $(this).find(":input[name='prpLpersonLossPayObjectSerialNo']");
                    var payObjectSerialNo = $payObjectSerialNo.val();
                    if ($.trim($prpLpersonLossKindCode.val()) == '' ) {
                        checkFlag = alertMessage($prpLpersonLossKindCode[0], "第 " + (i + 1) + " 筆人員傷亡賠付訊息,第 " + (j + 1) + " 筆費用資訊 ‘險別’必須錄入!");
                        return false; //跳出each
                    } else if ($prpLpersonLossLiabDetailCode.val() == '' || $prpLpersonLossLiabDetailName.val() == '') {
                        checkFlag = alertMessage($prpLpersonLossLiabDetailCode[0], "第 " + (i + 1) + " 筆人員傷亡賠付訊息,第 " + (j + 1) + " 筆費用資訊 ‘費用類別’必須錄入!");
 ...
 function checkPrpLcharge() {
    ...
    for(var i = 0;i < (alltr_length/clength);i++){
        ...
        var $feeSerialNo = $chargeObject.find(":input[name='prpLchargeFeeSerialNo']");
        ...
        var $paymentKind = $chargeObject.find(":input[name='prpLchargePaymentKind']"); //賠付類型
        ...
        if ($.trim($kindCode.val()) == '') {
            checkFlag = alertMessage($kindCode[0], "第 " + (i + 1) + " 筆費用資訊‘險別代碼’必須錄入!");
            break;
        } else if ($.trim($chargeCode.val()) == '' || $.trim($chargeName.val()) == '') {
            checkFlag = alertMessage($chargeCode[0], "第 " + (i + 1) + " 筆費用資訊‘費用代碼’、‘費用名稱’必須錄入!");
            break;
        } else if ($chargeCode.val() == 'B' && $.trim($feeSerialNo.val()) == '') {
            checkFlag = alertMessage($feeSerialNo[0], "第 " + (i + 1) + " 筆費用資訊‘B-代扣稅’必須錄入‘代扣費用序號’!");
            break;
        } else if ($.trim($ownerName.val()) == '') {
            checkFlag = alertMessage($ownerName[0], "第 " + (i + 1) + " 筆費用資訊‘賠付對象’必須錄入!");
            break;
            ...
            alert("第 " + (i + 1) + " 筆費用支付方式爲匯款，必須錄入費用支付帳戶資訊!");
            checkFlag = false;
            break;
        } else if (((ownerShip == 'B' && customBankCode !="9999") || ownerShip=='Q') && ($.trim(areaCode).length == 0 || $.trim(courierAddress).length == 0)) {
            alert("第 " + (i + 1) + " 筆費用資訊‘郵遞區號’、‘郵遞地址’必須錄入!");
            checkFlag = false;
            break;
        } else if ($paymentKind.val().length == 0) {
            checkFlag = alertMessage($paymentKind[0], "第 " + (i + 1) + " 筆費用資訊‘賠付類型’必須錄入!");
            checkFlag = false;
            break;
        } else {
            if(certificateCode == '01' || certificateCode =='02'){//證件類型為 身份證號，統一編號時需要校驗
                if($.trim(uniformNo).length == 0){
                    checkFlag = alertMessage($uniformNo[0], "第 " + (i + 1) + " 筆費用資訊，證件類型為‘"+$certificateCode.find("option:selected").text()+"’,‘證件號碼’必須錄入!");
                    break;
 ...
 function checkPrpLpayObjectInfo() {
     ...
     $("#PrpLpayObject").find("tr[name='PrpLpayObjectInfo']").each(function (i) {
         ...
         if ($.trim($ownerName.val()) == '') {
             checkFlag = alertMessage($ownerName[0], "賠付對象 " + (i + 1) + " ‘賠付對象’必須錄入!");
             return false; //跳出each
         } else if (paymentKind.length == 0) {
             alert("賠付對象 " + (i + 1) + " ‘賠付類型’必須錄入!");
             checkFlag = false;
             return false; //跳出each
         } else if ($.trim(beneficiaryPhone).length == 0) {
             alert("賠付對象 " + (i + 1) + " ‘受款人電話’必須錄入!");
             checkFlag = false;
             return false; //跳出each
             ...
             alert("賠付對象 " + (i + 1) + " 費用支付方式爲匯款，必須錄入支付帳戶資訊!");
             checkFlag = false;
             return false; //跳出each
         } else if (((ownerShip == 'B' && customBankCode !="9999") || ownerShip=='Q') && ($.trim(areaCode).length == 0|| $.trim(courierAddress).length == 0)) {
             alert("賠付對象 " + (i + 1) + " ‘郵遞區號’、‘郵遞地址’必須錄入!");
             checkFlag = false;
             return false; //跳出each
         } else {
             if(certificateCode == '01' || certificateCode =='02'){//證件類型為 身份證號，統一編號時需要校驗
	...
	if (count > 1) {
		for (var i = 1; i < count; i++) {
			if (fm.prpLchargeChargeCode[count].value == fm.prpLchargeChargeCode[i].value) {
				alert(i18n.compensate.sameCostNotEntry); //同种费用不能重复錄入!
				return false;
			}
		}
	}
...
//当手工錄入支付对象後，检查该对象是否存在银行帳号信息 begin
...
function setChargeInput(field){
    ...
    $("#Charge").find(":input[name='prpLchargeFeeSerialNo'][value='"+currSerialNo+"']").val("");
    calFund();
}
...
function calFund(){
    ...
    $("#lLoss").find("input[name='prpLlossDtoSumRealPayNTD']").each(function(){
        prpLlossRealPay += (isNaN($(this).val()) ? 0 : parseFloat($(this).val(), 10));
    });
    // 人伤赔款信息
    $("#Person").find("input[name='prpLpersonLossSumRealPayNTD']").each(function(){
        personLossRealPay += (isNaN($(this).val()) ? 0 : parseFloat($(this).val(), 10));
    });
    // 费用资讯信息
    $("#Charge").find("input[name='prpLchargeChargeAmountNTD']").each(function(){
        chargeAmount += (isNaN($(this).val()) ? 0 : parseFloat($(this).val(), 10));
    });
    // 费用资讯信息
    $("#Charge").find("input[name='prpLchargeSumRealPay']").each(function(){
        chargeRealPay += (isNaN($(this).val()) ? 0 : parseFloat($(this).val(), 10));
    });
 ...
 * 选择残废项目时，要清空残废程度
 */
function clearInjuryItemCode(field){
    var $prpLpersonLossObject = $(field).parents("tr[name='prpLpersonLossObject']");
    $prpLpersonLossObject.find(":input[name='prpLpersonLossInjuryItemCode']").val("");
    $prpLpersonLossObject.find(":input[name='prpLpersonLossInjuryItemName']").val("");
}
...
function setPayObjectSerialNo(field){
    ...
    if($serial.length > 0){//存在收取對象
        ...
        $serial.each(function(){
            ...
            strLi += "理赔金額:<input type=\"text\" onblur=\"setPayObjectPayAmount(this);\" name=\"payObjectPayAmount\"value=\""+payAmount+"\" class=\"common\" style=\"width:100px\"/>";
            strLi += "</li>";
        });
 ...
 * 费用讯息获取当前赔付币别对本位币NTD的汇率
 */
function getPrpLprpLpayObjectExchRate(field){

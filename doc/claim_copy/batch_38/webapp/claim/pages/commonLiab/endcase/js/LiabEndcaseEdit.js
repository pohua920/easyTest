...
function saveForm(field, saveType) {
	if (!confirm(i18n.commonAcci.endcase.notModifySureClos))
		return false;
	...
	//判断保费是否交付，否则不允许提交
	var premiumFee = fm.premiumFee.value;
	if (premiumFee == 0) {
		errorMessage = errorMessage + i18n.commonLiab.endcase.premiumActuallyNotAllowSubmit; //保费未实付，不允许提交/暂存！\n
		return false;
	}
...
function openWinSave1() {
	...
	msg = window.open("/claim/messageQueryInfo.do?businessNo=" + businessNo + "&nodeType=endca&policyNo=" + policyNo + "&claimNo=" + claimNo + "&registNo=" + registNo, "NewWindow", "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,width=500,Height=300");
}

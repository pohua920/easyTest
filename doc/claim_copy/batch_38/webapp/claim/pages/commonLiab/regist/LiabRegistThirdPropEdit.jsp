...
<table class="common" cellspacing="1" cellpadding="5">
	...
	<tr>
		<td class="common" colspan="4" style="text-align: left;">
			...
			<table class="common" style="display: none" id="PROPERTYLOSS_Data" cellspacing="1" cellpadding="5">
				<tbody>
					<tr>
						...
						<td class="input" style="width: 10%">
							<%-- 财车车牌号码 --%>
							<input name="prpLthirdPropGoodsCarLicenseNo" class="input" style="width: 90%" >
						</td>
			...
			<%-- 多行输入展现域 --%>
			<table class="common" id="PROPERTYLOSS" cellspacing="1" cellpadding="5">
				<thead>
					<tr>
						...
						<td class="centertitle" style="width: 22%">
							<s:text name="certainLoss.thirdCarLoss.goodsCarLicenseNo" />
						</td>
						<%-- 財車車牌號碼 --%>
						<td class="centertitle" style="width: 22%">
						...
						<%-- 损失程度描述 --%>
						<!--Reason:赔款费用模块合到财产损失信息中-->
						<c:set var="strThirdPropRows" value="5" />
						<c:if test="${prpLnodeType=='check'}">
							<td class="centertitle" style="width: 10%;display: none">
								<s:text name="db.prpDkind.kindCode" />
							</td>
							<%-- 险别代码 --%>
							<td class="centertitle" style="width: 20%;display: none">
					...
					<c:if test="${not empty requestScope.prpLthirdProp.thirdPropList}">
						<c:forEach var="ThirdPropdtox" items="${requestScope.prpLthirdProp.thirdPropList}" varStatus="status">
							<tr>
								...
								<td class="input" style="width: 10%">
									<%-- 财车车牌号码 --%>
									<input name="prpLthirdPropGoodsCarLicenseNo" class="input" style="width: 90%" value="<c:out value='${ThirdPropdtox.goodsCarLicenseNo}'/>">
								</td>
								<td class="input" style="width: 10%">

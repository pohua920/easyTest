...
<script src="/claim/pages/commonProp/check/js/PropCheckEdit.js"></script>
		...
		<mpc:container ID="oMPC">
			<mpc:page ID="tabMain" TABTITLE="<s:text name='title.checkBeforeEdit.editCheck'/>" TABTEXT="<s:text name='title.checkBeforeEdit.editCheck'/>">
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%-- 1.查勘/代查勘头信息 --%>
						<%@include file="/pages/commonProp/check/PropCheckMainHeadEdit.jsp"%>
						<%-- 2.查勘中信息 --%>
						<%--@include file="/pages/commonProp/check/PropCheckMainMiddleEdit.jsp" --%>
						<%-- 3.查勘尾信息 --%>
						<%@include file="/pages/commonProp/check/PropCheckMainTailEdit.jsp"%>
						<%-- 4.查勘备注信息 --%>
						<%@include file="/pages/commonProp/regist/PropRegistTextEdit.jsp"%>
						<%--5.估损金额信息 --%>
						<%--@include file="/pages/commonProp/claim/PropClaimFeeEdit.jsp"--%>
						<%-- 6.巨灾代码--%>
			...
			<mpc:page ID="tabMain" TABTITLE="<s:text name='claim.infoAmountLoss'/>" TABTEXT="<s:text name='claim.infoAmountLoss'/>"><%-- 估损金额讯息 --%>
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%--5.估损金额信息 --%>
						<%@include file="/pages/commonProp/check/PropCheckLossEdit.jsp"%>
					</DIV>
				</CENTER>
			</mpc:page>
			<%-- 1.2.报案受损信息页面 --%>
			<mpc:page ID="tabMain" TABTITLE="<s:text name="check.damagePropertyInfo" />" TABTEXT="<s:text name="check.damagePropertyInfo" />">
				<DIV name="tabMain" class="tabMain">
					<%-- 财产损失部位信息 --%>
					<%@include file="/pages/commonProp/regist/PropRegistThirdPropEdit.jsp"%>
					<%@include file="/pages/commonProp/regist/PropRegistPersonTraceEdit.jsp"%>
				</DIV>
			</mpc:page>
		</mpc:container>
		<TABLE id="btnCommon" class="common">
			<TR>
				<TD align="center">
					<c:if test="${param.editTypeOther!='SHOWTASK'}">
						<%-- 保存通用按钮 --%>
						<%@include file="/pages/commonProp/check/PropCheckSave.jsp"%>
					</c:if>
				</TD>
			</TR>
		</TABLE>

		...
		function init() {
			YAHOO.widget.DataTable.formatLink = function(elCell, oRecord,
				...
				if (oColumn.key == "serialNo") {
					...
					elCell.innerHTML = "<a href='${ctx}/remnantBeforeEdit.do?editType=show&claimNo="
					...
					elCell.innerHTML = "<a href='${ctx}/remnantBeforeEdit.do?editType=show&claimNo="
							...
							+ oRecord.getData("comName")
							+ "&compensateNo="
							+ oRecord.getData("compensateNo") + "'>" + "查看</a>"
				} else if (oColumn.key == "handleStatus") {
					elCell.innerHTML = "<s:text name='common.status.treated'/>";
				} else {
	...
	<%-- 
		...
		function executeQuery(pageNo, pageSize,field) {
			...
			var myDataSource = new YAHOO.util.DataSource(
					"${ctx}/remnantResultQuery.do?editType=showquery");
			myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
			myDataSource.connMethodPost = true;
			myDataSource.responseSchema = {
				resultsList : "data",
				fields : [ "claimNo", "comCode", "riskCodeName", "comName",
						"policyNo", "compensateNo", "remnantDate" ],
				metaFields : {

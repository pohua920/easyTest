...
function checkBlur(option, condition) {
	...
	if (eval("fm." + option).value == ":" && value != "")
		if (index < 0) {
			...
			errorMessage("请确认查询输入条件中包含:");
		} else if (index == 0 ||
		index == value.length - 1) {
		eval("fm." + condition).focus();
		errorMessage("请确认查询输入条件格式");
	}
...
function showPage(input, spanID) {
	...
	if (spanID.style.display == "") {
		...
		input.src = "/claim/images/butCollapseBlue.gif";
	} else {
		//打开
		spanID.style.display = "";
		input.src = "/claim/images/butExpandBlue.gif";
	}
}
...
function setRecordState(PageCode) {
	attributes = pagesAttributes[PageCode];
	eval("span$" + PageCode + "$State").innerHTML = "第" + (attributes.currentIndex + 1) + "条/共" + attributes.length + "条";
}
...
function locateRecord(PageCode, index) {
	...
	if (index > onePageData.count) {
		errorMessage("不存在记录号为" + index + "的记录!");
	} else if (index < 1) {
		errorMessage("记录号不能小於1!");
	} else //否则，读出下一行内容
	{
...
function checkDatetime(field, from, to) {
	...
	if (from == "year" && to == "month") {
		...
		if (regExpTest(strValue, /[\d]{4}[-][\d]{1,2}[-][\d]{1,2} [\d]{1,2}:[\d]{1,2}:[\d]{1,2}/) == false) {
			errorMessage("请输入合法的" + desc + "\n类型为日期时间，格式为YYYY-MM-DD hh:mm:ss");
			field.focus();
			field.select();
			return false;
		}
 ...
 *@param       registNo  赔案号
 *@param       registNo  节点代码
 * modify by liping 20080430
 */
function openCertify(actionName, paraName, registNo, nodeType, riskCode) {
	var win;
...
function showWorkFlowerByClaimNo(strClaimNo) {
	var vURL = '/claim/swfFlowBeforeQuery.do?claimNo=' + strClaimNo + '&ifclose=true';

...
SINOSOFT.util.navigation = function(oRequest,oParsedResponse,oCallback,oCaller){ 
	...
	//alert(oRequest.response.meta.totalRecords);
	//var totalRecords = contentDataTable.dataSource.totalRecords;
	var totalRecords =oRequest.response.meta.totalRecords;
...
function checkLength(field)
{
  ...
  if(count>length)
  {
    alert(i18n.common.contentLength +  i18n.common.maxLengths + length + i18n.common.englishCharacters);  //输入的内容超长！\n    最大长度为     个英文字符！\n请重新输入！
    field.focus();
    field.select();
    return false;
  }
...
/** 此项配置要放在dwr的引用之后 否则会提示DWREngine未定义*/

...
for (var b = 0; b < bk.length; b++) {
	...
	if ((bk[b].indexOf('.gif') != -1) || (bk[b].indexOf('.jpg') != -1)) {
		...
		if (bk[b] == "") {
			bks[b] = 'background-color:transparent\;';
		} else {
			bks[b] = 'background-color:' + bk[b] + '\;';
		}
	}
}
if (ui[2] && !ui[0]) {
	bks[0] += 'background-image:url(' + bnk + ')\;';
}
if (ui[0] && !ui[2]) {
	bks[2] += 'background-image:url(' + bnk + ')\;';
}
if (ui[5] && !ui[3]) {
	bks[3] += 'background-image:url(' + bnk + ')\;';
}
if (ui[3] && !ui[5]) {
	bks[5] += 'background-image:url(' + bnk + ')\;';
}
...
function changeImg(img) {
	img.src = "/reins/images/bgClaimArrowHead1.gif";
}

function changeBackImg(img) {
	img.src = "/reins/images/bgClaimArrowHead2.gif";
}
//hide grid
...
function clearSubMenus() {
	...
	if (currentID[0] > -1) {
		...
		if (typeof ncObj != "undefined") {
			if (ui[2]) {
				if (ui[0]) {
					ncObj.style.backgroundImage = 'url(' + bk[0].src + ')';
				} else {
				...
				if (ui[0]) {
					...
					ncObj.style.backgroundColor = bk[0];
				}
			}
...
function partialMainCell(mNum) {
	...
	if (sI[mNum].length > 0) {
		if (previousID[1] > -1) {
			clearCell(document.getElementById("subcell-" + mNum + "-" + previousID[1]));
		}
	}
...
function openSubMenu(mid) {
	...
	for (i = 0; i < sI[mid].length; i++) {
		clcu = '';
		if (cellCLICK && sI[mid][i][0] != "" && sI[mid][i][0] != "#" && sI[mid][i][0] != "~") {
			clcu = ' cursor:' + aCURSOR + '\;';
		}
		cllu = ' cursor:default\;';
		if (sI[mid][i][0] != "" && sI[mid][i][0] != "#" && sI[mid][i][0] != "~") {
			cllu = ' cursor:' + aCURSOR + '\;';
		}
		...
		if (Trows > 1) {
			shAry[i] = (subNavH * Trows) - (svPADDING * (Trows - 1));
		}

		titleText = "";
		if (altDISPLAY == "title") {
			titleText = sI[mid][i][3];
		}
	...
	if (sP[mid][1] == "left") {
		wAdj = false;
		if ((parseInt(ncObj.style.top) + thisNavH[mid]) > sTop && (sLeft < (hOFFSET + sP[mid][4] + mI[mid][2]))) {
			wAdj = true;
			sLeft += mI[mid][2];
		}
		if (hOFFSET > 0) {
		...
		if (wAdj) {
			if (slMargin > mI[mid][2]) {
				slMargin -= mI[mid][2];
			} else {
				sLeft -= (mI[mid][2] - slMargin);
				slMargin = 0;
			}
		}
	}
	...
	for (i = 0; i < sI[mid].length; i++) {
		...
		if (sP[mid][2] == "left") {
			stObj.style.left = stINDENT;
		}
		if (sP[mid][2] == "right") {
			stObj.style.left = 0 - stINDENT;
		}
	}
	...
	if (typeof snObj.filters != "unknown" && typeof snObj.filters[0] != "undefined") {
		snObj.filters[0].Apply();
	}
...
function clearSubRollover(rmNum, rsNum) {
	...
	if (previousID[1] > -1) {
		...
		if (ui[5]) {
			if (ui[3]) {
				pObj.style.backgroundImage = 'url(' + bk[3].src + ')';
			} else {
				pObj.style.backgroundImage = 'url(' + bnk + ')';
			}
		} else {
			if (ui[3]) {
				pObj.style.backgroundImage = 'url(' + bk[3].src + ')';
			} else {
				pObj.style.backgroundColor = bk[3];
			}
		}
...
function partialSubCell(mNum, sNum) {
	...
	if (cI[mNum][sNum].length > 0) {
		if (previousID[2] > -1) {
			clearCell(document.getElementById("childcell-" + mNum + "-" + sNum + "-" + previousID[2]));
		}
	}
...
function openChildMenu(mid, cid) {
	...
	for (i = 0; i < cI[mid][cid].length; i++) {
		cn += '<tr>';
		clcu = '';
		if (cellCLICK && cI[mid][cid][i][0] != "" && cI[mid][cid][i][0] != "#" && cI[mid][cid][i][0] != "~") {
			clcu = ' cursor:' + aCURSOR + '\;';
		}
		cllu = ' cursor:default\;';
		if (cI[mid][cid][i][0] != "" && cI[mid][cid][i][0] != "#" && cI[mid][cid][i][0] != "~") {
			cllu = ' cursor:' + aCURSOR + '\;';
		}
		...
		var Trows = splitT.length;
		if (Trows > 1) {
			chAry[mid][i] = (subNavH * Trows) - (svPADDING * (Trows - 1));
		}

		titleText = "";
		if (altDISPLAY == "title") {
			titleText = cI[mid][cid][i][3];
		}
	...
	for (i = 0; i < cI[mid][cid].length; i++) {
		...
		if (cP[mid][cid][2] == "left") {
			ctObj.style.left = stINDENT;
		}
		if (cP[mid][cid][2] == "right") {
			ctObj.style.left = 0 - stINDENT;
		}
	}
	...
	if (remoteTRIGGERING) {
		...
		if (com) {
			evst = document.documentElement.scrollTop;
		}
	...
	//apply transition filter
	if (typeof cnObj.filters != "unknown" && typeof cnObj.filters[0] != "undefined") {
		cnObj.filters[0].Apply();
	}
...
function clearCell(ccObj) {
	if (ui[5]) {
		if (ui[3]) {
			ccObj.style.backgroundImage = 'url(' + bk[3].src + ')';
		} else {
		...
		if (ui[3]) {
			...
			ccObj.style.backgroundColor = bk[3];
		}
	}
...
function childCell(mNum, sNum, cNum) {
	...
	if (!keepSubLIT) {
		...
		if (ui[5]) {
			if (ui[3]) {
				scObj.style.backgroundImage = 'url(' + bk[3].src + ')';
			} else {
				scObj.style.backgroundImage = 'url(' + bnk + ')';
			}
		} else {
			if (ui[3]) {
				...
				scObj.style.backgroundColor = bk[3];
			}
		}
	}
...
if (menuALIGN != "free") {
	...
	if (stretchMENU || showBORDERS) {
		...
		if (stretchMENU && showBORDERS) {
			mn += '<table cellpadding=0 cellspacing=' + bSIZE + ' border=0 style="width:' + bWidth + 'px\;height:' + (navH + (2 * bSIZE)) + 'px\;"><tr><td style="' + bks[0] + 'cursor:default">&nbsp;</td></tr></table>';
		}
		mn += '</div>';
	}
	...
	for (i = 0; i < mI.length; i++) {
		clcu = '';
		if (cellCLICK && mI[i][0] != "" && mI[i][0] != "#" && mI[i][0] != "~") {
			clcu = ' cursor:' + aCURSOR + '\;';
		}
		cllu = ' cursor:default\;';
		if (mI[i][0] != "" && mI[i][0] != "#" && mI[i][0] != "~") {
			cllu = ' cursor:' + aCURSOR + '\;';
		}
		titleText = "";
		if (altDISPLAY == "title") {
...
for (i = 0; i < mI.length; i++) {
	...
	if (mtObj) {
		...
		if (mI[i][3] == "left") {
			mtObj.style.left = tINDENT;
		}
		if (mI[i][3] == "right") {
			mtObj.style.left = 0 - tINDENT;
		}
...
function makeStatic() {
	...
	//scm tweak
	if (com) {
		staticTop = absTOP + document.documentElement.scrollTop;
	}
...
function keyListen(DnEvents) {
	...
	if (kc == 13) {
		if (focusID[2] > -1) {
			document.getElementById("childcell-" + focusID[0] + "-" + focusID[1] + "-" + focusID[2]).click();
		} else if (focusID[1] > -1) {
			document.getElementById("subcell-" + focusID[0] + "-" + focusID[1]).click();
		}
	}
	...
	if (eka || eks || ekc || remoteTRIGGERING) {
		return false;
	}
	...
	regexp1 = /^<input/i;
	regexp2 = /^<textarea/i;
	if (regexp1.test(html) || regexp2.test(html)) {
		return false;
	}
	...
	//open and send focus to submenu else if (kcc.indexOf("*") == -1) {
		for (i = 0; i < mI.length; i++) {
			if (kcc == mI[i][8]) {
	...
	//open and send focus to child menu else if (kcc.indexOf("*") != -1 && focusID[0] > -1) {
		if (kcc == "*l" || kcc == "*r") {
			focusID[2]++;
			if (focusID[2] > (cI[focusID[0]][focusID[1]].length - 1)) {
				focusID[2] = 0;
			}
...
function activateMenu(mmNum) {
	...
	if (com) {
		scaroTop = document.documentElement.scrollTop;
	}

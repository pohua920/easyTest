...
for (var f = 0; f < mI.length; f++) {
	if (typeof sP[f] == "undefined") {
		sP[f] = new Array(mI[f][2], mI[f][3], mI[f][3], 0, 0);
	}
	if (typeof cP[f] == "undefined") {
		cP[f] = new Array;
	}
	if (typeof sI[f] == "undefined") {
		sI[f] = new Array;
	}
	if (typeof cI[f] == "undefined") {
		cI[f] = new Array;
	}
	for (var sf = 0; sf < sI[f].length; sf++) {
		if (typeof cP[f][sf] == "undefined") {
			cP[f][sf] = new Array;
		}
		if (typeof cI[f][sf] == "undefined") {
			cI[f][sf] = new Array;
		}
	}
}
...
var Mrows = new Array;
var Nrows = new Array;
var thisNH = new Array;
var thisAH = new Array;
...
for (i = 0; i < mI.length; i++) {
	thisNH[i] = nH;
	if (menuALIGN == "free") {
		thisNH[i] = onH * Nrows[i];
	}
	thisAH[i] = nH + (bSIZE * 2);
	if (menuALIGN == "free") {
		thisAH[i] = thisNH[i] + (2 * bSIZE);
	}
}
...
if (menuALIGN == "left") {
	...
	abR = aL;
	rL = aL;
	if ((op5 || stretchMENU) && abR > 0) {
		stAdjust = abR + (2 * bSIZE);
	}
	if (rL < 0) {
		rL = 0;
	}
...
var ntl = 0;
for (var intl = 0; intl < mI.length; intl++) {
	if (mI[intl][1] != "") {
		ntl++;
	}
}
...
for (var b = 0; b < 6; b++) {
	...
	if ((back_defs[b].indexOf('.gif') != -1) || (back_defs[b].indexOf('.jpg') != -1) || back_defs[b] == '') {
		useIMG[b] = true;
	}
	if (useIMG[b]) {
...
function storeObjects() {
	...
	for (var sob = 0; sob < mI.length; sob++) {
		...
		if ((vOFFSET + sP[sob][3]) > 0) {
			oR[3][sob] = d.getElementById('gridblocker' + sob);
		}
		if (d.getElementById('gridLblocker' + sob)) {
			oR[12][sob] = d.getElementById('gridLblocker' + sob);
		}
		if (shCOLOR != "") {
			oR[4][sob] = d.getElementById('shadow' + sob);
		}
		for (var sobr = 5; sobr < 17; sobr++) {
			oR[sobr][sob] = new Array;
		}
		for (var soj = 0; soj < sI[sob].length; soj++) {
			...
			if (cP[sob][soj]) {
				if (cP[sob][soj].length > 0) {
					...
					if (shCOLOR != "") {
						oR[7][sob][soj] = d.getElementById('childshadow' + sob + '-' + soj);
					}
					if ((chhOFFSET + cP[sob][soj][4]) > 0) {
						oR[8][sob][soj] = d.getElementById('childgrid' + sob + '-' + soj);
					}
...
function aCM(n) {
	if (pId > -1) {
		...
		if (typeof oR[0][pId] != "undefined") {
			miH(oR[0][pId]);
		}
		if (typeof oR[3][pId] != "undefined") {
			miH(oR[3][pId]);
		}
		if (typeof oR[12][pId] != "undefined") {
			miH(oR[12][pId]);
		}
		if (typeof oR[1][pId] != "undefined") {
			miH(oR[1][pId]);
		}
		if (typeof oR[2][pId] != "undefined") {
			miH(oR[2][pId]);
		}
		if (typeof oR[4][pId] != "undefined") {
			miH(oR[4][pId]);
		}
		if (pCId > -1) {
			if (typeof oR[5][pId][pCId] != "undefined") {
				miH(oR[5][pId][pCId]);
			}
			if (cP[pId][pCId]) {
				if (cP[pId][pCId].length > 0) {
					for (cjc = 0; cjc < cI[pId][pCId].length; cjc++) {
						if (typeof oR[11][pId][pCId][cj] != "undefined") {
							miH(oR[11][pId][pCId][cj]);
						}
						if (typeof oR[15][pId][pCId][cj] != "undefined") {
							miH(oR[15][pId][pCId][cj]);
						}
						if (typeof oR[16][pId][pCId][cj] != "undefined") {
							miH(oR[16][pId][pCId][cj]);
						}
					}
					if (typeof oR[10][pId][pCId] != "undefined") {
						miH(oR[10][pId][pCId]);
					}
					if (typeof oR[9][pId][pCId] != "undefined") {
						miH(oR[9][pId][pCId]);
					}
					if (typeof oR[8][pId][pCId] != "undefined") {
						miH(oR[8][pId][pCId]);
					}
					if (typeof oR[7][pId][pCId] != "undefined") {
						miH(oR[7][pId][pCId]);
					}
				}
			}
		}
		if (typeof obGrid != "undefined") {
			miH(obGrid);
		}
	}
	pId = -1;
...
function oM(n, gridTrue) {
	if (typeof mI[n] == "undefined") {
		return false;
	}
	if (typeof oR[0][n] != "undefined") {
		miV(oR[0][n]);
	}
	if (sI[n] != "") {
		if (sI[n] != '') {
			if (typeof oR[4][n] != "undefined" && sI[n].length > 0) {
				miV(oR[4][n]);
			}
		}
		if (typeof oR[1][n] != "undefined") {
			miV(oR[1][n]);
		}
		if (typeof oR[2][n] != "undefined") {
			miV(oR[2][n]);
		}
		if (typeof oR[3][n] != "undefined") {
			miV(oR[3][n]);
		}
		if (typeof oR[12][n] != "undefined") {
			miV(oR[12][n]);
		}
		if (pCId > -1) {
			miH(oR[5][n][pCId]);
...
function cCM(snum, cnum) {
	if (pCId > -1) {
		if (typeof oR[5][snum][pCId] != "undefined") {
			miH(oR[5][snum][pCId]);
		}
		if (cP[snum][pCId].length > 0 && cI[snum][pCId].length > 0) {
			if (typeof oR[9][snum][pCId] != "undefined") {
				miH(oR[9][snum][pCId]);
			}
			if (typeof oR[10][snum][pCId] != "undefined") {
				miH(oR[10][snum][pCId]);
			}
			if (typeof oR[7][snum][pCId] != "undefined") {
				miH(oR[7][snum][pCId]);
			}
			if (typeof oR[8][snum][pCId] != "undefined") {
				miH(oR[8][snum][pCId]);
			}
		}
...
function goToUrl(cUrl, cTarg) {
	if (cellCLICK || (cellCLICK == mu)) {
		...
		if (cTarg == "_self") {
			self.document.location = cUrl;
			return true;
		} else if (cTarg == "_top") {
			top.document.location = cUrl;
			return true;
		} else if (cTarg == "_parent") {
			parent.document.location = cUrl;
			return true;
		} else if (cTarg == "_blank") {
			var newwin = open(cUrl);
			return true;
		} else {
			if (top[cTarg]) {
				top[cTarg].document.location = cUrl;
			} else {
				//to support iframe
...
if (bCOLOR != "") {
	...
	if (useIMG[1]) {
		spbk = ' background-image:url(' + bh + bCOLOR + ')\; ';
	}
}
...
for (i = 0; i < mI.length; i++) {
	...
	if (mI[i][5] != "none") {
		...
		if (altDISPLAY == "status") {
			aTxt[i] = ' onmouseover="return writeStatus(mI[' + i + '][5])" onmouseout="return writeStatus(altNull)" ';
		}
	}

	space[i] = ' left:0px\;';
	if (mI[i][3] == "left") {
		space[i] = ' left:' + tINDENT + 'px\;';
	}
	if (mI[i][3] == "right") {
		space[i] = ' left:-' + tINDENT + 'px\;';
	}
...
for (var c = 0; c < mI.length; c++) {
	...
	if ((vOFFSET + sP[c][3]) > 0) {
		M += '<div id="gridblocker' + c + '" style="width:' + sP[c][0] + 'px\; height:' + (vOFFSET + sP[c][3]) + '\; visibility:hidden\; z-index:' + (zORDER + 1) + '\;position:absolute\; top:' + (aP[xp] + thisAH[c]) + 'px\; left:' + acL + 'px\;" onmouseover="if(rTimer!=0){clearTimeout(rTimer)\;rCount=0\;}"></div>';
	}
	...
	if (gblw > 0) {
		M += '<div id="gridLblocker' + c + '" style="width:' + gblw + 'px\; height:' + (((snH + sbSIZE) * sI[c].length) + ((snH) * Xrows[c]) + sbSIZE + shb + shSIZE) + 'px\; visibility:hidden\; z-index:' + (zORDER + 1) + '\;position:absolute\; top:' + (aP[xp] + thisAH[c] + vOFFSET + sP[c][3]) + 'px\; left:' + gbposL + 'px\;" onmouseover="if(rTimer!=0){clearTimeout(rTimer)\;rCount=0\;}"></div>';
	}
	...
	for (i = 0; i < sI[c].length; i++) {
		...
		if (sI[c][i][3] != "none") {
			if (altDISPLAY == "title") {
				sTxt[c][i] = ' title="' + sI[c][i][3] + '" ';
			}
			if (altDISPLAY == "status") {
				sTxt[c][i] = ' onmouseover="return writeStatus(sI[' + c + '][' + i + '][3])" onmouseout="return writeStatus(altNull)" ';
			}
		}
	...
	if (shCOLOR != "") {
		M += '<span id="shadow' + c + '" style="visibility:hidden\;background-image:url(' + bh + shCOLOR + ')\; background-color:' + shCOLOR + '\; z-index:' + (zORDER + 3) + '\; position:absolute\; top:' + (aP[xp] + thisAH[c] + vOFFSET + sP[c][3] + shSIZE - shb) + 'px\; left:' + (acL + shSIZE - shb) + 'px\; width:' + (sP[c][0] + shb) + '\; height:' + (((snH + sbSIZE) * sI[c].length) + ((snH) * Xrows[c]) + sbSIZE + shb) + 'px\;"></span>';
	}
		...
		if (cP[c][sq].length > 0 && cI[c][sq].length > 0) {
			...
			for (cj = 0; cj < cI[c][sq].length; cj++) {
				...
				if (cI[c][sq][cj][3] != "none") {
					if (altDISPLAY == "title") {
						cTxt[c][sq][cj] = ' title="' + cI[c][sq][cj][3] + '" ';
					}
					if (altDISPLAY == "status") {
						cTxt[c][sq][cj] = ' onmouseover="return writeStatus(cI[' + c + '][' + sq + '][' + cj + '][3])" onmouseout="return writeStatus(altNull)" ';
					}
				}
				...
				if (cI[c][sq][cj][0] == "#") {
					cI[c][sq][cj][0] = "javascript:doNothing()";
				}
...
function findWindowSize() {
	...
	if (newBW == oldBW && newBH == oldBH) {
		...
		clearTimeout(fws);
		if (window.innerWidth > (nS + absLEFT)) {
...
function activateMenu(mmNum) {
	if (typeof sP[mmNum] == "undefined" || !remoteTRIGGERING) {
		return false;
	}

 ...
 *           zhouxianli   2002-11-21       新增函数compareFullDate,//比较两个日期字符串
                                           date1=date2则返回0 , date1>date2则返回大於0的整数, date1<date2则返回小於0的整数
...
/************************************************************************************
  ...
  日期处理函数
                ...
                convertFullDateToString 得到日期的字符串表达形式，传入参数为Date类型,如果不传，则默认为当天
        				...
        				getBeforeDateFullDate   得到上N天  
								getBeforeMonthFullDate  得到上n月
								getBeforeYearFullDate	  得到上n年
  ...
  输入域校验函数
                ...
                checkLength             如果输入域不为空，校验输入域的值是否为不大於最大长度的字符型,如果不是则弹出提示且无法离开
                checkCustom             如果输入域不为空，校验输入域的值是否为指定格式的值,如果不是则弹出提示且无法离开
                checkNull               判断输入域不为空，为空提示，但是不锁定焦点

  字符串处理函数
                leftTrim                返回输入字符串去除左侧开始的空格後的字符串
                rightTri                返回输入字符串去除右侧开始的空格後的字符串
                trim                    返回输入字符串去除左右两侧开始的空格後的字符串
                replace                 返回输入字符串全部替换後的结果
                newString               将给定字符串复制ｎ遍並返回
  ...
  其他
                ...
                setOnchangeOfTable        一次给所有的input域添加onchange方法，且保留原有onchange()，不适用於多行的上一条/下一条模式
                setOnchangeOfTableSpecial 一次给所有的input域添加onchange方法，且保留原有onchange()，用於多行的上一条/下一条模式
                setOption                 分割代码並放在select域里，串的格式: 值FIELD_SEPARATOR文本GROUP_SEPARATOR值FIELD_SEPARATOR文本...
...
function checkDatetime(field,from,to)
{
  ...
  if(from=="year" && to=="month")
  {
    ...
    if(!(month>=1 && month<=12))
    {
      errorMessage("请输入合法的月份数！月份数应该为大於等於1小於等於12的整数！");
      field.focus();
      field.select();
      return false;
    }
  ...
  else if(from=="year" && to=="minute")
  {
    ...
    if(!(hour>=0 && hour<=24))
    {
      errorMessage("请输入合法的小时数！小时数应该为大於等於0小於等於24的整数！");
      field.focus();
      field.select();
      return false;
    }
    ...
    {
      errorMessage("请输入合法的分钟数！分钟数应该为大於等於0小於等於59的整数！");
      field.focus();
      field.select();
      return false;
    }
...
/**********************************************
  * 都邦项目组  孙兆云
  * 发动机号校验函数  功能描述：校验字段长度是不是在min 和max之间 字段不允许为空
  *******************************************/
function  checkLength2(field,min,max)
{
 ...
 if(desc==null)
 {
  ...
  //alert("3333333333");
   if(value=="")
  {
    errorMessage(i18n.common.pleaseInputs+ desc );  //请输入
      //field.focus();
      //field.select();
      return false;
  }
 ...
 if(value.indexOf("^")>-1 ||
  ...
  {
    errorMessage(i18n.common.systemReservNotAllow);  //为系统保留字符，不允许输入！
    //field.focus();
    //field.select();
    return false;
  }
 ...
 for(var i=0;i<value.length;i++)
  ...
  if(count<minlength)
  {
   errorMessage(desc + i18n.common.contentInputNot + desc + i18n.common.minLengthss + minlength + i18n.common.englishCharacters); //输入的内容不够！\n       的最小长度为     个英文字符！\n请重新输入！
    //field.focus();
   // field.select();
    return false;
  }
  ...
  if(count>maxlength)
  {
   errorMessage(desc + i18n.common.veryLong + desc + i18n.common.maxLengthss + maxlength + i18n.common.englishCharacters);  //输入的内容超长！\n   的最大长度为           个英文字符！\n请重新输入！
    //field.focus();
    //field.select();
    return false;
  }  
...
/**********************************************
  * 都邦项目组  孙兆云
  * 车架号校验函数  功能描述：校验字段长度是不是在min 和max之间 字段允许为空
  *******************************************/
...
function insertRowForPG(PageCode,DataPageCode)
{
  ...
  {
    errorMessage(PageCode + "_Flag 資料欄位不存在");
  }
}
  ...
  if(mode==1)
  {
    ...
    for(i=0;i<fields.length;i++)
    {
      ...
      else //输入只有一个
      {
        ...
        {
          if(fm.all(fields[i]).style.display!='none' &&  //域处於显示状态
        ...
        else
        {
          if(fm.all(fields[i]).type!='button' &&         //非按钮
             ...
             fm.all(fields[i]).style.display!='none' &&  //域处於显示状态
  ...
  else if(mode==2)
  {
    ...
    for(i=1;i<=len;i++)
    {
      ...
      for(j=0;j<fields.length;j++)
      {
        ...
        {
          if(fm.all(fields[j])[i].style.display!='none' &&  //域处於显示状态
            ...
            )
          {
            ...
            break;
                 ...
                 fm.all(fields[j])[i].style.display!='none' &&  //域处於显示状态
    ...
    for(i=0;i<onePageData.length;i++)
    {
      ...
      for(j=0;j<fields.length;j++)
      {
        if(fm.all(fields[j]).type!='button' &&         //非按钮
           ...
           fm.all(fields[j]).style.display!='none' &&  //域处於显示状态
...
function calTimePremium(fieldname,ext)
{
  ...
  try
  {
    ...
    {
      pdate = new Date(fm.EndDate.value);    //批改後的终保日期
...
function ShowContractNo(strBizType,strContractNo)
{
	...
	{
		alert(i18n.common.notProvideValidNo);  //没有提供有效的合同号!
		return;
	}
...
function relate(strPolicyNo)
{
		...
		{
		 	alert(i18n.common.notProvidePolicyNo);  //没有提供有效的保单号！
		 	return;
		}
...
function relateProposalNo(strProposalNo)
{
		...
		{
		 	alert(i18n.common.notProvideApplicationNo);  //没有提供有效的投保单号！
		 	return;
		}
...
function viewTrace(businessNo)
{
	...
	var submitStr = fm.undwrtUrl.value;//新应用部署方式 赵慧宇 2008-02-29
...
//使用xmlhttp访问页面，並获取数据(财产卢续攀20030407)
function getResponseXmlText(strURL)
{
  ...
  {
    alert(i18n.common.notFindPage+ strURL);  //找不到页面：
    return "";
  }
  else
  {
    alert(i18n.common.access+ strURL +i18n.common.errorWrongNo+objXmlHttp.status);  //访问     出错，错误号：
    return "";
  }
}
...
* @作者 Liujun
* @时间 2008-10-11
* @功能 将一个ID内的input元素剪切到另一个ID内，並保存原始值
*/
...
function getCardKey(){
	...
	try
  {
		var strTemp = bjcactrl.GetUserList();  //证书通用名：代理商5和容器号
		//alert(strTemp);
		var i=strTemp.indexOf("&&&");
		if(i>=0)
		{
			var	strOption = strTemp.substring(0,i);
			var agentName = strOption.substring(0, strOption.indexOf("||"));//获取代理商名称
			var strContainerName = strOption.substring(strOption.indexOf("||") + 2, strOption.length);  //获取证书的容器号
			var lib = bjcactrl.GetUserInfo(strContainerName, 15);//获取key的类型
			if(lib==null||lib=="")
	   	{
	   		alert(i18n.common.withoutInsertKey);  //没有插入key
	   		return;
	   	}
			var varCert = bjcactrl.ExportUserCert(strContainerName);//获取签名证书
			info = bjcactrl.GetCertInfoByOid(varCert,"2.16.840.1.113732.2");//获取代理商编码
	   	info = info.substring(2);
	   	var enableTime = bjcactrl.GetCertInfo(varCert,12);//获取证书有效期限
	   	var caid = bjcactrl.GetBjcaKeyParam(lib,3);//获取CA唯一标识
		}
  }
  catch(e)
  {
      errorMessage(i18n.common.digitalCertificateExcept+e.message);  //数字证书获取异常，可能是没有安装控件，请联系IT人员解决：
  }
	return info;
}	
/**
...
function getLocalIPAddress()
{
    ...
    {
        errorMessage("本地IP獲取異常，可能是沒有安裝控制元件，請聯繫IT人員解決："+e.message);
    }

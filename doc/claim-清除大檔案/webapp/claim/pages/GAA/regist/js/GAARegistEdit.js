...
function saveForm(field, saveType) {
	...
	if (EstiCurrency.length < 1) {
		errorMessage = errorMessage + i18n.regist.moneyNotEmpty +"\n"; //币别不允许为空\n
	}
...
function initSet() {
	...
	if(prpLregistDamageStartDate!=""){
		...
		var curDate = getServerCurrTime();
		if (curDate < startDate || curDate > endDate) {
			message = message + i18n.regist.todayWithoutInsurance + "\n"; //系統日期不在保險區間！\n
		}

...
<input title="點選此處可獲得已出險相關信息" type=button ACCESSKEY="." value='...' class="smallbutton" name='button_Peril_Open_Context'
	onclick="buttonOnClick('perilInfoShow',fm.policyno.value,fm.registno.value);">
<span id="perilInfoShow" style='width: 800; display: none; position: absolute; background-color: FFFFFF;'>
	<table class="prompt" style="width: 400">
		...
		<!-- 插入出险次数详细信息-->
		<c:forEach items="${registClaimDtoList}" var="registClaimDto" varStatus="registClaim_status">
			<tr>
				<td class="prompt">${registClaim_status.count }</td>
		...
		<tr>
			<td colspan=14 class="prompttitle">
				<input type=button name='button_Peril_Close_Context' value="<s:text name='button.close.value' />" class="button" ACCESSKEY="O" onclick="hideSubPage(this,'perilInfoShow')">
			</td>
		</tr>
	</table>
</span>
<s:if test="#request.prpPheadDtoList.size()>0">
	<input title="點選此处可获得历次批单相关信息" type=button ACCESSKEY="." class="button" value="<s:text name='button.everySingle.value' />" name='button_Peril_Open_Context2'
		onclick="buttonOnClick2('perilInfoShow2');">
</s:if>
<span id="perilInfoShow2" style='width: 800; display: none; position: absolute; background-color: FFFFFF;'>
	<table class=common cellpadding="5" cellspacing="1">
		...
		<tr>
			<td colspan=14 class="common">
				<input type=button name='button_Peril_Close_Context2' value="<s:text name='button.close.value' />" class="button" ACCESSKEY="O" onclick="hideSubPage2(this,'perilInfoShow2')">
			</td>
		</tr>
	</table>
</span>

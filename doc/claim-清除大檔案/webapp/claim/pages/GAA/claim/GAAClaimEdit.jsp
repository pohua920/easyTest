...
<script src="${ctx }/pages/GAA/claim/js/GAAClaimEdit.js"></script>
<script src="${ctx }/pages/GAA/claim/js/GAAClaimEditDwr.js"></script>
<script type="text/javascript">
	// 當保單承保CA險種時，進入立案頁面，需要彈出提示訊息  begin
	var riskCode = '${prpLclaim.riskCode}';
	if(riskCode == "CA"){
		alert(i18n.prompt.claim.ViewA33And133DeductibleAdjustment); // 本賠案需檢視是否承保A33和133條款之自負額調整
	}
	// 當保單承保CA險種時，進入立案頁面，需要彈出提示訊息 end
	//mpc调整
	$(function() {
		initWindow();
	...
	<form name=fm action="${ctx }/claimSave.do" method="post" onsubmit="return validateForm(this);" autocomplete="off">
		...
		<mpc:container ID="oMPC">
			<mpc:page ID="tabMain" TABTITLE="<s:text name='regist.prpLregist.registMain'/>" TABTEXT="<s:text name='regist.prpLregist.registMain'/>"><%-- 基本讯息 --%>
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%-- 1.立案主头信息 --%>
						<%@include file="/pages/GAA/claim/GAAClaimMainHeadEdit.jsp"%>
						<%-- 2.立案中间信息 --%>
						<%@include file="/pages/GAA/claim/GAAClaimMainMiddleEdit.jsp"%>
						<%-- 3.立案尾信息 --%>
						<%@include file="/pages/GAA/claim/GAAClaimMainTailEdit.jsp"%>
						<%--送审审核片语和意见 --%>
			...
			<mpc:page ID="tabMain" TABTITLE="<s:text name='claim.infoAmountLoss'/>" TABTEXT="<s:text name='claim.infoAmountLoss'/>"><%-- 估损金额讯息 --%>
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%@include file="/pages/GAA/claim/GAAClaimLossEdit.jsp"%>
					</DIV>
				</CENTER>
			</mpc:page>
			<mpc:page ID="tabMain" TABTITLE="<s:text name='db.regist.registText.textType1'/>" TABTEXT="<s:text name='db.regist.registText.textType1'/>"><%-- 出险摘要 --%>
				<CENTER>
					<DIV name="tabMain" class="tabMain">
						<%-- 7.立案报告 --%>
						<%@include file="/pages/GAA/claim/GAAClaimTextEdit.jsp"%>
						<%-- 6.巨灾代码--%>
		...
		<TABLE id="btnCommon" class="common">
			<TR>
				<TD align="center">
					<c:if test="${param.editTypeOther!='SHOWTASK'}">
						<%-- 10.保存通用按钮 --%>
						<%@include file="/pages/GAA/claim/GAAClaimSave.jsp"%>
					</c:if>
				</TD>
			</TR>
		</TABLE>

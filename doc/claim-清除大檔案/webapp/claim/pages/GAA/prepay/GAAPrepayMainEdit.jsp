...
<table border="0" align="center" cellpadding="0" cellspacing="0" class="common">
	<tr>
		...
		<c:if test="${param.editTypeOther!='SHOWTASK'}">
			<td>
				<input type="button" name="message" value="<s:text name='button.claimsProcessingRecords.value'/>" class="bigbutton" onclick="openwinSave()">
			</td>
			<td width="70%" align="right">
				<font color="#666666"> <s:text name="scheduleObject.note1" />“<font color="#FF0000">*</font>”<s:text name="scheduleObject.note2" />，“<img src="${ctx}/images/bgDoubleClick2.gif" width="13"
					height="13" align="absbottom">”<%-- 赔案处理记录 --%>
					<%-- 注: --%>
...
<table border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#2D8EE1" class="title" width="100%">
	...
	<tr>
		...
		<%--币别  --%>
		<td class="input">
			<input name="prpLprepayCurrency" class="readonly" readonly style="width: 40px" value="${prpLprepay.currency}" ondblclick="code_CodeSelect(this, 'Currency');"
				onkeyup="code_CodeSelect(this, 'Currency');">
			<input name="prpLprepayCurrencyName" class="readonly" readonly style="width: 100px" value="${prpLprepay.currencyName}" ondblclick="code_CodeSelect(this, 'Currency','-1','always','none','post');"
	...
	<tr>
		...
		<%-- 经办人 --%>
		<td class="input">
			<input name="prpLprepayHandlerCode" class="codecode" style="width: 40%" value="${prpLprepay.handlerCode}" ondblclick="code_CodeSelect(this, 'HanderCode');"
				onkeyup="code_CodeSelect(this, 'HanderCode');">
			<input name="prpLprepayHandlerName" class="codename" style="width: 50%" title="经办人" value="${prpLprepay.handlerName}" ondblclick="code_CodeSelect(this, 'HanderCode','-1','always','none','post');"
	...
	<c:if test="${coinsFlag=='1'}">
		<tr>
			...
			<%-- 是否代付赔款 --%>
			<td class="right">
				<input type="radio" name="isPayForOther" <c:if test="${prpLprepay.isPayForOther=='1'}">checked</c:if> value="1">
				<s:text name="certainLoss.thirdCarLoss.yes" />
				<%-- 是 --%>
				<input type="radio" name="isPayForOther" <c:if test="${prpLprepay.isPayForOther=='0'}">checked</c:if> value="0">

...
$(window).unload(function() {
 	var objAML = window.opener.fm.prpLchargeAMLFlag ;
 	if (typeof objAML == "object") {
 	 	var serialNo = fm.serialNo.value;
		var ownerName = window.opener.fm.prpLchargeOwnerName[serialNo].value ;
		var certificateCode = window.opener.fm.prpLchargeUniformNo[serialNo].value ;
		var amlFlag = window.opener.fm.prpLchargeAMLFlag[serialNo].value ;
 ...
 function submitForm(index) {
 	...
 	var accountBankName = document.getElementsByName("prpdpaymentaccountcustomBankName")[index]; //分行名稱
 	...
 	if (ownerPhoneNo == null || trim(ownerPhoneNo.value) == "") {
	    ...
	    if (ownerPhoneNo.length > 0) {
			if (ownerPhoneNo.substr(0, 2)=='09'){
				    var reg =/^09[0-9]{8}$/;
				   if(!reg.test(ownerPhoneNo)){
				     alert("帳戶歸屬人聯系電話有誤\n");
				     return false;
				    }
				} else {
				    var  reg =/^[0-9]{2,3}[0-9]{7,8}$/;
				    if (!reg.test(ownerPhoneNo)){
				    	 alert("帳戶歸屬人聯系電話有誤\n");
				    	 return false;
				     }
 	...
 	var bankName = document.getElementsByName("prpdpaymentaccountBankName")[index].value
 	var inputArgs = {
 		...
 		CustomBankName: document.getElementsByName("prpdpaymentaccountcustomBankName")[index].value,
 	...
 	if (isPaymentUpdate.value == "0") {
 		backSaveAccount_show();
 	} else {
 		uiAccountCodeAction.saveAccount(inputArgs, backSaveAccount_show);
 	}
 }

 function backSaveAccount_show() {
 	var index = index_obj;
 	var serialNo = fm.serialNo.value;
 	var prpdpaymentaccountBankCode = document.getElementsByName("prpdpaymentaccountBankCode")[index];
 	var bankName = document.getElementsByName("prpdpaymentaccountBankName")[index].value
 	try {
 		window.opener.fm.prpLchargeAccountCode[serialNo].value = document.getElementsByName("prpdpaymentaccountAccountCode")[index].value; //帳户名称
 		window.opener.fm.prpLchargeBankName[serialNo].value = bankName //总行名称
 		window.opener.fm.prpLchargeBankCode[serialNo].value = document.getElementsByName("prpdpaymentaccountBankCode")[index].value; //总行代码
 		window.opener.fm.prpLchargeCustomBankName[serialNo].value = document.getElementsByName("prpdpaymentaccountcustomBankName")[index].value; //分行名称
 		window.opener.fm.prpLchargeCustomBankCode[serialNo].value = document.getElementsByName("prpdpaymentaccountCustomBankCode")[index].value; //分行代码
 		window.opener.fm.prpLchargeOwnerName[serialNo].value = document.getElementsByName("prpdpaymentaccountOwnerName")[index].value; //赔付对象
 		window.opener.fm.prpLchargeUniformNo[serialNo].value = document.getElementsByName("prpdpaymentaccountCertificateCode")[index].value; //统一编号
 		window.opener.fm.prpLchargeCourierAddress[serialNo].value = document.getElementsByName("prpdpaymentaccountCourierAddress")[index].value; //邮政地址
 		window.opener.fm.prpLchargeAreaCode[serialNo].value = document.getElementsByName("prpdpaymentaccountAreaCode")[index].value; //邮政区号
 		window.opener.fm.prpLchargeCertificateCode[serialNo].value = document.getElementsByName("prpdpaymentaccountCertificateType")[index].value; //證件類型
 		if (typeof window.opener.fm.prpLchargeAMLFlag[serialNo] == "object") {
 			window.opener.fm.prpLchargeAMLFlag[serialNo].value = getAMLResponse(index) ; //洗錢回覆字串
 		}	
 	} catch (e) {}
 	window.close();
 	return;
...
<body style="overflow: scroll">
	<form name="fm" action="${ctx}/AccountCode.do" method="post" autocomplete="off">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr align="center">
				<td>
					<input type="button" class="bigbutton" name="button" value="<s:text name="button.common.addAccountInformation"/>"
						onClick="paymentaccountAdd();">
				</td>
				<!-- 新  增  帳  户  信  息 -->
			</tr>
			<tr>
		...
		<c:forEach items="${requestScope.PaymentAccounList}" var="prpdpaymentaccountDto" varStatus="stat">
			<c:out value="${prpdpaymentaccountDto.serialNo}" />
			...
			<table border="0" cellpadding="5" cellspacing="1" class="subtable">
				<tr>
					<td colspan="4" align="center" class="common">
						<strong><s:text name="common.accountInformation" />
							<!--帳户信息-->
							<c:out value="${stat.count}" /> <s:text name="common.accountName" />
							<!-- 帳户名称： -->
							<c:out value="${prpdpaymentaccountDto.accountName}" /></strong>
					</td>
				</tr>
				<tr>
					...
					<td width="30%" class="right">
						<input type="text" class="readonly" name="prpdpaymentaccountAccountCode" onblur="setPayment(this);" title="${prpdpaymentaccountDto.accountCode}"
							value="<c:out value="${prpdpaymentaccountDto.accountCode}" />" readonly="readonly">
						<img src="/claim/images/imgMustInput.gif" />
						<input type="text" class="common" name="prpdpaymentaccountAccountCurrency" onblur="setPayment(this);" title="${prpdpaymentaccountDto.accountCurrency}"
				...
				<tr>
					<td width="20%" class="left">
						<s:text name="common.headquartersCode" />：
					</td>
					<!-- 总行代码： -->
					<td width="30%" class="right">
						<input type="text" class="readonly" readonly name="prpdpaymentaccountBankCode" onchange="setPayment(this);" onkeyup="getBank(this,'codeCode','0,1','1');"
							onblur="setPayment(this);isBank(this,'codeCode','1');" value="${prpdpaymentaccountDto.bankCode}" />
						<img src="/claim/images/imgMustInput.gif" />
					</td>
					<td width="20%" class="left">總行名稱：</td>
					<td width="30%" class="right">
						<input type="text" class="readonly" readonly name="prpdpaymentaccountBankName" onchange="setPayment(this);" onblur="setPayment(this);isBank(this,'codeName','1');"
							onkeyup="getBank(this,'codeName','-1,0','1');" value="${prpdpaymentaccountDto.bankName}" />
						<img src="/claim/images/imgMustInput.gif" />
					</td>
				</tr>
				<tr>
					...
					<!-- 分行代號： -->
					<td width="30%" class="right">
						<input type="text" class="common" id="prpdpaymentaccountCustomBankCode" name="prpdpaymentaccountCustomBankCode" maxlength="50" onchange="setPayment(this);"
							value="<c:out value="${prpdpaymentaccountDto.customBankCode}" />" onkeyup="getBank(this,'codeCode','0,1,-2,-1','2');" onblur="isBank(this,'codeCode','2');" />
						<img src="/claim/images/imgMustInput.gif" />
					</td>
					<td width="20%" class="left">分行名稱：</td>
					<!-- 分行名稱 -->
					<td width="30%" class="right">
						<input type="text" class="common" id="prpdpaymentaccountBankName" name="prpdpaymentaccountcustomBankName" maxlength="50" onblur="isBank(this,'codeName','2');" onchange="setPayment(this);"
							value="<c:out value="${prpdpaymentaccountDto.customBankName}" />" onkeyup="getBank(this,'codeName','-1,0,-3,-2','2');">
						<img src="/claim/images/imgMustInput.gif" />
					</td>
				</tr>
				<tr>
					<td width="20%" class="left">
						<s:text name="common.accountName" />：
					</td>
					<!-- 帳户名称： -->
					<td width="30%" class="right">
				...
				<tr style='display: none'>
					...
					<td width="20%" class="left">
						<s:text name="common.maintenanceUnitCode" />
					</td>
					<!-- 维修单位代码 ：-->
					<td width="30%" class="right">
				...
				<tr>
					<td width="20%" class="left">
						<s:text name="common.accountOwnershipAttribute" />：
					</td>
					<!-- 帳户归属人属性:  -->
					<td width="30%" class="right">
						<select name="prpdpaymentaccountOwnerType" class="common" onchange="setPayment(this);">
					...
					<td width="20%" class="left">
						<s:text name="common.accountOwnershipPersonName" />：
					</td>
					<!-- 帳户归属人姓名(支付对象帳户名称)： -->
					<td width="30%" class="right">
				...
				<tr>
					<td width="20%" class="left">
						<s:text name="common.accountOwnershipCertificateType" />：
					</td>
					<!-- 帳号归属人证件类型： -->
					<td width="30%" class="right">
						<s:select name="prpdpaymentaccountCertificateType" class="common" onchange="setPayment(this);" value="#attr.prpdpaymentaccountDto.certificateType" listKey="key" listValue="value"
							list="#request.prpdpaymentaccountCertificateTypeList" />
					</td>
					<td width="20%" class="left">
						<s:text name="common.accountOwnershipPersonCode" />：
					</td>
					<!-- 帳号归属人证件代码： -->
					<td width="30%" class="right">
				...
				<tr>
					<td width="20%" class="left">
						<s:text name="common.accountOwnershipPhoneNumber" />：
					</td>
					<!-- 帳户归属人联系电话： -->
					<td width="30%" class="right">
						<input type="text" class="common" name="prpdpaymentaccountOwnerPhoneNo" onblur="setPayment(this);" title="${prpdpaymentaccountDto.ownerPhoneNo}"
							value="<c:out value="${prpdpaymentaccountDto.ownerPhoneNo}" />">
						<img src="/claim/images/imgMustInput.gif" />
					</td>
					<td width="20%" class="left">
						<s:text name="common.operatorCode" />：
					</td>
					<!-- 操作人员代码: -->
					<td width="30%" class="right">
				...
				<tr>
					<td width="20%" class="left">
						<s:text name="common.operationsPeople" />：
					</td>
					<!-- 操作人归属机构： -->
					<td width="30%" class="right">
				...
				<tr>
					...
					<!-- 第一次采集日期： -->
					<td width="30%" class="right">
						<rc:rcDate  class="readonly" readonly="readonly" wdatePicker="false" onblur="setPayment(this);" name="prpdpaymentaccountOperateDate" value="${prpdpaymentaccountDto.operateDate}" />
					</td>
					<td width="20%" class="left">
					...
					<!-- 更新日期： -->
					<td width="30%" class="right">
	      				<rc:rcDate  class="readonly" readonly="readonly" wdatePicker="false" onblur="setPayment(this);" name="prpdpaymentaccountUpdateDate"  value="${prpdpaymentaccountDto.updateDate}" />
					</td>
				</tr>
				<tr style='display: none'>
					<td width="20%" class="left">
	    ...
	    <td width="20%" class="left">統一編號:</td>  -->
				...
				<tr>
					<td align="center" class="common" colspan="4">
						<input type="button" class="bigbutton" name="buttonsave" id="buttonsave" value="確定" onClick="submitForm('<c:out value='${stat.index}'/>')" ;/>
					</td>
				</tr>
				<tr>

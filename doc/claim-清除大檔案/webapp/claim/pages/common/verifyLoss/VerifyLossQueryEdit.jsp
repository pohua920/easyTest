	...
	function init(){
		YAHOO.widget.DataTable.formatLink = function(elCell, oRecord, oColumn, oData) {
			if(oColumn.key =="id.registNo"){
			   elCell.innerHTML="<a  href=\"${ctx}/verifyLoss/verifyLossFinishQueryList.do?prpLverifyLossRegistNo="+oRecord.getData("id.registNo")+"&editType="+oRecord.getData("editType")+"&riskCode="+oRecord.getData("riskCode")+"&lossItemCode="+oRecord.getData("id.lossItemCode")+"&nodeType=${param.nodeType}" + "\">"+ oData+"</a>";
			...
			}else if(oColumn.key =="insureCarFlag") {
				if(oData == "1"){
				  elCell.innerHTML = "標的車";
				}else{
				  elCell.innerHTML = "三者車";
				}
			}else if(oColumn.key =="underWriteEndDate") {
				if(oData!=null){
		...
		contentColumnHeaders =[
			 ...
			 <c:if test="${param.nodeType=='verif'}">
			 {key:"lossItemName",label:"<s:text name='db.prpCitem_car.licenseNo'/>",width:"40em",sortable:true},<%--车牌号码--%>
			 {key:"insureCarFlag",label:"<s:text name='regist.prpLregist.insureCar' />",width:"40em",sortable:true,formatter:YAHOO.widget.DataTable.formatLink},<%--保单车辆--%> 
			 </c:if>
			 <c:if test="${param.nodeType=='veriw'}">
			 {key:"lossItemName",label:"<s:text name='title.wound.personName' />",width:"40em",sortable:true},<%--人伤名称--%>
			 </c:if>
			 <c:if test="${param.nodeType=='propv'}">
			 {key:"lossItemName",label:"<s:text name='certainLoss.lossMarkName'/>",width:"40em",sortable:true},<%--损失名称--%>
			 </c:if>
			 {key:"underWriteCode",label:"<s:text name="backCheck.nuclearRomanCode" />",width:"40em",sortable:true},<%--核损人代码--%>                                               
			 {key:"underWriteEndDate",label:"<s:text name="backCheck.nuclearRomanDate" />",width:"40em",sortable:true,formatter:YAHOO.widget.DataTable.formatLink}<%--核损日期--%>
			];
	}
	/*
	...
	function executeQuery(pageNo,pageSize,field){
  	if((fm.InsuredNameSign.value=="="&&fm.InsuredName.value.length>0)
  	 ...
  	 //输入了一个条件，可以查
		...
		var myDataSource = new YAHOO.util.DataSource("${ctx}/verifyLoss/queryVerifyLoss.do");
		myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
		myDataSource.connMethodPost = true; 
		myDataSource.responseSchema = {
		   resultsList: "data",
		   fields: [{key:"id.registNo"},"policyNo", "underWriteCode", "lossItemName","underWriteEndDate", "status",{key:"id.lossItemCode"}, "riskCode","relatepolicyNo","editType","insureCarFlag"],
		   metaFields : {
...
<form name="fm" action="${ctx}/verifyLoss/queryVerifyLoss.do"  method="post" onsubmit="return validateForm(this);">
    <table width="90%" border="0" align="center" cellpadding="5" cellspacing="1"  class="common">
    <tr><td colspan=4 class="formtitle"><s:text name="certainLoss.nuclearDamageQuery"/></td></tr><%-- 查询核损信息 --%>
      <tr>
      ...
      <tr>
        ...
        <td class='title' ><s:text name="db.prpLclaimStatus.operatedate"/> :</td> <%-- 操作时间 --%>
        <td class='input'>
          ...
          <%-- <input type=text name="OperateDate" class="Wdate" style="width:40%" onclick="WdatePicker()">--%>
      ...
      <tr>
        ...
        <input type="checkbox" name="status" value="4"><s:text name="common.status.submited"/><%-- 已提交 --%>
          ...
          <%--报案查询增加被保险人查询条件--%>
       <td class='title' ><s:text name="db.view_larrearage.insuredname"/>:</td><%-- 被保险人名称 --%>
        <td class='input' >
    ...
    <td class="title" style="color:red" colspan="4">
     ...
     <s:text name="prompt.schedule.query3"/><br><%-- 车险必须输入报案号、保单号、车牌号、被保险人其中一项精确查询！--%>

...
<body >
	<form name="fm" action="${ctx }/claimRepleviedQuery.do" method="post" >
		...
		<c:forEach items="${requestScope.replevyStatusArray}" var="rs">
			<input type="hidden" name="replevyStatus" value="${pageScope.rs}">
		</c:forEach>
		<table cellpadding="3" cellspacing="1"  class="common"
			<thead>
				<tr>
					<td class="formtitle" colspan="14">已追償明細查詢</td>
				</tr>
				<tr class="tableHead">
					...
					<td class="centertitle">可追償金額</td>
					<td class="centertitle">追償金額</td>
					<td class="centertitle">理賠費用</td>
					<td class="centertitle">確認日期</td>
					<td class="centertitle">是否結案</td>
				</tr>
			</thead>
			<tbody>
			<c:forEach items="${requestScope.resultList}" var="mapObject" varStatus="stat">
				...
				<c:forEach items="${mapObject}" var="entry">
					<c:choose>
						<c:when test="${entry.key == 'damagestartdate' || entry.key == 'claimdate' || entry.key == 'replevydate' || entry.key == 'inputdate' }">
							<td align="center"><fmt:formatDate value="${entry.value}" pattern="yyyy-MM-dd"/></td>
						</c:when>
						<c:when test="${entry.key == 'sumloss' || entry.key == 'sumthispaid' || entry.key == 'sumnodutyfee'}">
			...
			<c:if test="${empty requestScope.resultList}">
				<tr class=content bgcolor='#F7F7F7'>
					<td align="center" colspan="13">&nbsp;無記錄！&nbsp;</td>
				</tr>
			</c:if>
			</tbody>
			<tfoot>

...
<body >
	<form name="fm" action="${ctx }/claimOutstandingQuery.do" method="post" >
		...
		<table cellpadding="3" cellspacing="1"  class="common"
			<thead>
				<tr>
					<td class="formtitle" colspan="19">未決賠案明細查詢</td>
				</tr>
				<tr class="tableHead">
					...
					<td class="centertitle">被保險人ID</td>
					<td class="centertitle">車牌號碼</td>
					<td class="centertitle">出險日期</td>
					<td class="centertitle">受理日期</td>
					<td class="centertitle">預估日期</td>
					<td class="centertitle">預估險種</td>
			...
			<c:forEach items="${requestScope.resultList}" var="mapObject" varStatus="stat">
				...
				<c:forEach items="${mapObject}" var="entry">
					<c:choose>
						<c:when test="${entry.key == 'damagestartdate' || entry.key == 'claimdate' || entry.key == 'claimlossdate' || entry.key == 'endcasedate'}">
							<td align="center"><fmt:formatDate value="${entry.value}" pattern="yyyy-MM-dd"/></td>
						</c:when>
						<c:when test="${entry.key == 'sumkindpay'}">
			...
			<c:if test="${empty requestScope.resultList}">
				<tr class=content bgcolor='#F7F7F7'>
					<td align="center" colspan="18">&nbsp;無記錄！&nbsp;</td>
				</tr>
			</c:if>
			</tbody>
			<tfoot>

...
<%
	...
	for(i=0;i<blPrpClimit.getSize();i++){
		...
		if(strLimitType.equals("01")){  //累计赔偿限额
			//取最後一条信息（注掉部分是取多个值的和）
			...
			dbSumAmount = Double.parseDouble(blPrpClimit.getArr(i).getLimitFee());
		...
		} else if(strLimitType.equals("06")){  //绝对免赔额
			//取最後一条信息（注掉部分是取多个值的和）
		...
		} else if(strLimitType.equals("02")){  //每次事故赔偿限额
			//取最後一条信息（注掉部分是取多个值的和）
		...
		} else if(strLimitType.equals("04")){//每次每人赔偿限额
	...
	String strLimitType = ""; //赔偿限额免赔额类别
	String strCurrency01 = ""; //累计赔偿限额币别
	String strCurrency02 = ""; //每次事故赔偿限额币别
	String strCurrency03 = ""; //每次事故财产损失赔偿限额币别
	String strCurrency04 = ""; //每次事故每人赔偿限额币别
	String strCurrency05 = ""; //每次事故每人人身伤亡赔偿限额币别
	String strCurrency06 = ""; //每次事故免赔额币别
	String strCurrency07 = ""; //每次事故免赔率币别
	double dbLimit01 = 0; //累计赔偿限额(累计)
	double dbLimit02 = 0; //每次事故赔偿限额币别（累计）
	double dbLimit03 = 0; //每次事故财产损失赔偿限额币别（累计）
	double dbLimit04 = 0; //每次事故每人赔偿限额币别（累计） 
	double dbLimit05 = 0; //每次事故每人人身伤亡赔偿限额币别（累计)
	double dbLimit06 = 0; //每次事故免赔额币别（累计）
	double dbLimit07 = 0; //每次事故免赔率币别（累计）
	String strLimit01 = ""; //累计赔偿限额（显示格式0.00）
	String strLimit02 = ""; //每次事故赔偿限额（显示格式0.00）
	String strLimit03 = ""; //每次事故财产损失赔偿限额（显示格式0.00）
	String strLimit04 = ""; //每次事故每人赔偿限额（显示格式0.00）v
	String strLimit05 = ""; //每次事故每人人身伤亡赔偿限额（显示格式0.00）
	String strLimit06 = ""; //每次事故免赔额（显示格式0.00）
	String strLimit07 = ""; //每次事故免赔率（显示格式0.00）
	//BLPrpClimit blPrpClimit = new BLPrpClimit();
	//String strSQL = "PolicyNo='" + strPolicyNo + "'";
	//blPrpClimit.query(strSQL);
	//for(i=0;i<blPrpClimit.getSize();i++)
	...
	if (policyDto != null && policyDto.getPrpClimitDtoList() != null && policyDto.getPrpClimitDtoList().size() > 0) {
		for (i = 0; i < policyDto.getPrpClimitDtoList().size(); i++) {
			...
			if (strLimitType.equals("01")) {//累计赔偿限额
				...
				dbLimit01 = prpClimitDto.getLimitFee();
				strCurrency01 = prpClimitDto.getCurrency();
			} else if (strLimitType.equals("03")) {//每次事故财产损失赔偿限额
				dbLimit01 = prpClimitDto.getLimitFee();
				strCurrency01 = prpClimitDto.getCurrency();
			} else if (strLimitType.equals("04")) {//每次事故每人赔偿限额
				dbLimit01 = prpClimitDto.getLimitFee();
				strCurrency01 = prpClimitDto.getCurrency();
			} else if (strLimitType.equals("05")) {//每次事故每人人身伤亡赔偿限额
				dbLimit01 = prpClimitDto.getLimitFee();
				strCurrency01 = prpClimitDto.getCurrency();
			} else if (strLimitType.equals("06")) {//每次事故免赔额
				dbLimit01 = prpClimitDto.getLimitFee();
				strCurrency01 = prpClimitDto.getCurrency();
			} else if (strLimitType.equals("07")) {//每次事故免赔率
				dbLimit01 = prpClimitDto.getLimitFee();
				strCurrency01 = prpClimitDto.getCurrency();
			}
		}
	...
	strLimit02 = new DecimalFormat("0.00").format(dbLimit02);
	strLimit03 = new DecimalFormat("0.00").format(dbLimit03);
	strLimit04 = new DecimalFormat("0.00").format(dbLimit04);
	strLimit05 = new DecimalFormat("0.00").format(dbLimit05);
	strLimit06 = new DecimalFormat("0.00").format(dbLimit06);
	strLimit07 = new DecimalFormat("0.00").format(dbLimit07);
%>
...
<%
	...
	//得到保险金额
	if ("2".equals(prpCmainDto.getCoinsFlag()) || "3".equals(prpCmainDto.getCoinsFlag())) {//从共、从联保额显示总保额
...
function loadForm() {
	...
	if ("2".equals(prpCmainDto.getCoinsFlag()) || "3".equals(prpCmainDto.getCoinsFlag())) {
		...
		if ("1".equals(prpCmainDto.getCoinsFlag())) { //主（联、共）保
	...
	tdUnderWriteFlag.innerHTML = '(' + '<%=underWriteFlag%>' + ')' <% strDamageStartDate = prpLclaimDto.getDamageStartDate().getYear() + "年" + prpLclaimDto.getDamageStartDate().getMonth() + "月" + prpLclaimDto.getDamageStartDate().getDate() + "日";
	...
	if (compensateDto.getPrpLchargeDtoList() != null) {
		for (index = 0; index < intChargeCount; index++) {
			...
			currencys = prpLchargeDto.getCurrency();
			if (strChargeCode.equals("07")) //检验鉴定费
			{
				dblJudgeFee += prpLchargeDto.getChargeAmount() + prpLchargeDto.getSumRealPay();
			} else if (strChargeCode.equals("02")) //核赔费
			{
				dblCheckFee2 += prpLchargeDto.getChargeAmount() + prpLchargeDto.getSumRealPay();
			}
			/** 页面没有代查勘费和诉讼费显示，所以应该计入“其他”项下
					else if( strChargeCode.equals("06") )  //代查勘费
					{    
					  dblCheckFee1 += prpLchargeDto.getChargeAmount()+prpLchargeDto.getSumRealPay();
					} 
					
					else if( strChargeCode.equals("05") )  //诉讼费
					{    
					  dblLawFee   += prpLchargeDto.getChargeAmount()+prpLchargeDto.getSumRealPay();
					}
					 **/
			else if (strChargeCode.equals("04")) //查勘费
			{
				dblCheckFee += prpLchargeDto.getChargeAmount() + prpLchargeDto.getSumRealPay();
			} else if (strChargeCode.equals("13")) //公估费
			{
				dblAssessFee += prpLchargeDto.getChargeAmount() + prpLchargeDto.getSumRealPay();
			//} else if (strChargeCode.equals("03")) //施救费
			//{
				//dblRescueFee += prpLchargeDto.getChargeAmount() + prpLchargeDto.getSumRealPay();
			}
			/** 页面没有代理费显示，所以应该计入“其他”项下
					else if( strChargeCode.equals("09"))   //代理费
					{ 
					  dblAgentFee  += prpLchargeDto.getChargeAmount()+prpLchargeDto.getSumRealPay();
					}
					 **/
			else if (strChargeCode.equals("11")) //共损救助费
			{
				dblGsjzFee += prpLchargeDto.getChargeAmount() + prpLchargeDto.getSumRealPay();
			} else if (strChargeCode.equals("15")) //法律费
			{
				dblFlFee += prpLchargeDto.getChargeAmount() + prpLchargeDto.getSumRealPay();
			} else //其它
			{
				dblElseFee += prpLchargeDto.getChargeAmount() + prpLchargeDto.getSumRealPay();
			}
		}
	}
	if (currencys.equals("")) {
	...
	tdRescueFee.innerHTML = '&nbsp;<%=currencys%>&nbsp;' + '<%=new DecimalFormat("#,##0.00").format(dblRescueFee)%>';
	//查勘费
	tdCheckFee.innerHTML = '&nbsp;<%=currencys%>&nbsp;' + '<%=new DecimalFormat("#,##0.00").format(dblCheckFee)%>';
	//检验鉴定费
	tdJudgeFee.innerHTML = '&nbsp;<%=currencys%>&nbsp;' + '<%=new DecimalFormat("#,##0.00").format(dblJudgeFee)%>';
	//共损救助费
	tdGsjzFee.innerHTML = '&nbsp;<%=currencys%>&nbsp;' + '<%=new DecimalFormat("#,##0.00").format(dblGsjzFee)%>';
	//公估费
	tdAssessFee.innerHTML = '&nbsp;<%=currencys%>&nbsp;' + '<%=new DecimalFormat("#,##0.00").format(dblAssessFee)%>';
	//法律费
	tdFlFee.innerHTML = '&nbsp;<%=currencys%>&nbsp;' + '<%=new DecimalFormat("#,##0.00").format(dblFlFee)%>';
	//其它
	tdElseFee.innerHTML = '&nbsp;<%=currencys%>&nbsp;' + '<%=new DecimalFormat("#,##0.00").format(dblElseFee)%>';
	//核赔费
	tdCheckFee2.innerHTML = '&nbsp;<%=currencys%>&nbsp;' + '<%=new DecimalFormat("#,##0.00").format(dblCheckFee2)%>'; <% strSumThisPaid = new DecimalFormat("#,##0.00").format(dblSumThisPaid - dblJudgeFee - dblCheckFee - dblLawFee - dblElseFee - dblSumPrePaid - dblSumRest);
	...
	if (prpLextDto != null) {
		...
		strSailStartDate = prpLextDto.getSailStartDate().getYear() + "年" + prpLextDto.getSailStartDate().getMonth() + "月" + prpLextDto.getSailStartDate().getDay() + "日";

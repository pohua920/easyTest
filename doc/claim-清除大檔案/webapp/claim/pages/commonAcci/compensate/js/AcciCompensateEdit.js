...
function checkLossAmountForComp(field, saveType){
	...
	if(hasTr47){//有TR47 僅能TR47不能帶其他險類
		$("input[name='prpLpersonLossKindCode']").each(function() {
		    var kindCode = $(this).val();
		    if(kindCode!=""){
			    if(kindCode!="TR47" && check){
			    	check = false;
					alert("TR47海外突發疾病健康保險，不能再選擇其他險別");
					return false;
			    }
		    }
		});
	...
	if(check){
		$span_prpDpolicyRules.each(function() {
			...
			if(!check){
				alert("出險地點為指定地區時，必須有TR47海外突發疾病健康保險");
				return false;
			}
		});
	}
	if(!amountCheck && check){
		...
		if(!amountCheck){
			alert("賠損人的被保險人賠付訊息TR47賠付金額必須小於等於保險金額!");
			return false;
		}
	}
	if(amountCheck && check){

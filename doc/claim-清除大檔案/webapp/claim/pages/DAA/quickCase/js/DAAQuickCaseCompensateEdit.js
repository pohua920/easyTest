...
function initSet() {
	...
	if (chiefFlag != 0) {
		alert(i18n.compensate.policyAttentGenerateInfo); //本保单为主共保单,请注意生成联共保分摊信息!
	}
...
function saveForm(field, saveType) {
	...
	if ('1' == fm.isSpecial.value && (fm.prplregistAdvance.value == '1' || fm.prplregistAdvance.value == '2') && '4' == saveType) //如果是配置了的机构和险别且为全责垫付状况
	{
	...
	for (var j = 1; j < fm.prpLchargeSerialNo.length; j++) {
		if (fm.prpLchargeKindCode[j].value == "") {
			alert(i18n.compel.first + j + i18n.compel.costRisksCodeNoEmpty); //第    条费用险别代码不能为空
			return false;
		}
		if (fm.prpLchargeKindName[j].value == "") {
			alert(i18n.compel.first + j + i18n.compel.costRisksNameNoEmpty); //第    条费用险别名称不能为空
			return false;
		}
	}
	...
	if (D3Count > 1) {
		alert(i18n.compensate.driverLiabilityPerson); //驾驶员责任险只能赔偿1人
		return false;
	}
	if (D3D4Count > seatCount) {
		alert(i18n.compensate.liabilityPersonnelVehicleNo); //车上人员责任险赔偿人数不能大於车辆核定载客数
		return false;
	}
	var errorMessage = "";
	...
	if (context.length < 1) {
		errorMessage = errorMessage + i18n.compel.adjustReportNotAllowNull; //理算报告不允许为空\n
	}
	...
	if (!validateForm(fm, 'Engage_Data,CompensateLoss_Data,PersonFeeLoss_Data,Person_Data,Charge_Data')) {
		return false;
	}


	if (parseInt(fm.prpLcompensateIndemnityDutyRate.value) > 100 || parseInt(fm.prpLcompensateIndemnityDutyRate.value < 0)) {
		alert(i18n.claim.dutyRatioNoZero); //责任比例不能大於100或小於0
		fm.prpLcompensateIndemnityDutyRate.select();
		fm.prpLcompensateIndemnityDutyRate.focus();
		return false;
	}
	...
	if (saveType == 4) {
		...
		if (prpLlossDtoSerialNo == undefined && prpLpersonLossSerialNo == undefined) {
			alert(i18n.quickCase.payMarkInfoMustFill); //赔付标的信息或赔付人员信息二者其一必须填写，否则不能提交实赔
			return false;
		}
	}
...
function quickCaseGenerateCtext() {
	...
	fm.action = "/claim/compensate/compensateGenerate.do?compelFlag=notCompel";
	fm.target = "fraCalculate";
...
function compelGenerateCtext() {
	...
	fm.action = "/claim/compensate/compensateGenerate.do?compelFlag=compel";
	fm.target = "fraCalculate";
...
function calFund() {
	...
	for (i = 1; i < fm.all("prpLchargeSumRealPay").length; i++) {
		if (fm.all("prpLchargeKindCode")[i].value != ConstantCodes.KINDCODE_D_BZ) {
		...
		} else if (fm.all("prpLchargeKindCode")[i].value == ConstantCodes.KINDCODE_D_BZ) {
			chargeRealPay = parseFloat(fm.all("prpLchargeSumRealPay")[i].value); //记入赔款
			chargeAmount = parseFloat(fm.all("prpLchargeChargeAmount")[i].value); //费用
			if (isNaN(chargeRealPay))
				chargeRealPay = 0;
			...
			compelDblSumDutyPaid = compelDblSumDutyPaid + chargeRealPay;
			compelDblSumNoDutyFee = compelDblSumNoDutyFee + (chargeAmount - chargeRealPay);
		}
	}
	...
	for (i = 1; i < fm.all("prpLlossDtoSumRealPay").length; i++) {
		if (fm.all("prpLlossDtoKindCode")[i].value != ConstantCodes.KINDCODE_D_BZ) {
			lossRealPay = parseFloat(fm.all("prpLlossDtoSumRealPay")[i].value);

			if (isNaN(lossRealPay))
				lossRealPay = 0;
			dblSumDutyPaid = dblSumDutyPaid + lossRealPay;
			if ((fm.all("prpLlossDtoKindCode")[i].value == RISKINFO.KINDCODE_D_A || fm.all("prpLlossDtoKindCode")[i].value == RISKINFO.KINDCODE_D_B) && fm.all("prpLlossDtoFeeTypeCode")[i].value == '27') {
				dblRealPayAB = dblRealPayAB + lossRealPay;
			}
			if (fm.all("prpLlossDtoKindCode")[i].value == RISKINFO.KINDCODE_D_A) {
				dblRealPayA = dblRealPayA + lossRealPay;
			}
		} else if (fm.all("prpLlossDtoKindCode")[i].value == ConstantCodes.KINDCODE_D_BZ) {
			lossRealPay = parseFloat(fm.all("prpLlossDtoSumRealPay")[i].value);
			if (isNaN(lossRealPay))
				lossRealPay = 0;

			compelDblSumDutyPaid = compelDblSumDutyPaid + lossRealPay;
		}
	...
	compelDblSumPaid = compelDblSumDutyPaid + compelDblSumNoDutyFee;

	//计算实赔金额
	compelDblSumThisPaid = compelDblSumDutyPaid;
	...
	fm.compelprpLcompensateSumThisPaid.value = point(round(compelDblSumThisPaid, 0), 0);
	fm.compelPrpLcompensateSumDutyPaid.value = point(round(compelDblSumDutyPaid, 0), 0);
	fm.compelPrpLcompensateSumNoDutyFee.value = point(round(compelDblSumNoDutyFee, 0), 0);
	fm.compelPrpLcompensateSumPaid.value = point(round(compelDblSumPaid, 0), 0);
	fm.compensateSumDutyPaid.value = point(round((compelDblSumDutyPaid + dblSumDutyPaid), 0), 0);
	fm.compensateSumNoDutyFee.value = point(round((compelDblSumNoDutyFee + dblSumNoDutyFee), 0), 0);
	fm.compensateSumPaid.value = point(round((compelDblSumPaid + dblSumPaid), 0), 0);
}
...
function checkKindCode(Field) {
	...
	if (Field.name.indexOf("prpLpersonLossKind") > -1) {
		if (fm.all("prpLpersonLossKindCode")[findex].value != "" &&
			fm.all("prpLpersonLossKindCode")[findex].value != RISKINFO.KINDCODE_D_B &&
			fm.all("prpLpersonLossKindCode")[findex].value != "B1" &&
			fm.all("prpLpersonLossKindCode")[findex].value != RISKINFO.KINDCODE_D_D1 &&
			fm.all("prpLpersonLossKindCode")[findex].value != "D11" &&
			fm.all("prpLpersonLossKindCode")[findex].value != "D12" &&
			fm.all("prpLpersonLossKindCode")[findex].value != "S" &&
			...
			alert(i18n.compel.compensatAllowTrafficAccident); //人员赔付只允许輸入第三者责任、驾驶员责任、乘客责任、交通事故精神损害！
...
function changeLossSumRestShowStyle(Field) //赔付标的信息中若险别为“三者险”则置残值为空，且只读
{
	...
	if (fm.prpLlossDtoKindCode[index1].value == RISKINFO.KINDCODE_D_B && fm.prpLlossDtoFeeTypeCode[index1].value != "27") {
...
function changeLossClaimRate(Field) //赔付标的信息中若险别为“车损险”则置赔付比例为（A险别的保额除以新车购置价），且赔付比例只读
{
	var index1 = 0;
	index1 = getElementOrder(Field) - 1;
	if (fm.prpLlossDtoKindCode[index1].value == RISKINFO.KINDCODE_D_A) {
...
function getValue(Field) {
	var index1 = 0;
	index1 = getElementOrder(Field) - 1;
	if ((fm.prpLlossDtoKindCode[index1].value == RISKINFO.KINDCODE_D_A || fm.prpLlossDtoKindCode[index1].value == RISKINFO.KINDCODE_D_B) && fm.prpLlossDtoFeeTypeCode[index1].value == "27") {
		var countA = 0;
		var countB = 0;
		for (var i = 0; i < fm.all("prpLlossDtoKindCode").length; i++) {
			if (fm.all("prpLlossDtoFeeTypeCode")[i].value == '27') {
				...
				} else if (fm.all("prpLlossDtoKindCode")[i].value == RISKINFO.KINDCODE_D_B) {
					countB++;
					if (countB >= 2) {
						errorMessage("赔付标的中每个险别下的扣免赔只能輸入一次!");
						return false;
		...
		if (countA < 2 && countB < 2) {
			...
			fm.target = "fraSubmit"
			fm.action = "/claim/pages/DAA/compensate/DAAGetValueSubmit.jsp?KindCode=" + KindCode + "&PolicyNo=" + fm.prpLpolicyNo.value + "&Index=" + index1;
			fm.submit();
		...
		if (fm.prpLlossDtoKindCode[index1].value == RISKINFO.KINDCODE_D_A && fm.prpLcompensatePurchasePrice.value != fm.prpLlossDtoAmount[index1].value) {
			setReadonlyOfElementOfLoss(fm.prpLlossDtoClaimRate[index1]);
		}
...
function checkLossDeductibleRate(Field) {
	if (parseFloat(Field.value) < parseFloat(Field.oldValue)) {
		alert(i18n.compel.franchiseIncreaseNotAllowCut); //免赔率只能上调不允许下调！
...
function refreshLoss(Field) {
	...
	var oldAction = fm.action;
	var oldTarget = fm.target;
	fm.action = "/claim/DAA/compensate/DAAGetDeductibleSubmit.jsp?PolicyNo=" + fm.policyNo.value + "&DamageStartDate=" + fm.DamageStartDate.value + "&Index=" + index;
	fm.target = "fraCalculate";
...
function calRealpayForSunny(field) {
	...
	if (fm.prpLcompensateRiskCode.value == "DAS" && fm.all("prpLlossDtoKindCode")[findex].value == "F") {
		DeductibleRate = 10.0;
		fm.LossDeductibleRate.value = point(round(DeductibleRate, 0), 0)
	}
	...
	if (fm.prpLcompensateRiskCode.value == "DAS" && fm.all("prpLlossDtoKindCode")[findex].value == "F" && Deductibletemp < 100.0 && temp > 0.0) {
		...
		if (temp > 0) {
			...
			fm.all("prpLlossDtoDeductibleRate")[findex].value = point(round(DeductibleRate, 0), 0)
		}
	} else {
		//个别附加险要获得主险的绝对免赔率
		if (fm.all("prpLlossDtoKindCode")[findex].value == RISKINFO.KINDCODE_D_D2 ||
			...
			mainKindCode = RISKINFO.KINDCODE_D_B;
		}
		if (fm.all("prpLlossDtoKindCode")[findex].value == "G0" ||
			...
			mainKindCode = RISKINFO.KINDCODE_D_A;
		}
	...
	if ((fm.all("prpLlossDtoKindCode")[findex].value == RISKINFO.KINDCODE_D_B || fm.all("prpLlossDtoKindCode")[findex].value == RISKINFO.KINDCODE_D_D2 || fm.all("prpLlossDtoKindCode")[findex].value == "W") && temp > Amount) {
	...
	if ((fm.all("prpLlossDtoKindCode")[findex].value == RISKINFO.KINDCODE_D_A || fm.all("prpLlossDtoKindCode")[findex].value == RISKINFO.KINDCODE_D_B) && fm.all("prpLlossDtoFeeTypeCode")[findex].value == "27") {
		Realpay = SumRest * (-1);
	}
	fm.all("prpLlossDtoSumRealPay")[findex].value = point(round(Realpay, 0), 0);
	//计算责任赔款合计、赔款合计、其它费用、实赔金额
	calFund();
...
function calRealpay1ForSunny(Field) {
	...
	//部分险别需要承上所在主险的绝对免赔率
	if (fm.all("prpLpersonLossKindCode")[findex1].value == RISKINFO.KINDCODE_D_D2 ||
...
function getArrangeRate(field) {
	...
	for (i = 1; i < fm.all("prpLlossDtoArrangeRate").length; i++) {
		if (fm.all("prpLlossDtoKindCode")[i].value == prpLlossDtoKindCode_A) {
			fm.all("prpLlossDtoArrangeRate")[i].value = point(round(ArrangeRate_A, 0), 0);
		}
	}
	calRealpay(field);
...
function getIndemnityDutyRate(field) {
	...
	for (i = 1; i < fm.all("prpLlossDtoIndemnityDutyRate").length; i++) {
		if (fm.all("prpLlossDtoKindCode")[i].value == prpLlossDtoKindCode_A) {
			fm.all("prpLlossDtoIndemnityDutyRate")[i].value = point(round(IndemnityDutyRate_A, 0), 0);
		}
	}
	calRealpay(field);
}
...
function calRealpayForDuBang(field) {
	...
	try {
		...
		if (fm.all("prpLlossDtoKindCode")[findex].value == "G") {
			fm.all("prpLlossDtoSumRealPay")[findex].value = point(round(fm.all("prpLlossDtoSumDefPay")[findex].value, 0), 0);
		}
...
function calRealpayForDuBangAll() {
	...
	for (i = 1; i < fm.all("prpLlossDtoSumLoss").length; i++) {
		...
		//============个别附加险要获得主险的绝对免赔率
		if (kindCode == RISKINFO.KINDCODE_D_D2 ||
			kindCode == "D3" ||
			kindCode == "D4" ||
			kindCode == "Y" ||
			kindCode == "H") {
		...
		//reason:实赔处理，车上货物责任险、无过失责任险超限时系统的赔付计算不对
		if ((kindCode == RISKINFO.KINDCODE_D_B || kindCode == RISKINFO.KINDCODE_D_D2 || kindCode == "W") && temp > Amount) {
			if (Amount > 0) {
				Realpay = Amount * (1 - (DutyDeductibleRate + DeductibleRate));
			}
		}
		...
		//reason:险别是车损或三者，並且损失明细为27扣免赔，则赔偿金额=-残值
		if ((kindCode == RISKINFO.KINDCODE_D_A || kindCode == RISKINFO.KINDCODE_D_B) && fm.all("prpLlossDtoFeeTypeCode")[findex].value == "27") {
			Realpay = SumRest * (-1);
		}
		fm.all("prpLlossDtoSumRealPay")[findex].value = point(round(Realpay, 0), 0);
	}
...
function checkInputPower(field) {
	...
	if (fm.all("prpLlossDtoKindCode")[findex].value == RISKINFO.KINDCODE_D_B && fm.all("prpLlossDtoFeeTypeCode")[findex].value != "27") {
		alert(i18n.compel.risksNotAlloweEntryValue); //B险别不允许輸入残值！
...
function checkQuotaByKindCode() {
	...
	for (var i = 0; i < fm.all("kindCode").length; i++) {
		...
		if (kindCode == 'B' && (sumRealPay > KindAmount)) { //目前只考虑B
			alert(i18n.compensate.thirdPartyInsuranceLimit + KindAmount + i18n.compensate.adjustReparationsReport); //第三者责任险赔偿金额超过了赔偿限额(     元),请调整核定赔款，再重新生成理算报告
			return false;
		} else { //其他险别
...
function checkIndemnityDutyRate() {
	...
	if (compensateIndemnityDuty == '0' || compensateIndemnityDuty == '2') {
		if (parseInt(fm.prpLcompensateIndemnityDutyRate.value) != 100 || parseInt(fm.prpLcompensateIndemnityDutyRate.value) != 50) {
			alert(i18n.compensate.accidentResponsibCannotChange); //事故责任为全责或同责时，责任比例不可修改！
			return false;
		}
	} else {
		if (compensateIndemnityDuty == '1') {
			if (parseInt(fm.prpLcompensateIndemnityDutyRate.value) > 99 || parseInt(fm.prpLcompensateIndemnityDutyRate.value) < 51) {
				...
				alert(i18n.compensate.accidentPriorityDuty99); //事故责任为主责时，责任比例不能大於99%、小於51%
				return false;
			}
		}
		if (compensateIndemnityDuty == '3') {
			if (parseInt(fm.prpLcompensateIndemnityDutyRate.value) > 51 || parseInt(fm.prpLcompensateIndemnityDutyRate.value) < 1) {
				...
				alert(i18n.compensate.accidentPriorityDuty51); //事故责任为主责时，责任比例不能大於51%、小於1%
				return false;
			}
		}
		if (compensateIndemnityDuty == '9' || compensateIndemnityDuty == '4') {
			fm.prpLcompensateIndemnityDutyRate.value = "0";
			alert(i18n.compensate.responsibilityAccidentModified); //事故责任为无责或其他时，责任比例不可修改！
			return false;
		}
...
function initExceptDeductible() {
	...
	if (fm.exceptDeductibleKindCode) {
			...
			for (var index = 0; index < fm.exceptDeductibleKindCode.length; index++) {
				if (fm.exceptDeductibleKindCode[index].value != "")
					if (fm.prpLlossDtoKindCode[index1].value == fm.exceptDeductibleKindCode[index].value) {
						...
						fm.exceptDeductiblePay[index].value = point(round(exceptDeductiblePay, 0), 0);
		...
		fm.exceptDeductibleRateAll.value = point(round(exceptDeductibleRateAll, 0), 0);
	}
}
...
//add by lidonghui 2007-05-19用於在不计免赔栏中添加数据
...
function insertRow2(field) {
	...
	if (fieldname == "prpLlossDtoKindCode" || fieldname == "prpLlossDtoKindName")
				...
				if (fm.prpLlossDtoKindCodeShow[index].value != "")
					if (fm.prpLlossDtoKindCodeShow[index].value == fm.prpLlossDtoKindCode[index1].value) {
						if ((fm.prpLlossDtoKindCode[index1].value == 'Y' || fm.prpLlossDtoKindCode[index1].value == 'S') && fm.licenseNo[index1].value == fm.prpLcompensateLicenseNo.value) {} else {
							sumDefPayAllShow = parseInt(fm.prpLsumDefPayAllShow[index].value);
							sumDefPayAllShow += parseInt(fm.prpLlossDtoSumDefPay[index1].value);
							fm.prpLsumDefPayAllShow[index].value = sumDefPayAllShow;
							sumDefPayAllShowMiddle = parseInt(fm.prpLsumDefPayAllShowMiddle[index].value);
							sumDefPayAllShowMiddle += parseInt(fm.prpLlossDtoSumRest[index1].value);
							sumDefPayAllShowMiddle += parseInt(fm.prpLlossDtoCompelPay[index1].value);
...
function calRealValuen() {
	...
	fm.factValue.value = point(round(factValue, 0), 0);
}
//用於控制A险的输入及B险的输入
...
function inputControl(field) {
	if (field.value != "") {
		var fieldname = field.name;
		var licenseNo = fm.lossItemName.value;
		var flagNameKind = 0;
		var findex;
		var flag = 0;
		var flagG = 0;
		var riskCode = fm.riskCode.value;
		var count2 = getElementCount("prpLlossDtoKindCode");
		for (var i = 1; i < fm.all(fieldname).length; i++) {
			if (fm.all(fieldname)[i] == field) {
				findex = i;
				break;
		...
		for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
			if (fm.prpLlossDtoKindCode[index].value == 'G' || fm.prpLlossDtoKindCode[index].value == 'D11' || fm.prpLlossDtoKindCode[index].value == 'D12') {
				flagG++;
			}
		}
		if (flagG >= 1) {
			alert(i18n.quickCase.simpleClaimNotAllowProcess); //简易赔案不允许对此险别进行处理！
			if (field.value == 'G' || field.name == 'prpLlossDtoKindName') {
				fm.prpLlossDtoKindCode[findex].value = "";
				fm.prpLlossDtoKindName[findex].value = "";
			} else if (count2 > 2) {
				fm.prpLlossDtoKindCode[findex].value = "";
				fm.prpLlossDtoKindName[findex].value = "";
			} else {
				fm.prpLpersonLossKindCode[findex].value = "";
				fm.prpLpersonLossKindName[findex].value = "";
			}
			field.focus;
			return false;
		}
		//控制G险的輸入 end
		if (field.name == 'prpLlossDtoKindName' || field.name == 'prpLlossDtoKindCode') {
			field = fm.prpLlossDtoKindCode[findex];
			flagNameKind = 1;
		}

		if (findex != 'undefined') {
			if (flagNameKind == 1) //如果是对险别控制的情况标的 start
			{
				if (field.value == 'A' || field.value == 'F' || field.value == 'NX' || field.value == 'NY' || ((field.value == 'K2' || field.value == 'K1') && riskCode == '0502') || field.value == 'Z' || field.value == 'E') //控制与车损互斥的险别的輸入
				{
					for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
						if (fm.prpLlossDtoKindCode[index].value == 'A' || fm.prpLlossDtoKindCode[index].value == 'F' || fm.prpLlossDtoKindCode[index].value == 'E' || fm.prpLlossDtoKindCode[index].value == 'NX' || fm.prpLlossDtoKindCode[index].value == 'NY' || fm.prpLlossDtoKindCode[index].value == 'K2' || fm.prpLlossDtoKindCode[index].value == 'K1' || fm.prpLlossDtoKindCode[index].value == 'Z') {
							flag++;
						}
					}
					if (parseInt(flag) > 1) {
						alert(i18n.compensate.risksMutexExisEntry); //已经有该险别或与该险别互斥的险别存在，该险别不能輸入！
						fm.prpLlossDtoKindCode[findex].value = "";
						fm.prpLlossDtoKindName[findex].value = "";
						field.focus;
						return false;
					}
				} else if (field.value == 'C5') //异地出险住宿费特约条款輸入处理
				{
					for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
						if (fm.prpLlossDtoKindCode[index].value == 'C5') {
							flag++;
						}
					}
					if (parseInt(flag) > 1) {
					...
					fm.prpLlossDtoDeductibleRate[findex].readOnly = true;
					fm.prpLlossDtoDutyDeductibleRate[findex].readOnly = true;
					fm.prpLlossDtoCompelPay[findex].readOnly = true;
					fm.prpLlossDtoSumRest[findex].readOnly = true;
					fm.prpLlossDtoSumDefPay[findex].readOnly = true;
					fm.prpLlossDtoSumLoss[findex].value = "";
					fm.prpLlossDtoSumLoss[findex].style.backgroundColor = "#ecfff9";
				} else if (field.value == 'X1') //发动机特约条款
				{
					for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
						if (fm.prpLlossDtoKindCode[index].value == 'X1') {
							flag++;
						}
					}
					if (parseInt(flag) > 1) {
				...
				} else if (field.value == 'C6') {
					for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
						if (fm.prpLlossDtoKindCode[index].value == 'C6') {
							flag++;
						}
					}
					if (parseInt(flag) > 1) {
				...
				} else if (field.value == 'V1') {
					for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
						if (fm.prpLlossDtoKindCode[index].value == 'V1') {
							flag++;
						}
					}
					if (parseInt(flag) > 1) {
				...
				} else if (field.value == 'T') //车辆停驶险
				{
					for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
						if (fm.prpLlossDtoKindCode[index].value == 'T') {
							flag++;
						}
					}
					if (parseInt(flag) > 1) {
			...
			} //if(flagNameKind==2) end 
			else //如果是对车牌控制的情况 start
			{
				if (fm.prpLlossDtoKindCode[findex].value == 'Y' || fm.prpLlossDtoKindCode[findex].value == 'S') //教练车特约、附加出境特约
				{
					for (var i = 1; i < fm.prpLlossDtoKindCode.length; i++) {
						if (fm.prpLlossDtoKindCode[i].value == 'A') {
							if (fm.licenseNo[i].value == field.value) {
								alert(i18n.compensate.coverageMutexes); //该险别与车损险互斥！
								fm.prpLlossDtoKindCode[findex].value = "";
								fm.prpLlossDtoKindName[findex].value = "";
								fm.licenseNo[findex].value = "";
								return false;
							}
						} else if (fm.prpLlossDtoKindCode[i].value == 'B') {
							if (fm.licenseNo[i].value == field.value) {
								alert(i18n.compensate.risksMutexes3); //该险别与第三者责任险互斥！
								fm.prpLlossDtoKindCode[findex].value = "";
								fm.prpLlossDtoKindName[findex].value = "";
								fm.licenseNo[findex].value = "";
								return false;
				...
				} else if (fm.prpLlossDtoKindCode[findex].value == 'X' || fm.prpLlossDtoKindCode[findex].value == 'X1' || (riskCode == '0502' && (fm.prpLlossDtoKindCode[findex].value == 'K1' || fm.prpLlossDtoKindCode[findex].value == 'K2')) || fm.prpLlossDtoKindCode[findex].value == 'E' || fm.prpLlossDtoKindCode[findex].value == 'Z') //新增设备损失险
				{
					if (field.value != licenseNo) {
						alert(i18n.compensate.riskInputTargetCar); //该险别只能輸入标的车！
						fm.licenseNo[findex].value = "";
						return false;
					}
				} else if (fm.prpLlossDtoKindCode[findex].value == 'C7') //节假日特约条款
				{
					var kindcode;
					if (fm.licenseNo[findex].value == licenseNo)
						kindcode = 'A';
					else
						kindcode = 'B';
					for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
						if (fm.prpLlossDtoKindCode[index].value == kindcode && fm.licenseNo[index].value == fm.licenseNo[findex].value) {
							flag++;
						}
					}
					if (parseInt(flag) == 0) {
						alert(i18n.compensate.noVehiclesRisksExist); //没有相应车辆或险别存在，不能輸入该险别！
						fm.prpLlossDtoKindCode[findex].value = "";
						fm.prpLlossDtoKindName[findex].value = "";
						fm.licenseNo[findex].value = "";
						fm.prpLlossDtoKindCode[findex].focus;
						return false;
					}
					calC7(findex, kindcode);
				} else if (fm.prpLlossDtoKindCode[findex].value == 'D2') //车上货物责任险
				{
					if (fm.licenseNo[findex].value == licenseNo) {
						alert(i18n.compensate.carTruckCargoInput); //车上货物责任险只能輸入三者车，不能輸入标的车！
						fm.prpLlossDtoKindCode[findex].value = "";
						fm.prpLlossDtoKindName[findex].value = "";
						fm.licenseNo[findex].value = "";
						fm.prpLlossDtoKindCode[findex].focus;
		...
		} //end for if findex!='undefined'
	} //end for if field.value!=""
}
			...
			if (fm.prpLlossDtoKindCode[index].value == kindCode && fm.prpLlossDtoSumDefPay[index].value != "") {
				if ((kindCode == 'Y' || kindCode == 'S') && fm.licenseNo[index].value == fm.prpLcompensateLicenseNo.value) {} else {
					...
					if (parseInt(fm.prpLlossDtoSumDefPay[index].value) != 0) {
						...
						fm.prpLlossDtoSumRealPay[index].value = point(round(((sumDefPay - sumRest - compelPay) / realPayAll) * amount1, 0), 0);
					}
					if (parseInt(fm.prpLlossDtoExceptDeductibleRate[index].value) != 0) {
						...
						fm.prpLlossDtoExceptDeductiblePay[index].value = point(round(exceptDeductiblePay1, 0), 0);
						initExceptDeductible();
					}
				}
			}
...
function checkAmountn() {
	...
	for (var index = 0; index < fm.prpLlossDtoKindCodeShow.length; index++) {
		if (((parseInt(fm.prpLsumDefPayAllShow[index].value) - parseInt(fm.prpLsumDefPayAllShowMiddle[index].value)) * parseInt(dutyRate) / 100) > point(round(fm.kindAmount[index].value, 0), 0)) {
			if (fm.prpLlossDtoKindCodeShow[index].value == 'B') //三者险超限提示並处理
			{
				...
				distributeAmount(parseInt(point(round(fm.kindAmount[index].value, 0), 0)), (parseInt(fm.prpLsumDefPayAllShow[index].value) - parseInt(fm.prpLsumDefPayAllShowMiddle[index].value)), fm.prpLlossDtoKindCodeShow[index].value);
			...
			} else if (fm.prpLlossDtoKindCodeShow[index].value == 'X') //新增加设备损失险别超限提示
			{
				...
				fm.compensateMessage1.value = message;
				// modify by liping 限额不扣减残值与交强险赔款 080606
				doK2(parseInt(point(round(fm.kindAmount[index].value, 0), 0)), (parseInt(fm.prpLsumDefPayAllShow[index].value) - parseInt(fm.prpLsumDefPayAllShowMiddle[index].value)), fm.prpLlossDtoKindCodeShow[index].value);
			} else if (fm.prpLlossDtoKindCodeShow[index].value == 'D11') //车上人员(驾驶员)超限提示並处理
			{
				...
				fm.compensateMessage1.value = message;
				doD11((parseInt(fm.kindAmount[index].value) - parseInt(fm.prpLsumDefPayAllShowMiddle[index].value)), parseFloat(parseInt(fm.prpLsumDefPayAllShow[index].value) - parseInt(fm.prpLsumDefPayAllShowMiddle[index].value)));
			...
			{
				var message = fm.compensateMessage1.value;
				var re = new RegExp("车辆停驶险核定赔偿金额超限，按限额调整！", "i");
				var r = message.match(re);
				if (r == null) {
					message = message + "车辆停驶险核定赔偿金额超限，按限额调整！\n";
				}
				fm.compensateMessage1.value = message;
			} else if ('D2' == fm.prpLlossDtoKindCodeShow[index].value) //车上货物责任险
			{
				var message = fm.compensateMessage1.value;
				var re = new RegExp("车上货物责任险核定赔偿金额超限，将在各条损失之间分摊！", "i");
				var r = message.match(re);
				if (r == null) {
					message = message + "车上货物责任险核定赔偿金额超限，将在各条损失之间分摊！\n";
				}
				fm.compensateMessage1.value = message;
				// modify by liping 限额不扣减残值与交强险赔款 080606
				doK2(parseInt(point(round(fm.kindAmount[index].value, 0), 0)), (parseInt(fm.prpLsumDefPayAllShow[index].value) - parseInt(fm.prpLsumDefPayAllShowMiddle[index].value)), fm.prpLlossDtoKindCodeShow[index].value);
		...
		if (parseFloat(fm.prpLchargeSumRealPayAllShow[index].value) > parseFloat(fm.prpLchargeAmountShow[index].value)) {
			alert(fm.prpLlossDtoKindNameShow[index].value + i18n.compensate.rescueCostOverrun); //的施救费超限，请调整！
			...
			for (var i = 0; i < fm.prpLpersonLossKindCode.length; i++) {
				...
				if (fm.prpLpersonLossKindCode[i].value != "") {
					...
					if (kindCode == fm.prpLlossDtoKindCodeShow[index].value) {
						if (kindCode == 'D12') {
							if ((parseInt(fm.prpLsumDefPayAllShow[index].value) - parseInt(fm.prpLsumDefPayAllShowMiddle[index].value)) > parseInt(fm.unitAmount[index].value)) {
								var message = fm.compensateMessage1.value;
								...
								var amount1 = unitAmount * parseInt(fm.prpLpersonLossIndemnityDutyRate[i].value) / 100 * (1 - parseFloat(fm.prpLpersonLossDutyDeductibleRate[i].value) / 100 - parseFloat(fm.prpLpersonLossDeductibleRate[i].value) / 100);
								...
								for (var i1 = 1; i1 < fm.personLossSerialNo.length; i1++) {
									if (fm.personLossSerialNo[i1].value == fm.prpLpersonLossSerialNo[i].value) {
										...
										fm.prpLpersonLossSumRealPay[i1].value = point(round((sumRealPay / realpay) * amount1 - compelPay, 0), 0);
										if (parseInt(fm.prpLpersonLossExceptDeductibleRate[i1].value) != 0) {
											...
											fm.prpLpersonLossExceptDeductiblePay[i1].value = point(round(exceptDeductiblePay1 * (sumRealPay / realpay) - compelPay, 0), 0);
											initExceptDeductible();
										}
							...
							//modify by liping 080605 事故责任下和限额进行比较
		...
		for (var index = 1; index < fm.prpLpersonLossKindCode.length; index++) {
			if (fm.prpLpersonLossKindCode[index].value == 'D11' && fm.prpLpersonLossSumDefPay1[index].value != "") {
				if (parseInt(fm.prpLpersonLossSumDefPay1[index].value) != 0) {
					...
					var amount1 = parseFloat(amount * dutyRate / 100 * (1 - dutyDeductibleRate / 100 - deductibleRate / 100));
					for (var i = 1; i < fm.personLossSerialNo.length; i++) {
						if (fm.personLossSerialNo[i].value == fm.prpLpersonLossSerialNo[index].value) {
							...
							if (parseInt(fm.prpLpersonLossSumDefPay[i].value) != 0) {
								...
								fm.prpLpersonLossSumRealPay[i].value = point(round(((sumDefPay - compelPay) / realpay) * amount1, 0), 0);
							}
							if (parseInt(fm.prpLpersonLossExceptDeductibleRate[i].value) != 0) {
								...
								fm.prpLpersonLossExceptDeductiblePay[i].value = point(round((sumDefPay - compelPay) / realpay * exceptDeductiblePay, 0), 0);
								initExceptDeductible();
							}
						}
					}
...
function doF(amount) {
	for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
		if ('F' == fm.prpLlossDtoKindCode[index].value) {
			fm.prpLlossDtoSumRealPay[index].value = point(round(amount, 0), 0);
		}
	}
	//计算责任赔款合计、赔款合计、其它费用、实赔金额
	calFund();
...
function inputControl2(field) {
	...
	if (field.name == 'prpLlossDtoSumDefPay') {
		if (fm.prpLlossDtoSumLoss[findex].value == "") {
			alert(i18n.compensate.lossAmountNoEntry); //损失金额没值，不能輸入！
			field.value = "";
			field.focus;
			return false;
		}
		if (parseFloat(fm.prpLlossDtoSumDefPay[findex].value) > parseFloat(fm.prpLlossDtoSumLoss[findex].value)) {
			alert(i18n.compensate.compensationAountNotLoss); //核定赔偿金额不能超过损失金额
			field.value = "";
			field.focus;
			return false;
		}
...
function doK2(amount, sumdefPay1, kindCode) {
	for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
		if (fm.prpLlossDtoKindCode[index].value == kindCode && fm.prpLlossDtoSumDefPay[index].value != "") {
			...
			var dutyDeductibleRate = parseInt(fm.prpLlossDtoDutyDeductibleRate[index].value);
			var deductibleRate = parseInt(fm.prpLlossDtoDeductibleRate[index].value);
			var amount1 = parseFloat(amount * dutyRate / 100 * claimRate / 100 * (1 - dutyDeductibleRate / 100 - deductibleRate / 100));
			if (parseInt(fm.prpLlossDtoSumDefPay[index].value) != 0) {
				var sumDefPay = parseFloat(fm.prpLlossDtoSumDefPay[index].value);
				var sumRest = parseFloat(fm.prpLlossDtoSumRest[index].value);
				var compelPay = parseFloat(fm.prpLlossDtoCompelPay[index].value);
				fm.prpLlossDtoSumRealPay[index].value = point(round(((sumDefPay - sumRest - compelPay) / sumdefPay1) * amount1, 0), 0);
			}
			if (parseInt(fm.prpLlossDtoExceptDeductibleRate[index].value) != 0) {
				var exceptDeductibleRate = parseFloat(fm.prpLlossDtoExceptDeductibleRate[index].value);
...
function calC7(index1, kindCode) {
	for (var index = 1; index < fm.prpLlossDtoKindCode.length; index++) {
		if (fm.prpLlossDtoKindCode[index].value == kindCode && fm.licenseNo[index].value == fm.licenseNo[index1].value) {
			var result = parseFloat(fm.prpLlossDtoSumRealPay[index].value) / (1 - (parseInt(fm.prpLlossDtoDeductibleRate[index].value) + parseInt(fm.prpLlossDtoDutyDeductibleRate[index].value)) / 100) * 0.1;
			fm.prpLlossDtoSumLoss[index1].value = point(round(result, 0), 0);
			fm.prpLlossDtoSumDefPay[index1].value = point(round(result, 0), 0);
			fm.prpLlossDtoSumRealPay[index1].value = point(round(result, 0), 0);
		}
	}
}
...
function checkBeyondQuota(field) {
	...
	var configCode = fm.prpLRegistRPolicyNo.value;
	var quotaBZ_D = 0;
	var quotaBZ_M = 0;
	var quotaBZ_P = 0;
	...
	if ('' != configCode) {
		...
		for (i = 1; i < fm.all("prpLlossDtoSumDefPay").length; i++) {
			if (fm.all("prpLlossDtoKindCode")[i].value == ConstantCodes.KINDCODE_D_BZ) {
				propSumDefPay = parseFloat(fm.prpLlossDtoSumDefPay[i].value);
				nulllicenseNo = trim(fm.licenseNo[i].value);
				if (licenseNo == nulllicenseNo) {
		...
		for (i = 1; i < fm.all("prpLchargeKindCode").length; i++) {
			if (fm.all("prpLchargeKindCode")[i].value == ConstantCodes.KINDCODE_D_BZ && fm.all("prpLchargeChargeCode")[i].value == '03') {
				rescuFee = parseFloat(fm.all("prpLchargeChargeReport")[i].value);
				if (isNaN(rescuFee)) rescuFee = 0;
				propfee += rescuFee;
			}
		}
		//add by zhangyurui 2008-12-4 将无责车辆数进行统计，决定总赔付金额（限额+无责代赔分摊）begin
		var licenseNos = document.getElementsByName("checkPrpLthirdPartyDtoLicenseNo");
		var carDutys = document.getElementsByName("checkPrpLthirdPartyDtoDutyPercent");
		var noDutyCount = 0; //无责车数量
		var noDutyFee = 0;
		for (var i = 0; i < carDutys.length; i++) {
			if (carDutys[i].value < 0.5) {
				noDutyCount++;
			}
		}
		if (licenseNos.length - noDutyCount != 0) {
			noDutyFee = (noDutyCount - 1) * 100 / (licenseNos.length - noDutyCount);
		} else {
			noDutyFee = 0; //全无责（标的无责）属不正常情况，在这里不作处理
		}
		//无责代赔
		var licenseNo = "";
		var noDutyLicnseFee = 0.0;
		for (var i = 0; i < fm.all("checkPrpLthirdPartyDtoLicenseNo").length; i++) {
			if (trim(fm.all("checkPrpLthirdPartyDtoInsureCarFlag")[i].value) == '1') {
				licenseNo = trim(fm.all("checkPrpLthirdPartyDtoLicenseNo")[i].value);
			}
		}
		for (var i = 0; i < fm.all("licenseNo").length; i++) {
			if (licenseNo == fm.all("licenseNo")[i].value) {
				noDutyLicnseFee = parseFloat(fm.all("prpLlossDtoSumDefPay")[i].value) + parseFloat(noDutyLicnseFee);
			}
		}
		if (noDutyLicnseFee > noDutyFee) {
			alert(i18n.quickCase.withoutGeneratAmountCase + noDutyFee + i18n.quickCase.yuanCarInsuranceAmount); //这个案件无责代赔金额为      元，交强险赔本车不能超过无责代赔金额
			field.value = 0.0;
			return false;
		}
		propfee = parseFloat(propfee) - parseFloat(noDutyLicnseFee);
		//add by zhangyurui 2008-12-4 将无责车辆数进行统计，决定总赔付金额（限额+无责代培分摊）end
		if (propfee > quotaBZ_P) {
			alert(i18n.compel.propertyVerificatLimit + quotaBZ_P + i18n.compel.yuan); //  财产核定赔偿超过了财产限额（          元）
			field.value = 0.0;
			return false;
		}
	}
...
function checkBeyondQuotaLienceNo(field) {
	var i = 0;
	var duty = fm.indemnityDuty.value; //责任
	var configCode = fm.prpLRegistRPolicyNo.value;
	var quotaBZ_D = 0;
...
function changeAdvanceStatus(field) {
	if (field.value == '1') //全责垫付
	{
		...
		fm.displayGetFromPlatForm.style.display = "none";
	...
	{
		fm.displayGetFromPlatForm.style.display = "";
...
function clearPrpLctext() {
	...
	var compelContext = document.getElementsByName('compelPrpLctextContextInnerHTML');
	if (Context.length > 0) {
		Context[0].value = '';
	}
	if (compelContext.length > 0) {
		compelContext[0].value = '';
	}
}
...
function getPayObject(field) {
	...
	if (chargeCodeList[index] == null || chargeCodeList[index].value == "") {
		alert(i18n.compensate.pleaseSelectName); //请选择费用名称
		return;
	} else {
...
function getExternAlagency(field, index) {
	...
	if ((ChargeCode == "04" || ChargeCode == "05" || ChargeCode == "06" || ChargeCode == "13") && PayObjectType == "B" && PayObjectCode != "") {
		var inputArgs = {
			comCode: PayObjectCode
		};
		DWRUtil.setValues(inputArgs);
		...
		document.all["bank"][index].style.display = "none";
		...
		fm.buttonAddAcc[index].disabled = true;
	}
}
...
function rollbackExternAlagency(inputObject, outputObject, returnObject) {
	...
	fm.prpLchargeOwnerShip[findex].options[1].selected = true;

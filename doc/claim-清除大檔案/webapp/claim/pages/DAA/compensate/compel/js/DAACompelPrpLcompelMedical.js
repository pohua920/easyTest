...
function savePrpLcompelMedical(actionType,status){
	...
	if (actionType == "ADD" || actionType == "EDIT") {// 強制險理算計算書處理
		...
		if(sumFeeA024>FeeA024){
			errorMessage += "費用A024上限為5萬元\n";
		}
		if(sumFeeA026>FeeA026){
			errorMessage += "費用A026上限為2萬元\n";
		}
		if(sumFeeA03>FeeA03){
			errorMessage += "費用A03上限為2萬元\n";
		}
		if(sumFeeA04>FeeA04){
			errorMessage += "費用A04上限為3萬元6千元\n";
		}
		if(sumFeeA>FeeA00){
			errorMessage += "費用A00上限為20萬元\n";
		}

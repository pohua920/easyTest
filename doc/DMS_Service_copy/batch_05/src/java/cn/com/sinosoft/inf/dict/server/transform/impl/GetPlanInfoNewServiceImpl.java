/**
 * 从 GetPlanInfoServiceImpl 中拷贝 新增 prpdrationPeriodrate 保费/费率区间表 、prpdChannelinfo --个性信息表 等 6张表
 * @author wpf
 * @date 2012-3-29
 */
...
import cn.com.sinosoft.inf.dict.xmlmsg.getPlanInfo.GetPlanInfoNewReqPacket;
...
public class GetPlanInfoNewServiceImpl implements
		DataTransformer<GetPlanInfoNewReqPacket, GetPlanInfoResPacket> {

	public String execute(String requestxml) throws Exception {
		GetPlanInfoResPacket resPacket = new GetPlanInfoResPacket();
		GetPlanInfoNewReqPacket requestPacket = xmlToSchema(requestxml);
		...
		String startDate = requestPacket.getBODY().getStartDate();
		String startHour = requestPacket.getBODY().getStartHour();
		String endDate = requestPacket.getBODY().getEndDate();
		String endHour = requestPacket.getBODY().getEndHour();
		String policyWayCode = requestPacket.getBODY().getPolicyWayCode();
		
		RationObj rationObj = dictionaryService.getRationInfoNew(systemCode, prpDration,agentCode,comCode,startDate,startHour,endDate,endHour,policyWayCode);
	...
	public GetPlanInfoNewReqPacket xmlToSchema(String requestxml) throws Exception {
		if (requestxml == null || "".equals(requestxml)) {
			throw new Exception("报文不可以为空");
		}
		XStream xs = new XStream();
		xs.alias("GetPlanInfoNewReqPacket", GetPlanInfoNewReqPacket.class);
		...
		GetPlanInfoNewReqPacket ep = (GetPlanInfoNewReqPacket) xs.fromXML(requestxml);
		return ep;
	}

...
public class PrpDbankServiceSpringImpl extends GenericDaoHibernate<PrpDbank, String> implements PrpDbankService {
    ...
    public Page getPrpDbankList(PrpDbank prpDbank, String userCode,int pageNo, int pageSize) throws Exception{
        ...
        hql.append(" from PrpDbank prpDbank where ");
        hql.append(con);
        HqlRulesUtil hqlRules = new HqlRulesUtil();
        hqlRules.addLike("bankCode", prpDbank.getBankCode());
        hqlRules.addLike("bankName", prpDbank.getBankName());
    ...
    public PrpDbank findByPrimaryKey(String strBankCode){
        PrpDbank prpDbank = this.get(strBankCode);
        return prpDbank;
    }
    public PrpDbank findByPrimaryKey1(String strBankCode){
    	String hql="from PrpDbank prpDbank where prpDbank.bankCode=? and validStatus=1";
    	
        List list = new ArrayList();
    	list = this.findByHql(hql, strBankCode);
    	if(list.size()!=0){
    		return (PrpDbank) list.get(0);
    	}else{
    		return null;
    	}
    }
    	...
    	if(syncflag.equals(SyncConstants.SyncFlag)){
            ...
            .getService("checkSameKeyService");// 获得Spring管理的bean
	   		 ...
	   		 for (String comCode : strOnlineCom) {	
	   			 ...
	   			 utiISyncLog.setClassName(SyncConstants.RequestFlag_PrpDbankMaintain);
	   			 ...
	   			 utiISyncLog.setStrKey("bankCode = '" + prpDbank.getBankCode() + "'");
	...
	public  String addPower(String userCode) throws Exception{
		...
		String condition =  saaAPIService.addPower(IConstants.SVRCODE, userCode, IConstants.SEARCH_PRPDBANK_COMCODE, IConstants.PRPDBANK_BM, "", "");
		if(!"".equals(condition))
			return condition;
		else
			return " 1 != 1";

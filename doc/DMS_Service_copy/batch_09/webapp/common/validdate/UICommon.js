...
/*
 * add by wangrongjia 20080819
 * 增加新的客户查询方式
 * @param strInsured 类别
 */
 function addInsuredCustomer(strInsured)
{
  var strURL = "/prpall/common/pub/forwardToPlatform.jsp?CurrentCustomer="+strInsured;
  window.open(strURL,'','top=0,left=0,toolbar=0,location=0,directories=0,menubar=0,scrollbars=1,resizable=1,status=0');
}

// add by wangrongjia 20080819 查询客户个人客户返回方法
function addPersonalCustomerCallBack(strCustomerFlag,strCustomerType,strInsuredType,strCustomerCode,strCustomerCName,
									strCustomerKind,strIdentifyType,strIdentifyNumber,strBank,strAccount,strGender,
									strBirthDate,strMarriage,strOccupationCode,strOccupationCodeName,strEducationCode,
									strEducationCodeName,strPostAddress,strPostCode,strPhoneNumber,
									strNationality,strUnit,strUnitAddress,strUnitPostcode,strUnitPhoneNumber){
	if(strCustomerFlag=="AppliInsuredForE"){//投保人
	  	fm.AppliPrpInsuredInsuredCode.value = strCustomerCode;
	  	fm.AppliPrpInsuredInsuredName.value = strCustomerCName;
	  	fm.AppliInsuredType.value			= strInsuredType;
	  	fm.AppliInsuredNature.value			= strCustomerType;
	  	fm.AppliIdentifyType.value 			= strIdentifyType;
	  	fm.AppliIdentifyNumber.value 		= strIdentifyNumber;
	  	fm.AppliBank.value 					= strBank;
	  	fm.AppliAccount.value 				= strAccount;
	  	fm.AppliNSex.value 					= strGender;
	  	fm.AppliNBirthday.value 			= strBirthDate;
	  	fm.AppliNMarriage.value 			= strMarriage;
	  	fm.AppliOccupationCode.value 		= strOccupationCode;
	  	fm.AppliOccupationName.value		= strOccupationCodeName;
	  	fm.AppliNRoomAddress.value 			= strPostAddress;
	  	fm.AppliNRoomPostCode.value 		= strPostCode;
	  	fm.AppliNRoomPhone.value 			= strPhoneNumber;
	  	fm.AppliNLocalPoliceStation.value 	= strNationality;
	  	fm.AppliNUnit.value					= strUnit;
	  	fm.AppliNUnitAddress.value			= strUnitAddress;
	  	fm.AppliNUnitPostCode.value			= strUnitPostcode;
	  	fm.AppliNUnitPhoneNumber.value		= strUnitPhoneNumber;

	  	changeAppliInsuredNature();
	  	checkAppliIdentifyNumber(fm.AppliIdentifyNumber);
  	} else if (strCustomerFlag=="InsuredInsuredForE") { //被保险人
  		fm.PrpInsuredInsuredCode.value 	= strCustomerCode;
	  	fm.PrpInsuredInsuredName.value 	= strCustomerCName;
	  	fm.InsuredType.value			= strInsuredType;
	  	fm.InsuredNature.value			= strCustomerType;
	  	fm.IdentifyType.value 			= strIdentifyType;
	  	fm.IdentifyNumber.value 		= strIdentifyNumber;
	  	fm.Bank.value 					= strBank;
	  	fm.Account.value 				= strAccount;
	  	fm.NSex.value 					= strGender;
	  	fm.NBirthday.value 				= strBirthDate;
	  	fm.NMarriage.value 				= strMarriage;
	  	fm.OccupationCode.value 		= strOccupationCode;
		fm.OccupationName.value			= strOccupationCodeName;
	  	fm.RoomAddress.value 			= strPostAddress;
	  	fm.RoomPostCode.value 			= strPostCode;
	  	fm.RoomPhone.value 				= strPhoneNumber;
	  	fm.LocalPoliceStation.value 	= strNationality;
	  	fm.Unit.value					= strUnit;
	  	fm.UnitAddress.value			= strUnitAddress;
	  	fm.UnitPostCode.value			= strUnitPostcode;
	  	fm.UnitPhoneNumber.value		= strUnitPhoneNumber;
	  	
	  	changeInsuredNature();
	  	checkIdentifyNumber(fm.IdentifyNumber);
	  	//added by MinYingqi 20080725 选择职业风险类别。
	  	if(fm.OccupationName.value != null && trim(fm.OccupationName.value) != ''){
	  	   code_CodeChange(fm.OccupationName);
	  	}
  	} else if (strCustomerFlag=="BenefInsuredForE") { //受益人
  		try
  		{
	      var count = 0;
	      insertRowBenefInsured();
	      count = fm.BenefPrpInsuredInsuredCode.length;
	  //add by tianzejun,20080709,添加自动生成受益顺序,begin
      fm.BenefBenefitFlag[count-1].value=count-1;
      //add end.
	      fm.BenefPrpInsuredInsuredCode[count-1].value 	= strCustomerCode;
  		  fm.BenefPrpInsuredInsuredName[count-1].value 	= strCustomerCName;
  		  fm.BenefInsuredNature[count-1].value			= strCustomerType;
  		  fm.BenefNSex[count-1].value					= strGender;
  		  fm.BenefIdentifyType[count-1].value			= strIdentifyType;
  		  fm.BenefIdentifyNumber[count-1].value 		= strIdentifyNumber;
		  fm.BenefInsuredNature[count-1].value         	= strCustomerType;
		  
	    } catch(E){}
	    checkBenefIdentifyNumber(fm.BenefIdentifyNumber[count-1]);
  	} else if (strCustomerFlag=="AppliInsured") {
  		fm.AppliCode.value 	= strCustomerCode;
	  	fm.AppliName.value 	= strCustomerCName;
	  	//fm.AppliInsuredInsuredType.value	= strInsuredType;
	  	//fm.AppliInsuredInsuredNature.value	= strCustomerType;
		//fm.AppliInsuredIdentifyNumber.value = strIdentifyNumber;
		fm.AppliInsuredBank.value 			= strBank;
		fm.AppliInsuredAccount.value 		= strAccount;
	  	//fm.AppliInsuredPostAddress.value 	= strPostAddress;
		fm.AppliPostCode.value 		= strPostCode;
		fm.AppliPhoneNumber.value 	= strPhoneNumber;

		//changeAppliInsuredInsuredType();
	  	//setMainAppliInsuredValue();
  	} else if (strCustomerFlag=="InsuredInsured") {
  		fm.InsuredCode.value			= strCustomerCode;
  		fm.InsuredName.value			= strCustomerCName;
  		//fm.InsuredInsuredType.value			= strInsuredType;
  		fm.IdentifyNumber.value		= strIdentifyNumber;
  		//fm.InsuredInsuredNature.value		= strCustomerType;
  		//fm.InsuredPostAddress.value			= strPostAddress;
  		fm.InsuredBank.value				= strBank;
  		fm.InsuredAccount.value				= strAccount;
  		fm.InsuredPostCode.value			= strPostCode;
  		fm.InsuredPhoneNumber.value			= strPhoneNumber;
  		
	  	//changeInsuredInsuredType();
  		//setMainInsuredValue();
  	}

}

// add by xieguorun 20080627 查询客户团体客户返回方法
function addGroupCustomerCallBack(strCustomerFlag,strCustomerType,strInsuredType,strCustomerCode,strCustomerCName,
									strCustomerKind,strOrganizeCode,strBusinessSource,strBusinessSourceName,
									strBusinessSort,strBusinessSortName,strPhoneNumber,strPostcode,strLinkerName,
									strLinkerPhoneNumber,strMobile,strEMail){
	if(strCustomerFlag=="AppliInsuredForE") {
		fm.AppliPrpInsuredInsuredCode.value = strCustomerCode;
	  	fm.AppliPrpInsuredInsuredName.value = strCustomerCName;
	  	fm.AppliInsuredType.value			= strInsuredType;
	  	fm.AppliInsuredNature.value			= strCustomerType;
	  	fm.AppliIdentifyNumber.value 		= strOrganizeCode;
	  	fm.AppliBusinessSource.value		= strBusinessSource;
	  	fm.AppliBusinessSourceName.value 	= strBusinessSourceName;
	  	fm.AppliBusinessSort.value			= strBusinessSort;
	  	fm.AppliBusinessSortName.value		= strBusinessSortName;
	  	fm.AppliAPhoneNumber.value			= strPhoneNumber;
	  	fm.AppliAPostCode.value				= strPostcode;
	  	fm.AppliLinkerName.value			= strLinkerName;
	  	fm.AppliPhoneNumber.value			= strLinkerPhoneNumber;
	  	fm.AppliMobile.value				= strMobile;
	  	fm.AppliEmail.value					= strEMail;
	  	...
	  	fm.InsuredType.value			= strInsuredType;
	  	fm.InsuredNature.value			= strCustomerType;
	  	fm.IdentifyNumber.value 		= strOrganizeCode;
	  	fm.BusinessSource.value			= strBusinessSource;
	  	fm.BusinessSourceName.value		= strBusinessSourceName;
	  	fm.BusinessSort.value			= strBusinessSort;
		fm.BusinessSortName.value		= strBusinessSortName;
	  	fm.APhoneNumber.value			= strPhoneNumber;
	  	fm.APostCode.value				= strPostcode;
	  	fm.LinkerName.value				= strLinkerName;
	  	fm.PhoneNumber.value			= strLinkerPhoneNumber;
	  	fm.Mobile.value					= strMobile;
	  	...
	  	//fm.AppliInsuredInsuredType.value	= strInsuredType;
	  	//fm.AppliInsuredInsuredNature.value	= strCustomerType;
		//fm.AppliInsuredIdentifyNumber.value = strOrganizeCode;
		fm.AppliEmail.value 			= strEMail;
		fm.AppliLinkerName.value 	= strLinkerName;
		fm.AppliPostCode.value 		= strPostcode;
		fm.AppliPhoneNumber.value 	= strLinkerPhoneNumber;
		...
		//changeAppliInsuredInsuredType();
  		...
  		fm.InsuredName.value			= strCustomerCName;
  		//fm.InsuredInsuredType.value			= strInsuredType;
  		//fm.InsuredIdentifyNumber.value		= strOrganizeCode;
  		//fm.InsuredInsuredNature.value		= strCustomerType;
  		fm.InsuredEmail.value				= strEMail;
  		fm.InsuredMobile.value				= strMobile;
  		fm.InsuredPostCode.value			= strPostcode;
  		fm.InsuredPhoneNumber.value			= strLinkerPhoneNumber;
  		fm.InsuredLinkerName.value			= strLinkerName;
...
// add by xieguorun 20080627 查询客户虚拟客户返回方法
function addInventedCustomerCallBack(strCustomerFlag,strCustomerType,strInsuredType,strCustomerCode,strCustomerCName,strLinkerName,
									strPhoneNumber,strPostAddress,strPostCode){
	if(strCustomerFlag=="AppliInsuredForE") {
		fm.AppliPrpInsuredInsuredCode.value = strCustomerCode;
	  	fm.AppliPrpInsuredInsuredName.value = strCustomerCName;
	  	fm.AppliInsuredType.value			= strInsuredType;
	  	fm.AppliInsuredNature.value			= strCustomerType;
	  	fm.AppliLinkerName.value			= strLinkerName;
	  	fm.AppliPostAddress.value			= strPostAddress;
	  	fm.AppliPostCode.value				= strPostCode;
	  	fm.AppliPhoneNumber.value			= strPhoneNumber;
	  	...
	  	fm.InsuredNature.value			= strCustomerType;
	  	fm.LinkerName.value				= strLinkerName;
	  	fm.PostAddress.value			= strPostAddress;
	  	fm.PostCode.value				= strPostCode;
...
function getShortRate(strMode,strStartDate,strStartHour,strEndDate,strEndHour)
{
  ...
  //update by panyafeng 20080928 begin
  else if(parseInt(strMode,10)==2 )  //日比例
  {
    shortRate = getShortRateDay(strStartDate,strStartHour,strEndDate,strEndHour);
  }
  else
  {
    shortRate = 100;
  ...
  //update by panyafeng 20080928 end
  return shortRate;
}
//按月比例计算短期费率函数
...
{
//modify by chenhuanhuan at 20081016 不批改车辆信息时查询连接不可用 begin
  if(fm.BIZTYPE.value!="ENDORSE" || fm.BIZTYPE.value=="ENDORSE" && fm.EndorseItemCar.value.indexOf("40") > -1)
  {
...
//modify by chenhuanhuan at 20081016 end
}
...
function queryHandler1Code()
{
  ...
  var strURL = "/prpall/common/pub/UIHandler1CodeQueryInput.jsp";
...
function changePolicySort()
{
  try{
    ...
    if(policySort=="11" || policySort=="定额保单" )
    {
    ...
    else  if(policySort=="13" || policySort=="普通保单" )
    {

...
import cn.com.sinosoft.inf.PMS.reqDomains.sendPrpDitemIdReqPacket.SendPrpDitemIdReqPacket;
...
public class TransFindPrpDitemById implements
		DataTransformer<SendPrpDitemIdReqPacket, PrpDitemListResPacket> {

	public String execute(String requestxml) throws Exception {
		List<PrpDitem> list = new ArrayList<PrpDitem>();
		String requestType = "";
		/** 请求的xml报文翻译成Packet对象 */
		SendPrpDitemIdReqPacket requestPacket = xmlToSchema(requestxml);
		/*************************************
		 * 调用持久层操作，获得所需数据
		 * **********************************/
		...
		list = dictionaryService.findPrpDitemById(requestPacket.getHEAD()
				.getSYSTEMCODE(), requestPacket.getBODY().getITEMCODE());
		
		//TODO 在ServiceInfoConst中添加对应接口的代码（FINDPRPDCLASSBYID）
		requestType = ServiceInfoConst.FINDPRPDITEMBYID;// 设置返回报文的requesttype
	...
	public SendPrpDitemIdReqPacket xmlToSchema(String requestxml)
			throws Exception {
		JOXBeanInputStream joxIn = new JOXBeanInputStream(
				new ByteArrayInputStream(requestxml.getBytes(IConstants.ENCODING_UTF8)));
		SendPrpDitemIdReqPacket response = (SendPrpDitemIdReqPacket) joxIn
				.readObject(SendPrpDitemIdReqPacket.class);
		return response;
	}
}

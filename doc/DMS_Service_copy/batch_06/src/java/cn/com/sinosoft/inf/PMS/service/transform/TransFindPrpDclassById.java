...
public class TransFindPrpDclassById implements
		DataTransformer<SendPrpDclassIdReqPacket, PrpDclassListResPacket> {

	public String execute(String requestxml) throws Exception {
		List<PrpDclass> list = new ArrayList<PrpDclass>();
		String requestType = "";
		/** 请求的xml报文翻译成Packet对象 */
		SendPrpDclassIdReqPacket requestPacket = xmlToSchema(requestxml);
		/*************************************
		 * 调用持久层操作，获得所需数据
		 * **********************************/
		...
		list = dictionaryService.findPrpDclassById(requestPacket.getHEAD()
				.getSYSTEMCODE(), requestPacket.getBODY().getCLASSCODE());
		
		//TODO 在ServiceInfoConst中添加对应接口的代码（FINDPRPDCLASSBYID）
		requestType =ServiceInfoConst.FINDPRPDCLASSBYID;// 设置返回报文的requesttype
		// TODO 添加常量定义
//		requestType = "";//TODO 记得删除
	...
	public SendPrpDclassIdReqPacket xmlToSchema(String requestxml)
			throws Exception {
		JOXBeanInputStream joxIn = new JOXBeanInputStream(
				new ByteArrayInputStream(requestxml.getBytes(IConstants.ENCODING_UTF8)));
		SendPrpDclassIdReqPacket response = (SendPrpDclassIdReqPacket) joxIn
				.readObject(SendPrpDclassIdReqPacket.class);
		return response;
	}
}

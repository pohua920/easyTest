...
import cn.com.sinosoft.inf.PMS.reqDomains.sendPrpDriskShortRateIdReqPacket.SendPrpDriskShortRateIdReqPacket;
...
public class TransFindPrpDriskShortRateById implements
		DataTransformer<SendPrpDriskShortRateIdReqPacket, PrpDriskShortRateListResPacket> {

	public String execute(String requestxml) throws Exception {
		List<PrpDriskShortRate> list = new ArrayList<PrpDriskShortRate>();
		String requestType = "";
		/** 请求的xml报文翻译成Packet对象 */
		SendPrpDriskShortRateIdReqPacket requestPacket = xmlToSchema(requestxml);
		/*************************************
		 * 调用持久层操作，获得所需数据
		 * **********************************/
		...
		PrpDriskShortRateId prpDriskShortRateId = new PrpDriskShortRateId();
		prpDriskShortRateId.setRiskCode(requestPacket.getBODY().getRISKCODE());
		if(null!=requestPacket.getBODY().getSERIALNO()&&!"".equals(requestPacket.getBODY().getSERIALNO())){
			prpDriskShortRateId.setSerialNo(Integer.parseInt(requestPacket.getBODY().getSERIALNO()));
		}
		prpDriskShortRateId.setShortRateID(requestPacket.getBODY().getSHORTRATEID());
		list = dictionaryService.findPrpDriskShortRateById(requestPacket.getHEAD()
				.getSYSTEMCODE(), prpDriskShortRateId);
		
		//TODO 在ServiceInfoConst中添加对应接口的代码（FINDPRPDCLASSBYID）
		requestType = ServiceInfoConst.FINDPRPDRISKSHORTRATEBYID;// 设置返回报文的requesttype
	...
	public SendPrpDriskShortRateIdReqPacket xmlToSchema(String requestxml)
			throws Exception {
		JOXBeanInputStream joxIn = new JOXBeanInputStream(
				new ByteArrayInputStream(requestxml.getBytes(IConstants.ENCODING_UTF8)));
		SendPrpDriskShortRateIdReqPacket response = (SendPrpDriskShortRateIdReqPacket) joxIn
				.readObject(SendPrpDriskShortRateIdReqPacket.class);
		return response;
	}
}

...
import cn.com.sinosoft.inf.PMS.reqDomains.sendPrpDriskLimitIdReqPacket.SendPrpDriskLimitIdReqPacket;
...
public class TransFindPrpDriskLimitById implements
		DataTransformer<SendPrpDriskLimitIdReqPacket, PrpDriskLimitListResPacket> {

	public String execute(String requestxml) throws Exception {
		List<PrpDriskLimit> list = new ArrayList<PrpDriskLimit>();
		String requestType = "";
		/** 请求的xml报文翻译成Packet对象 */
		SendPrpDriskLimitIdReqPacket requestPacket = xmlToSchema(requestxml);
		/*************************************
		 * 调用持久层操作，获得所需数据
		 * **********************************/
		...
		PrpDriskLimitId prpDriskLimitId = new PrpDriskLimitId();
		prpDriskLimitId.setLimitCode(requestPacket.getBODY().getLIMITCODE());
		prpDriskLimitId.setRiskCode(requestPacket.getBODY().getRISKCODE());
		if(!"".equals(requestPacket.getBODY().getSERIALNO())&&null!=requestPacket.getBODY().getSERIALNO()){
			prpDriskLimitId.setSerialNo(Integer.parseInt(requestPacket.getBODY().getSERIALNO()));
		}
		
		list = dictionaryService.findPrpDriskLimitById(requestPacket.getHEAD()
				.getSYSTEMCODE(), prpDriskLimitId);
		
		//TODO 在ServiceInfoConst中添加对应接口的代码（FINDPRPDCLASSBYID）
		requestType = ServiceInfoConst.FINDPRPDRISKLIMITBYID;// 设置返回报文的requesttype
	...
	public SendPrpDriskLimitIdReqPacket xmlToSchema(String requestxml)
			throws Exception {
		JOXBeanInputStream joxIn = new JOXBeanInputStream(
				new ByteArrayInputStream(requestxml.getBytes(IConstants.ENCODING_UTF8)));
		SendPrpDriskLimitIdReqPacket response = (SendPrpDriskLimitIdReqPacket) joxIn
				.readObject(SendPrpDriskLimitIdReqPacket.class);
		return response;
	}
}

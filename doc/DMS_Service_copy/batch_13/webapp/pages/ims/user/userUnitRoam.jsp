						...
						<script language="javascript">
							...
							<s:iterator value="comCodList" status="stuts">	
						  	 ...
						  	 d.add('<s:property value="%{comCodList[#stuts.index].comCode}" />','<s:property value="%{comCodList[#stuts.index].upperComCode}" />',
						  		  	 '<s:property value="%{comCodList[#stuts.index].comCName}" />','','','','','','',true,'',true,false);
						    </s:iterator>
...
tabFlag.push("taskIframe1");
  ...
  function updateMethod(){
	  var selectedCount = 0;
		...
		if(selectedCount>1) {
			alert("一个人员只能有一个漫游地，请重新选择！");
			return false;
		} else {	
			 fm.action="${ctx}/utiIUser/adjustUserOrg.do";
			 target="pages";
			 fm.submit();
		     return true;
		}

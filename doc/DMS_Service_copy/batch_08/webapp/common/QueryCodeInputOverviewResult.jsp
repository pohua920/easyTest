...
<%
	...
	/** 将双击查询出的字符串内容取出双引号和单引号 start update by wangzhifu 2010-11-18 */
	ArrayList codeValues = (ArrayList) valueStack.findValue("codeValues");
	for(int i=0; i<codeValues.size(); i++) {
	    codeValues.set(i, ((String)codeValues.get(i)).replaceAll("\\\"", "&quot;").replaceAll("\\\'", "&apos;"));
	}
	ArrayList codeLabels = (ArrayList) valueStack.findValue("codeLabels");
	for(int i=0; i<codeValues.size(); i++) {
	    codeLabels.set(i, ((String)codeLabels.get(i)).replaceAll("\\\"", "&quot;").replaceAll("\\\'", "&apos;"));
	}
	/** 将双击查询出的字符串内容取出双引号和单引号 end update by wangzhifu 2010-11-18 */
%>
...
function loadPage(){
	genNavigator(${pageNo},${pageSize},${totalPage},${totalCount});

...
class DTDReader {
	 ...
	 * @throws IOException
	 *             If there is an error reading the DTD
	 */
	public String[] getElements() throws IOException {
		...
		try {
			...
			in = new PushbackInputStream(new BufferedInputStream(urlConn
					.getInputStream()));
	...
	protected boolean scanForLTBang(PushbackInputStream in) throws IOException {
		...
		while ((ch = in.read()) >= 0) {
			...
			if (ch == '<') {
				...
				// the second < is still in the input stream so the routine can
				// check
	...
	protected void skipWhiteSpace(PushbackInputStream in) throws IOException {
		...
		while ((ch = in.read()) >= 0) {
			if (!Character.isWhitespace((char) ch)) {
				// If the most recent character isn't whitespace, unread the
				// character
	...
	protected String getString(PushbackInputStream in) throws IOException {
		...
		while ((ch = in.read()) >= 0) {
			if (Character.isWhitespace((char) ch)) {
				// If the most recent character is whitespace, unread the
				// character
				// so another routine can pick it up
				in.unread((byte) ch);
				return str.toString();

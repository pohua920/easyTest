...
public class JOXSAXBeanInput extends HandlerBase {
	...
	protected static HashMap beanInfos = new HashMap(); //LZY: add
	...
	//	LZY: add	
	private BeanInfo getBeanInfo(Object targetObject) throws IntrospectionException {

		Class targetClass = targetObject.getClass();
		String key = targetClass.getName();

		BeanInfo beanInfo = (BeanInfo) beanInfos.get(key);
		if (beanInfo == null) {
			beanInfo = Introspector.getBeanInfo(targetClass, java.lang.Object.class);
	...
	public void startElement(String elementName, AttributeList attributes)
		...
		if (obStack.isEmpty()) {
			try {
				//LZY: BeanInfo info = Introspector.getBeanInfo(rootObject.getClass(), Object.class);
				BeanInfo info = getBeanInfo(rootObject);
				...
				throw new SAXException("Error getting bean info for "
			...
			if (namesMatch(props[i].getName(), elementName)) {
				...
				if ((props[i] instanceof IndexedPropertyDescriptor)
					...
					try {
						...
						throw new SAXException("Error setting property "
				...
				if (isSimpleType(props[i])) {
					...
					ObjectProperty obProp = new ObjectProperty(ob, props[i], v);

					obStack.push(obProp);
					return;
				} else {
					try {
						//LZY: ObjectInfo newObjectInfo = new ObjectInfo(newOb, Introspector.getBeanInfo(newOb.getClass(), Object.class));
						ObjectInfo newObjectInfo = new ObjectInfo(newOb, getBeanInfo(newOb));
						
						processAttributes(newObjectInfo, attributes);
						obStack.push(newObjectInfo);
					} catch (IntrospectionException exc) {
						throw new SAXException("Error getting bean info for "
								+ newOb.getClass().getName() + ": " + exc.toString(), exc);
					}
					return;
				}
			}
			...
			for (int j = 0; j < props.length; j++) {
				...
				if (namesMatch(props[j].getName(), attributes.getName(i))) {
					if (props[j] instanceof IndexedPropertyDescriptor) {
						...
						Object obValue = getObjectValue(props[j], attributes.getValue(i));
						...
						if (obValue != null) {
							try {
								...
								setter.invoke(objectInfo.ob,new Object[] { obValue });
							} catch (InvocationTargetException exc) {
								throw new SAXException("Error setting property "
	...
	public void endElement(String elementName) throws SAXException {
		...
		if (ob instanceof StringBuffer) {
			...
			if (obValue != null) {
				try {
					// Set the property value
					setter.invoke(obProp.ob, new Object[] { obValue });
				} catch (InvocationTargetException exc) {
					throw new SAXException("Error setting property "
			...
			if (obInfo.indexedProperties != null) {
				PropertyDescriptor[] props = obInfo.beanInfo
						.getPropertyDescriptors();
				...
				for (int i = 0; i < props.length; i++) {
					...
					Vector v = (Vector) obInfo.indexedProperties.get(props[i].getName());
	...
	public Object getObjectValue(PropertyDescriptor desc, String value)
			throws SAXException {
	...
	public Object parseDate(String str, DateFormat dateFormat)
		...
		try {
			...
			throw new SAXException("Error parsing date " + str, exc);
		}
	}
	...
	/**
	 * Returns true if two names match without regard to case or the presence of
	 * '-' or '_' characters.
	 * 
	 * @param beanName
	 *            The name of the bean property to compare
	 * @param elementName
	 *            The name of the element to compare
	...
	public boolean namesMatch(String beanName, String elementName) {
		...
		while ((elementPos < elementNameLen) && (beanPos < beanNameLen)) {
			...
			// If the next character in the element name is a '-' or a '/', skip
			// it
			char elementCh = Character.toLowerCase(elementName
					.charAt(elementPos));

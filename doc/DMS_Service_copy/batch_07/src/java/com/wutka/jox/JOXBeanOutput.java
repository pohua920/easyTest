 ...
 * @version 2.0 2007-01-19 李子扬，去掉多于的"\n"
 */
...
public class JOXBeanOutput {
	...
	public static Hashtable beanCache = new Hashtable ();
	...
	/**
	 * Creates a JOXBeanOutput that uses a DTD to determine which attributes to
	 * write and what the format of the names should be.
	 * 
	 * @param dtdURL
	 *            The URL of the DTD
	 * @param theStream
	 ...
	 */
	public JOXBeanOutput(String dtdURL, JOXOutput theStream) throws IOException {
		...
		// like it eventually should, we'll know which items need to be
		// attributes
	 ...
	 * 
	 * @param dtd
	 *            The DTD for the XML to write
	 * @param theStream
	 *            The stream where the XML will be written
	 */
	public JOXBeanOutput(DTD aDtd, JOXOutput theStream) throws IOException {
		outputStream = theStream;
	...
	protected DTD readDTD(String dtdURL) throws IOException {
		Reader reader = null;
	 ...
	 *            a bean, the tag name will be the property name)
	 * @param ob
	...
	public void writeObject(String rootName, Object ob) throws IOException {
		...
		outputStream.writeString(">");//outputStream.writeString(">\n");
	 ...
	 * 
	 * @param xmlTagName
	 *            The XML tag name of the bean being written
	 * @param ob
	 *            The bean whose properties will be written
	 * @param writeType
	...
	public void writeObjectFields(String xmlTagName, Object ob, int writeType)
		...
		try {
			...
			if (info == null) {
				// Get the bean info for the bean, but don't get the attributes
				// from object
			...
			if ((dtd != null) && (writeType == WRITE_AS_ELEMENT)) {
				props = filterPropertyDescriptors(props, xmlTagName, ob
						.getClass());
			}
	...
	public void writeProperty(String xmlTagName, Object ob,
			PropertyDescriptor prop, int writeType) throws IOException {
		try {
			...
			throw new IOException("Unable to read property " + prop.getName()
					+ ": " + exc.toString());
	...
	public void writeIndexedProperty(String xmlTagName, Object ob,
			IndexedPropertyDescriptor prop) throws IOException {
		try {
	...
	public void writePropertyValue(String xmlTagName, Object propValue,
		...
		if (type.isArray() && (propValue != null)) {
			...
			for (int i = 0; i < len; i++) {
				...
				writePropertyValueByType(xmlTagName, realValue, prop,
						writeType, type);
			}
		} else {
			writePropertyValueByType(xmlTagName, propValue, prop, writeType,
					type);
		}
	}

	public void writePropertyValueByType(String xmlTagName, Object propValue,
			PropertyDescriptor prop, int writeType, Class type)
			throws IOException {
		...
		if (type.equals(Integer.TYPE) || type.equals(Long.TYPE)
				|| type.equals(Short.TYPE) || type.equals(Byte.TYPE)
				|| type.equals(Boolean.TYPE) || type.equals(Float.TYPE)
	...
	public void writeBasicType(String xmlTagName, String propName,
			Object propValue, int writeType) throws IOException {
	...
	public void writeString(String xmlTagName, String propName,
		...
		if ((writeAttributes || writeAsAttribute(xmlTagName, tagName))
			...
			if ((propString.indexOf('<') >= 0)
					...
					|| (propString.indexOf('&') >= 0)) {
				outputStream.writeString("<![CDATA[");//outputStream.writeString("<![CDATA[\n");
	...
	public void writeDate(String xmlTagName, String propName, Object propValue,
			int writeType) throws IOException {
		// Get the translated name of the property
		String tagName = getTranslatedChildName(xmlTagName, propName);
		...
		if ((writeAttributes || writeAsAttribute(xmlTagName, tagName))
			...
			if (writeType == WRITE_AS_ATTRIBUTE) {
				...
				outputStream.writeString(dateFormat
						.format((java.util.Date) propValue));
				outputStream.writeString("\"");
			} else {
	...
	protected void setupNameTranslation() {
		...
		while (e.hasMoreElements()) {
			...
			if (children instanceof DTDContainer) {
				expandChildElements((DTDContainer) children, childTransTable);
			}
	...
	public void expandChildElements(DTDContainer children,
		...
		while (childElems.hasMoreElements()) {
			...
			if (nextChild instanceof DTDElement) {
				...
				expandChildElements((DTDContainer) nextChild, childTransTable);
	...
	/**
	 * Finds the correct format of a name as given by the DTD. If there is no
	 * DTD, this method returns the original name. If there is a DTD but no
	 * match, this method returns null.
	 * 
	 * @param The
	 *            name to translate
	...
	public boolean writeAsAttribute(String elementName, String attributeName) {
		...
		Hashtable attrTrans = (Hashtable) attrTranslation
				.get(stripName(elementName));
	 ...
	 * descriptors that aren't in the DTD and ordering the elements the way they
	 * are in the DTD.
	 */
	protected PropertyDescriptor[] filterPropertyDescriptors(
		...
		PropertyDescriptor[] returnProps = (PropertyDescriptor[]) propertyDescCache
				.get(new DescCacheKey(xmlTagName, beanClass));
		if (returnProps != null) {
			return returnProps;
		}
		...
		DTDElement element = (DTDElement) dtd.elements
				.get(getTranslatedEntityName(xmlTagName));
		...
		for (int i = 0; i < descriptors.length; i++) {
			String tagName = getTranslatedChildName(xmlTagName, descriptors[i]
					.getName());
			if (tagName != null) {
		...
		extractPropertyOrder((DTDContainer) item, propLookup, v);
		...
		propertyDescCache
				.put(new DescCacheKey(xmlTagName, beanClass), newProps);
		return newProps;
	}

	/**
	 * Recursively look through a DTDContainer adding properties to the property
	 * list.
	 */
	protected void extractPropertyOrder(DTDContainer container,
		...
		while (e.hasMoreElements()) {
			...
			if (item instanceof DTDName) {
				...
				PropertyDescriptor desc = (PropertyDescriptor) propLookup
						.get(aName.value);
				if (desc != null) {
				...
				extractPropertyOrder((DTDContainer) item, propLookup, propList);
			}
		}
	}
}
...
class DescCacheKey {
	...
	public boolean equals(Object other) {
		if (other == null)
			return false;
		if (!(other instanceof DescCacheKey))
			return false;

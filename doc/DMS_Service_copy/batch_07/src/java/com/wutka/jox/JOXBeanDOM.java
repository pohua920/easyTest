...
public class JOXBeanDOM extends JOXBeanOutput {
	 ...
	 */
	public JOXBeanDOM(Document aDocument) throws IOException {
		this(aDocument, true);
	}
	 ...
	 */
	public JOXBeanDOM(String dtdURL) throws IOException {
		dtd = readDTD(dtdURL);
	 ...
	 * @param dtdURL The URL of the DTD
	 */
	public JOXBeanDOM(DTD aDTD) throws IOException {
		dtd = aDTD;
		setupNameTranslation();
	 ...
	 */
	public JOXBeanDOM(Document aDocument, String dtdURL) throws IOException {
		document = aDocument;
	 ...
	 */
	public JOXBeanDOM(boolean writeAttributesFlag) throws IOException {
	 ...
	 */
	public Element storeObject(String rootName, Object ob) throws IOException {
	...
	public void storeObjectFields(String xmlTagName, Object ob, int writeType,
			Element parentElement) throws IOException {
	...
	public void storePropertyValue(String xmlTagName, Object propValue,
		...
		if (type.isArray() && (propValue != null)) {
			...
			storePropertyValueByType(xmlTagName, propValue, prop, writeType,
					parentElement, type);
		}
	}
	...
	public void storePropertyValueByType(String xmlTagName, Object propValue,
		...
		if (type.equals(Integer.TYPE) || type.equals(Long.TYPE)
			...
			if (propValue != null) {
				storeBasicType(xmlTagName, prop.getName(), propValue,
						writeType, parentElement);
			}
	...
	public void storeDate(String xmlTagName, String propName, Object propValue,
			int writeType, Element parentElement) throws IOException {
		// Get the translated name of the property
		String tagName = getTranslatedChildName(xmlTagName, propName);
		...
		if ((writeAttributes || writeAsAttribute(xmlTagName, tagName))
				&& (writeType != WRITE_AS_INDEXED_ELEMENT)) {
			if (writeType == WRITE_AS_ATTRIBUTE) {
				parentElement.setAttribute(tagName, dateFormat
			...
			Element child = document.createElement(tagName);
			child.appendChild(document.createTextNode(dateFormat
					.format((java.util.Date) propValue)));

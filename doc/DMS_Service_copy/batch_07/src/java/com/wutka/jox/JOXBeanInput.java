...
public class JOXBeanInput {
	 ...
	 * @throws IOException If there is an error reading the document
	 */
	public void readProperty(Object ob, PropertyDescriptor desc,
			NodeList nodes, NamedNodeMap attrs) throws IOException {
			...
			if (namesMatch(desc.getName(), attrs.item(i).getNodeName())) {
				...
				if (obValue != null) {
					try {
						...
						throw new IOException("Error setting property "
		...
		if (arrayBuild != null) {
			...
			Object[] obValues = (Object[]) Array.newInstance(desc
					.getPropertyType(), arrayBuild.size());
	 ...
	 * @throws IOException If there is an error reading the document
	 */
	public void readIndexedProperty(Object ob, IndexedPropertyDescriptor desc,
			NodeList nodes, NamedNodeMap attrs) throws IOException {
		...
		Object propArray = Array.newInstance(desc.getPropertyType()
				.getComponentType(), v.size());
		...
		try {
			...
			throw new IOException("Error setting property " + desc.getName()
					+ ": " + exc.toString());
		} catch (IllegalAccessException exc) {
			throw new IOException("Error setting property " + desc.getName()
					+ ": " + exc.toString());
	 ...
	 * @throws IOException If there is an error reading the document
	 */
	public Object getObjectValue(PropertyDescriptor desc, Element element)
			throws IOException {
		...
		if (type.equals(Integer.TYPE) || type.equals(Long.TYPE)
			...
			try {
				...
				throw new IOException("Error creating object for "
						+ desc.getName() + ": " + exc.toString());
			} catch (IllegalAccessException exc) {
				throw new IOException("Error creating object for "
						+ desc.getName() + ": " + exc.toString());

		...
		<div id="wrapper">
			<div id="container">
					<table class="fix_table">
						<tr>
							<td class="bgc_tt short" nowrap="nowrap">代码类型：</td>
							<td class="long">${prpDtype.codeType}</td>
							<td class="bgc_tt short" nowrap="nowrap">类型名称：</td>
							<td class="long">${prpDtype.codeTypeDesc}</td>
							<td class="bgc_tt short" nowrap="nowrap">描述：</td>
							<td class="long">${prpDtype.codeTypeDesc}</td>
						</tr>
					</table>
				<div id="crash_menu">
				...
				<s:form name="fm" action="queryPrpDcode" namespace="/dictionary"
					method="post">
					<s:hidden name="flag" id="flag"></s:hidden>
					<s:hidden name="prpDcode.id.codeType" id="codeType"
						value="${codeType}"></s:hidden>
					...
					<table class="fix_table">
						<tr>
							...
							<td class="long">
								<input name="prpDcode.id.codeCode" id="prpDcode.id.codeCode"
									class='input_w w_10'>
							</td>
							<td class="bgc_tt short">
								名称：
							...
							<td class="long">
								<input name="prpDcode.codeCName" id="prpDcode.codeCName"
									class='input_w w_10'>
							</td>
...
<!--									onclick="executeQuery(1,10);">-->
							...
							<td colspan="2" align="center">
								...
								%>
								<button type="button"  value=""
									onclick="addMethod();"><span><em>增 加</em></span></button>
...
<!--									onclick="addMethod();">-->
							...
							<%--
							<td colspan="2" align="center">
								<input type="button" class="button_ty" value="删除"
...
<script type="text/javascript">
	...
	function init() {
		...
		YAHOO.widget.Column.formatLink = function(elCell, oRecord, oColumn,
            ...
            if(oColumn.key=="status"){
		    	if(valid == "1"){
		    	 	elCell.innerHTML = "<a href=\"#\" onclick=\"changeValidStatus('"+codeCode+"','"+codeType+"','"+valid+"')\">注销</a>";
		    	}
		    	else{
		    		elCell.innerHTML = "<a href=\"#\" onclick=\"changeValidStatus('"+codeCode+"','"+codeType+"','"+valid+"')\">启用</a>";
		    	}
		      }
			if (oColumn.key == "id.codeCode") {
				elCell.innerHTML = "<a href=\"#\" onclick=\"showRecord('${ctx}/dictionary/prepareUpdatePrpDcode.do?editType=view&prpDcode.id.codeCode="
						+ codeCode
						+ "&prpDcode.id.codeType="
						+ codeType
						+ "')\">" + oData + "</a>"
			}
			if (oColumn.key == "edit") {
				elCell.innerHTML = "<a href=\"#\" onclick=\"editRecord('${ctx}/dictionary/prepareUpdatePrpDcode.do?prpDcode.id.codeCode="
						+ codeCode
						+ "&prpDcode.id.codeType="
						+ codeType
			...
			if (oColumn.key == "chkbox") {
				elCell.innerHTML = "<input type='checkbox' name='chkbox' value='" + codeCode + "@" + codeType + "'>";
			}
	...
	function executeQuery(pageNo, pageSize) {
		...
		var myDataSource = new YAHOO.util.DataSource(
				"${ctx}/dictionary/queryPrpDcode.do");
		myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
		myDataSource.responseSchema = {
			resultsList :"data",
			fields : [ "id.codeType", "id.codeCode", "codeCName", "codeEName",
					"validStatus" ],
	...
	function deleteMethod() {
		...
		if (chkbox.length == 0) {
			...
			if (flag&&confirm("删除该代码，会更改其所有下级代码，确认要删除该代码吗？")) {
				deleteRecord1("${ctx}/dictionary/deletePrpDcode.do?chkbox=" + checkedValue);
			} else if(!flag){
				alert("没有选中列！");
			}
		}
	...
	function addMethod(){
		var addType = document.getElementById("codeType").value;
		editRecord('${ctx}/dictionary/prepareInsertPrpDcode.do?editType=insert&prpDcode.id.codeType='+addType);
	}
	
	
	function deleteRecord1(url){
	...
	function changeValidStatus(codeCode,codeType,valid){
		var result;
	    if(valid == "1"){
	   	 	result = "确定要注销吗？"
	   	}
		...
		if(confirm(result)){
		url="${ctx}/dictionary/changePrpDcodeValidStatus.do?prpDcode.id.codeCode="+codeCode+"&prpDcode.id.codeType="+codeType;
		var handleSuccess = function(o){
			var args = new SINOSOFT.util.QueryString( contentDataTable.initialRequest );
			var pageSize = parseInt(args["pageSize"],10);
			var pageNo = parseInt(args["pageNo"],10);

...
<script type="text/javascript">
    ...
    function init() {
        YAHOO.widget.Column.formatLink = function(elCell, oRecord, oColumn,oData) {
            ...
            if (oColumn.key == "id.comCode") {
				elCell.innerHTML = "<a href=\"#\" onclick=\"showRecord('${ctx}/dictionary/prepareUpdatePrpDcodeCom.do?editType=view&prpDcodeCom.id.comCode="
						+ comCode
						+ "&prpDcodeCom.id.codeType="
						+ codeType
						+ "&prpDcodeCom.id.codeCode="
						+ codeCode
						+ "')\">" + oData + "</a>"
			}
			...
			if (oColumn.key == "edit") {
				elCell.innerHTML = "<a href=\"#\" onclick=\"showRecord('${ctx}/dictionary/prepareUpdatePrpDcodeCom.do?prpDcodeCom.id.comCode="
						+ comCode
						+ "&prpDcodeCom.id.codeType="
						+ codeType
						+ "&prpDcodeCom.id.codeCode="
			...
			if(oColumn.key == "status"){
				if(validStatus == "1"){
					elCell.innerHTML = "<a href=\"#\" onclick=\"changeValidStatus('${ctx}/dictionary/changePrpDcodeComValidStatus.do?prpDcodeCom.id.comCode="
						+ comCode
						+ "&prpDcodeCom.id.codeType="
						+ codeType
						+ "&prpDcodeCom.id.codeCode="
        ...
        contentColumnHeaders = [
          ...
          {key :"id.comCode",text :"机构代码",width :"15em",sortable :true,resizeable :true,type:"link"},
          ...
          {key :"validStatus",text :"有效状态",width :"15em",sortable :true,resizeable :true},
    ...
    function executeQuery(pageNo,pageSize) {
        ...
        var myDataSource = new YAHOO.util.DataSource("${ctx}/dictionary/queryPrpDcodeCom.do");
        
        myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
        myDataSource.responseSchema = {
            resultsList:"data",
            fields:["id.comCode","id.codeType","id.codeCode","validStatus"],
...
<table class="fix_table">	
	<tr>
		...
		<td class="long">
			<s:textfield name="prpDcodeCom.id.comCode" id="comCode" cssClass='input_w w_15' maxlength="" />
		</td>
		<td class="bgc_tt short">代码类型</td>
		<td class="long">
			<s:textfield name="prpDcodeCom.id.codeType" id="codeType" cssClass="input_w w_15" maxlength="30" />
		</td>
	</tr>  
	<tr>
		...
		<button type="button"  value="" onclick="editRecord('${ctx}/dictionary/prepareInsertPrpDcodeCom.do?editType=insert');"><span><em>增 加</em></span></button>
<!--			<input type="button" class="button_ty" value="查 询" onclick="executeQuery(1,10);">-->
<!--			<input type="button" class="button_ty" value="增 加" onclick="editRecord('${ctx}/dictionary/prepareInsertPrpDcodeCom.do?editType=insert');">-->
		</td>
	</tr>
</table>
</s:form></div>

...
<s:form name="fm" action="queryPrpDresource" namespace="/dictionary" method="post">
<s:hidden name="flag" id="flag"></s:hidden>
<s:hidden name="deployCom" id="deployCom" value="${deployCom}"></s:hidden>
	<table class="fix_table">	
		<tr>
			<td class="bgc_tt short">专管专营代码</td>			
			<td class="long"><input name="prpDresource.resourceCode"
				id="prpDresource.resourceCode" class='input_w w_15'></td>
			<td class="bgc_tt short">专管专营名称</td>
			<td class="long"><input name="prpDresource.resourceName"
				id="prpDresource.resourceName" class='input_w w_15'></td>
...
<!--				class="button_ty" value="查 询" onclick="executeQuery(1,10);">-->
			...
			<button type="button" value="" onclick="editRecord('${ctx}/dictionary/prepareInsertPrpDresource.do?editType=insert');"><span><em>增 加</em></span></button>
<!--			<input type="button"class="button_ty" value="增 加" onclick="editRecord('${ctx}/dictionary/prepareInsertPrpDresource.do?editType=insert');">-->
			<%}%>
			</td>
		</tr>
	</table>
...
<script type="text/javascript">
    ...
    function init() {
        ...
        YAHOO.widget.Column.formatLink = function(elCell, oRecord, oColumn,oData) {
            var resourceCode = oRecord.resourceCode;
            var valid = oRecord.validStatus;
            if (oColumn.key == "resourceCode") {
                elCell.innerHTML = "<a href=\"#\" onclick=\"showRecord('${ctx}/dictionary/prepareUpdatePrpDresource.do?resourceCode="
                    + resourceCode + "&editType=view')\">"+oData+"</a>"
            }
            if (oColumn.key == "edit") {
                elCell.innerHTML = "<a href=\"#\" onclick=\"editRecord('${ctx}/dictionary/prepareUpdatePrpDresource.do?resourceCode="
                        + resourceCode + "&editType=update')\">修改</a>";
            }
 			if(oColumn.key=="status"){
		    	
		    	if(valid == "1"){
		    	 	elCell.innerHTML = "<a href=\"#\" onclick=\"changeValidStatus('"+resourceCode+"','"+valid+"')\">注销</a>";
		    	}else{
		    		elCell.innerHTML = "<a href=\"#\" onclick=\"changeValidStatus('"+resourceCode+"','"+valid+"')\">启用</a>";
		    	}    		
		    }
            if(oColumn.key == "chkbox"){
             	elCell.innerHTML = "<input type='checkbox' name='chkbox' value='"+resourceCode+"'>";
             }
        ...
        contentColumnHeaders = [ 
          ...
          {key :"resourceCode",text :"专管专营代码",width :"20em",sortable :true,type :"link"}, 
          {key :"resourceName",text :"专管专营名称",width :"40em",sortable :true}, 
          {key :"projectCode",text :"项目代码",width :"50em",sortable :true},
    ...
    function executeQuery(pageNo, pageSize) {
        ...
        var myDataSource = new YAHOO.util.DataSource("${ctx}/dictionary/queryPrpDresource.do");
        myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
        myDataSource.responseSchema = {
            resultsList :"data",
            fields : [ "resourceCode", "resourceName", "projectCode", "comCode","validStatus" ],
	...
	function changeValidStatus(resourceCode,valid){
		var result;
		if(valid=="1"){
				result = "确定要注销吗？";
			}
		...
		if(confirm(result)){
			url="${ctx}/dictionary/changePrpDresourceValidStatus.do?prpDresource.resourceCode="+resourceCode;

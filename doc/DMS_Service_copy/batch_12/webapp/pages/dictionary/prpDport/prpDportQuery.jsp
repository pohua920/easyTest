...
<s:form name="fm" action="queryPrpDport" namespace="/dictionary" method="post">
<s:hidden name="flag" id="flag"></s:hidden>
<s:hidden name="deployCom" id="deployCom" value="${deployCom}"></s:hidden>
	<table class="fix_table">	
		<tr>
			<td class="bgc_tt short">港口代码</td>			
			<td class="long"><input name="prpDport.portCode"
				id="prpDport.portCode" class='input_w w_15'></td>
			<td class="bgc_tt short">港口中文名</td>
			<td class="long"><input name="prpDport.portCName"
				id="prpDport.portCName" class='input_w w_15'></td>
...
<!--				class="button_ty" value="查 询" onclick="executeQuery(1,10);">-->
			...
			<button type="button"
				value="" onclick="editRecord('${ctx}/dictionary/prepareInsertPrpDport.do?editType=insert');"><span><em>增 加</em></span></button>
<!--			<input type="button"-->
<!--				class="button_ty" value="增 加" onclick="editRecord('${ctx}/dictionary/prepareInsertPrpDport.do?editType=insert');">-->
			<%}%>
			</td>
<%--
...
<script type="text/javascript">
    ...
    function init() {
        ...
        YAHOO.widget.Column.formatLink = function(elCell, oRecord, oColumn,oData) {
            var portCode = oRecord.portCode;
            var valid = oRecord.validStatus;
            if (oColumn.key == "portCode") {
                elCell.innerHTML = "<a href=\"#\" onclick=\"showRecord('${ctx}/dictionary/prepareUpdatePrpDport.do?portCode="
                    + portCode + "&editType=view')\">"+oData+"</a>"
            }
            if (oColumn.key == "edit") {
                elCell.innerHTML = "<a href=\"#\" onclick=\"editRecord('${ctx}/dictionary/prepareUpdatePrpDport.do?portCode="
                        + portCode + "&editType=update')\">修改</a>";
            }
 			if(oColumn.key=="status"){
		    	
		    	if(valid == "1"){
		    	 	elCell.innerHTML = "<a href=\"#\" onclick=\"changeValidStatus('"+portCode+"','"+valid+"')\">注销</a>";
		    	}else{
		    		elCell.innerHTML = "<a href=\"#\" onclick=\"changeValidStatus('"+portCode+"','"+valid+"')\">启用</a>";
		    	}
	    		
		      }
             if (oColumn.key == "delete") {
                elCell.innerHTML = "<a href=\"#\" onclick=\"deleteRecord('${ctx}/dictionary/deletePrpDport.do?portCode="
                        + portCode + "')\">删除</a>";
            }
             
             if(oColumn.key == "chkbox"){
             	elCell.innerHTML = "<input type='checkbox' name='chkbox' value='"+portCode+"'>";
             }
        ...
        contentColumnHeaders = [ 
          ...
          {key :"portCode",text :"港口代码",width :"20em",sortable :true,type :"link"}, 
          {key :"portCName",text :"港口中文名称",width :"40em",sortable :true}, 
          {key :"countryCName",text :"国家中文名称",width :"50em",sortable :true}, 
    ...
    function executeQuery(pageNo, pageSize) {
        ...
        var myDataSource = new YAHOO.util.DataSource("${ctx}/dictionary/queryPrpDport.do");
        myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
        myDataSource.responseSchema = {
            resultsList :"data",
            fields : [ "portCode", "portCName", "countryCode", "countryCName","validStatus" ],
    ...
    function deleteMethod(){
    	...
    	if(chkbox.length==0){
			...
			if(flag){
				deleteRecord('${ctx}/dictionary/deletePrpDport.do?chkbox='+checkedValue);
			}else{
				alert("没有选中列！");
			}
         }
    }
	function changeValidStatus(portCode,valid){
	  	var result;
	    if(valid == "1"){
	    result = "确定要注销吗？"
	    }
		...
		if(confirm(result)){
		url="${ctx}/dictionary/changePrpDportValidStatus.do?prpDport.portCode="+portCode;

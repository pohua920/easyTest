...
<form name="fm" action="" method="post" >
	...
	<input type="hidden" name="svrName" id="svrName" value=""/>
			...
			<td class="long">
			    <div id="validStatusMapDiv" class="selectui-indiv  dc-chk">
			        ...
			        <c:set var="checked" value="0" />
			  			<ce:select name="svrCode" id="svrCode" cssClass="selectui-input-up input_w w_30" value="${checked}" list="svrList" />
			    </div>
...
//	tabFlag.push("taskIframe1");
    ...
    function next(){
        ...
        if(sCode == "请选择" || sCode == ""||sCode == "undefined"){
			alert("请选择服务！");
			return false;
        }else{
            var userCode = document.getElementById("userCode").value;
            Ims.checkAccountBySvr(sCode,userCode,callBack);
	...
	function callBack(date){
		if(date == 'yes'){
			if(confirm('该用户在该服务中已经存在账户，确认还要添加账户吗？')){

...
public class InsuredEndorseServiceImpl implements InsuredEndorseService{

	private InsuredEndorseDao insuredEndorseDao;

	@Override
	public int countInsuredEndorse(Map params) throws SystemException, Exception {
		return insuredEndorseDao.count(params);
	}
	...
	public Result findInsuredEndorseByPageInfo(PageInfo pageInfo) throws SystemException, Exception {
		...
		int rowCount = insuredEndorseDao.count(pageInfo.getFilter());
		if (0 == rowCount) {
		...
		List<InsuredEndorse> searchResult = insuredEndorseDao.findByPageInfo(pageInfo);
	...
	public Result findInsuredEndorseByParams(Map params) throws SystemException, Exception {
		...
		Result result = new Result();
		List<InsuredEndorse> searchResult = insuredEndorseDao.findByParams(params);
		if (null == searchResult || 0 == searchResult.size()) {
			result.setMessage(Message.getMessage(Constants.SEARCH_NO_DATA));
		} else {
			result.setResObject(searchResult);
	...
	public Result findInsuredEndorseByUK(String transactionId) throws SystemException,Exception {
		...
		InsuredEndorse persisted = insuredEndorseDao.findByUK(params);
		if (null == persisted) {
			result.setMessage(Message.getMessage(Constants.SEARCH_NO_DATA));
		} else {
	...
	public Result updateInsuredEndorse(InsuredEndorse insuredEndorse) throws SystemException, Exception {
		...
		if (insuredEndorse == null) {
			throw new SystemException(Message.getMessage(Constants.DATA_NOT_EXIST));
		}
		
		if(insuredEndorseDao.isUnique(insuredEndorse)) {
			throw new SystemException(Constants.DATA_NOT_EXIST);  
		}
		
		boolean status = insuredEndorseDao.update(insuredEndorse);
		if (!status) {
			throw new SystemException(Message.getMessage(Constants.UPDATE_DATA_FAIL));
		}
		Result result = new Result();
		result.setMessage(Message.getMessage(Constants.UPDATE_DATA_SUCCESS));
		result.setResObject(insuredEndorse);
		return result;
	}
	...
	public Result insertInsuredEndorse(InsuredEndorse insuredEndorse) throws SystemException, Exception {
		...
		insuredEndorseDao.insert(insuredEndorse);
	...
	public Result removeInsuredEndorse(String transactionId) throws SystemException, Exception {
		...
		boolean status = insuredEndorseDao.remove(persisted);
		if(!status){
	...
	public InsuredEndorseDao getInsuredEndorseDao() {
		return insuredEndorseDao;
	}

	public void setInsuredEndorseDao(InsuredEndorseDao insuredEndorseDao) {
		this.insuredEndorseDao = insuredEndorseDao;
	}
}

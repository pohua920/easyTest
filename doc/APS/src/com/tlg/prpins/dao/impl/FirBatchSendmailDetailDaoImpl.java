package com.tlg.prpins.dao.impl;

import java.math.BigDecimal;
import java.util.List;

import com.tlg.aps.vo.WriteForBatchSendmailVo;
import com.tlg.iBatis.impl.IBatisBaseDaoImpl;
import com.tlg.prpins.dao.FirBatchSendmailDetailDao;
import com.tlg.prpins.entity.FirBatchSendmailDetail;

public class FirBatchSendmailDetailDaoImpl extends IBatisBaseDaoImpl<FirBatchSendmailDetail, BigDecimal> implements FirBatchSendmailDetailDao {
	
	@Override
	public String getNameSpace() {
		return "FirBatchSendmailDetail";
	}
	
	/*mantis：CAR0508，處理人員：BJ085，需求單編號：CAR0508 APS查詢電子保單發送結果(新增車險與旅平險)*/
	public List<WriteForBatchSendmailVo> findRiskcodeForNoticeEmail() {
 		List<WriteForBatchSendmailVo> queryForList = getSqlMapClientTemplate().queryForList(getNameSpace()+".selectRiskcodeForNoticeEmail");
 		return queryForList;
 	}
}
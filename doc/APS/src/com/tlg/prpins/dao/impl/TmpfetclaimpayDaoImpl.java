...
public class TmpfetclaimpayDaoImpl extends IBatisBaseDaoImpl<Tmpfetclaimpay, BigDecimal> implements TmpfetclaimpayDao {
	
	@Override
	public String getNameSpace() {
		return "Tmpfetclaimpay";
	}
	...
	@Override
	public void processInBatch(final List<Tmpfetclaimpay> listTmpfetclaimpay) {
		try {
			Integer count = (Integer) getSqlMapClientTemplate().execute( new SqlMapClientCallback() {
	            public Object doInSqlMapClient( SqlMapExecutor executor ) throws SQLException {
	            	int batch = 0;
	                executor.startBatch();
	                Iterator<Tmpfetclaimpay> iter = listTmpfetclaimpay.iterator();
	                while(iter.hasNext()) {
	                	batch++;

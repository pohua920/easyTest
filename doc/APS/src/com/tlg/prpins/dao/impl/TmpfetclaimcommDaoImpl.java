...
public class TmpfetclaimcommDaoImpl extends IBatisBaseDaoImpl<Tmpfetclaimcomm, BigDecimal> implements TmpfetclaimcommDao {
	
	@Override
	public String getNameSpace() {
		return "Tmpfetclaimcomm";
	}
		...
		try {
			Integer count = (Integer) getSqlMapClientTemplate().execute( new SqlMapClientCallback() {
	            public Object doInSqlMapClient( SqlMapExecutor executor ) throws SQLException {
	            	int batch = 0;
	                executor.startBatch();
	                Iterator<Tmpfetclaimcomm> iter = listTmpfetclaimcomm.iterator();
	                while(iter.hasNext()) {
	                	batch++;

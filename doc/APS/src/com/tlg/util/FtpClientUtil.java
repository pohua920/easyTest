...
public class FtpClientUtil { 
	...
	protected Logger logger = Logger.getLogger(FtpClientUtil.class);
	...
	public void get() throws SocketException, IOException, Exception {
		this.get(ftpserver, ftpuser, ftppass, "", storeDir, false, false);
	}		
	...
	public void get(
			...
			String cwdDirectory, File storeDir, boolean deleteFtpFile, boolean passive) throws SocketException, IOException, Exception {
		
		if (!this.ftpClient.isConnected()) {
			this.login(ftpserver, ftpuser, ftppass);
		}			
		this.get(cwdDirectory, storeDir, null, deleteFtpFile, passive);
	}	
	 ...
	 * @throws Exception
	 */	
	public void get(String cwdDirectory, File storeDir, String head, boolean passive) throws SocketException, IOException, Exception {
		this.get(cwdDirectory, storeDir, head, false, passive);
	}
	
	/**
	 * 給王安NetTram FTP 用的版本, 一般正常的 FTPd 不要用這個method
	 * 
	 * 
	 * 把目錄下的檔案抓下來,如果有權限 
	 * 
	 ...
	 */
	public void get(
			String cwdDirectory, File storeDir, String head, boolean deleteFtpFile, boolean passive) throws SocketException, IOException, Exception {
	...
	/**
	 * 刪除時重新連線 , 提供給 FNSZ05Job 用, 因為Z05跑很久, 等刪除檔案時就timeout了,
	 * 所以重新建連線
	 * 

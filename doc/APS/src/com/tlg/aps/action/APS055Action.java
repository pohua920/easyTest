...
public class APS055Action extends BaseAction {
	...
	private FirBotGenFileService firBotGenFileService;
	private BotRenewalFileService botRenewalFileService;
	private ConfigUtil configUtil;
	
	private List<Aps055BotDetailVo> devResults;
	private List<Aps055BotDetailVo> dtlDevResults;
	...
	private Aps055DownloadlVo botDownloadVo;
	...
	public String btnQuery() throws Exception {
		try{
			...
			getPageInfo().getFilter().put("businessnature", "I99004");
	...
	/** APS055E0.jsp(主檔)分頁資料中，重新輸入要顯示的頁數  */
	...
	/** APS055E0.jsp，(主檔)分頁資料中，重新選擇下拉式選單中更改每頁所顯示的資料筆數 */
	...
	/** APS055E0.jsp頁面按下主檔批次號碼資料，查詢明細資料 */
	@SuppressWarnings("unchecked")
	...
	/** APS055E1.jsp(明細)分頁資料中，重新輸入要顯示的頁數 */
	...
	/** APS055E1.jsp(明細)分頁資料中，重新選擇下拉式選單中更改每頁所顯示的資料筆數 */
	...
	/** APS055E1.jsp(明細)分頁資料中， 查詢結果點選上下三角型排序 */
	...
	public String btnDownloadXls() throws Exception {
		try {
			...
			try (SXSSFWorkbook workbook = new SXSSFWorkbook()) {
				...
				Result result = firAgtrnBatchDtlService.findBotRnDataForXlsByBatchNo(getDPageInfo());

				if (result.getResObject() != null) {
					List<Aps055BotGenFileVo> excelList = (List<Aps055BotGenFileVo>) result.getResObject();
					for (int i = 0; i < excelList.size(); i++) {
						SXSSFRow row = sheet.createRow(i + 1); // 建立儲存格
						Aps055BotGenFileVo xlsData = excelList.get(i);
	...
	/** APS055E1.jsp(明細)分頁資料中， 點選序號進入資料修改頁面*/
	...
	private void update() throws Exception {
		...
		botRenewalFileService.updateFirAgtTocoreAndAgtrnBatchDtl(botDetailVo, getUserInfo().getUserId().toUpperCase());
		setMessage("儲存成功");
	}
	
	/** APS055E0.jsp頁面按下進入，進入檔案產生、下載頁面 */
	...
	public String btnGenFile() throws Exception {
		...
		if("reFile".equals(fileType)) {
			result = firBotGenFileService.genReFile(batchNo, userId);
			message = result.getMessage().toString();
		}
		if("enFile".equals(fileType)) {
			result = firBotGenFileService.genEnFile(batchNo, userId);
			message = result.getMessage().toString();
		}
		if("rnproFile".equals(fileType)) {
			result = firBotGenFileService.genRnproFile(batchNo, userId, rnYyyymm);
			message = result.getMessage().toString();
		}
	...
	public String btnFixData() throws Exception {
		...
		if(result.getResObject()!=null) {
			...
			for(FirAgtrnBatchDtl dtl : fixList) {
				...
				//mantis：FIR0621，處理人員：BJ085，需求單編號：FIR0621 住火_臺銀續保作業_續件資料處理作業_第二階段處理作業
				dtl.setDataStatus("B");
	...
	public Result updateQuery(Map<String,Object> params) throws Exception {
		Result result = firAgtrnBatchDtlService.findBotrnInsuredData(params);
		if (null == result.getResObject()) {
			setMessage(result.getMessage().toString());
			return result;
		} else {
			botDetailVo = (Aps055BotDetailVo) result.getResObject();
		}
	...
	private void query() throws Exception {
		parameterHandler();
		Result result = firAgtrnBatchMainService.findBatchMainForBotrnByPageInfo(getPageInfo());
		if (null != result.getResObject()) {
			devResults = (List<Aps055BotDetailVo>) result.getResObject();
		} else {
			setMessage(result.getMessage().toString());
		}
	}
	...
	private Result dtlQuery() throws Exception {
		Result result = new Result();	
		result = firAgtrnBatchDtlService.findForBotrnDetail(getDPageInfo());
		if (null != result.getResObject()) {
			dtlDevResults = (List<Aps055BotDetailVo>) result.getResObject();
		}else {
			setMessage(result.getMessage().toString());
		}
		return result;
	...
	public Result downloadQuery(Map<String, Object> params) throws Exception {
		Result result = firAgtrnBatchMainService.findBotrnDownloadData(params);
		if (null == result.getResObject()) {
			setMessage(result.getMessage().toString());
			return result;
		} else {
			botDownloadVo = (Aps055DownloadlVo) result.getResObject();
		}
		return result;
	}
	...
	public List<Aps055BotDetailVo> getDevResults() {
		return devResults;
	}

	public void setDevResults(List<Aps055BotDetailVo> devResults) {
		this.devResults = devResults;
	}

	public List<Aps055BotDetailVo> getDtlDevResults() {
		return dtlDevResults;
	}

	public void setDtlDevResults(List<Aps055BotDetailVo> dtlDevResults) {
	...
	public Aps055DownloadlVo getBotDownloadVo() {
		return botDownloadVo;
	}

	public void setBotDownloadVo(Aps055DownloadlVo botDownloadVo) {
		this.botDownloadVo = botDownloadVo;
	}

	public FirBotGenFileService getFirBotGenFileService() {
		return firBotGenFileService;
	}

	public void setFirBotGenFileService(FirBotGenFileService firBotGenFileService) {
		this.firBotGenFileService = firBotGenFileService;
	}

...
public class FirBotMaintainServiceImpl implements FirBotMaintainService {

	private static final Logger logger = Logger.getLogger(FirBotMaintainServiceImpl.class);
	private FirVerifyDatasService firVerifyDatasService;

	private Map<String, String> structureMap = new LinkedHashMap<>();

	@Override
	public Map<String, Object> basicDataCheck(Aps055BotDetailVo botDetailVo, String userId, String checkType)
			throws Exception {
		...
		// 受理編號檢核
		String orderseq = botDetailVo.getOrderseq();
		if (StringUtil.isSpace(orderseq) || orderseq.length() != 19) {
			errMsg.append("受理編號必填且長度需為19碼;");
		} else if (!"18".equals(oldpolicyno.substring(0, 2))) {
			errMsg.append("受理編號需為18開頭;");
		}else if (!"T".equals(orderseq.substring(5,6))||!"T".equals(orderseq.substring(6,7))) {
			errMsg.append("受理編號第6碼及第7碼都需為T;");
		}else if (!StringUtil.isSpace(botDetailVo.getExtracomcode()) 
				&& !orderseq.substring(2,5).equals(botDetailVo.getExtracomcode().substring(3,6))) {
			errMsg.append("受理編號第3-5碼需為分行代號的第4-6碼;");
		}else if (!StringUtil.isSpace(oldpolicyno) 
				&& !orderseq.substring(orderseq.length()-12).equals(oldpolicyno.substring(oldpolicyno.length()-12))) {
			errMsg.append("受理編號後12碼需為續保單號的後12碼;");
		...
		if (StringUtil.isSpace(handleridentifynumber)) {
			...
			resultMap = firVerifyDatasService.findPrpyddagentByParams(handleridentifynumber, "I99004");

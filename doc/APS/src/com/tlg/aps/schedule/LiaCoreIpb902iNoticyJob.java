...
public class LiaCoreIpb902iNoticyJob extends QuartzJobBean {

	protected Logger logger = Logger.getLogger(LiaCoreIpb902iNoticyJob.class);

	public LiaCoreIpb902iNoticyJob() {
		super();
	}

	@Override
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		logger.info("LiaCoreIpb902iNoticyJob begin...");
		try {
			 ...
			 if (jobs != null && !jobs.isEmpty()) {
				 for (JobExecutionContext job : jobs) {
					 if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						 ...
						 String auth = "smtp";
						 String subject = "新核心 IPB902I呼叫CWP公會通報排程重複執行(前一次未執行完)通知，請儘快處理～";
			...
			logger.info("新核心 IPB902I傳送至公會通報排程作業完成");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("新核心 IPB902I傳送至公會通報排程作業失敗" + DateUtils.getDateString(new Date()));
		...
		logger.info("LiaCoreIpb902iNoticyJob end...");
	}
}

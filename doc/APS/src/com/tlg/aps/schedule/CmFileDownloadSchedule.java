...
public class CmFileDownloadSchedule implements Job{

	protected Logger logger = Logger.getLogger(CmFileDownloadSchedule.class);	
	private RunFetPolicyService runFetPolicyService;
	
	public CmFileDownloadSchedule() {
		super();
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		logger.info("CmFileDownloadSchedule begin...");
		try {
			 ...
			 if (jobs != null && !jobs.isEmpty()) {
				 for (JobExecutionContext job : jobs) {
					 if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						 ...
						 String auth = "smtp";
						 String subject = "行動裝置險-將安達提供的佣金回饋檔記錄到CM_FILE資料表作業暫停執行";
			...
			runFetPolicyService.downloadCmFile();
			logger.info("將安達提供的佣金回饋檔記錄到CM_FILE資料表");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("將安達提供的佣金回饋檔記錄到CM_FILE資料表失敗");
		...
		logger.info("CmFileDownloadSchedule end...");
	}
}

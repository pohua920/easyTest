...
public class ClFileDownloadSchedule implements Job{

	protected Logger logger = Logger.getLogger(ClFileDownloadSchedule.class);	
	private RunFetPolicyService runFetPolicyService;
	
	public ClFileDownloadSchedule() {
		super();
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		logger.info("ClFileDownloadSchedule begin...");
		try {
			 ...
			 if (jobs != null && !jobs.isEmpty()) {
				 for (JobExecutionContext job : jobs) {
					 if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						 ...
						 String auth = "smtp";
						 String subject = "行動裝置險-將安達提供的理賠回饋檔記錄到CL_FILE資料表作業暫停執行";
			...
			runFetPolicyService.downloadClFile();
			logger.info("將安達提供的理賠回饋檔記錄到CL_FILE資料表");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("將安達提供的理賠回饋檔記錄到CL_FILE資料表失敗");
		...
		logger.info("ClFileDownloadSchedule end...");
	}
}

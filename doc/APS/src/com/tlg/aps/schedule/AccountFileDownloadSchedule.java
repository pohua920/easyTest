...
public class AccountFileDownloadSchedule implements Job{

	protected Logger logger = Logger.getLogger(AccountFileDownloadSchedule.class);	
	private RunFetPolicyService runFetPolicyService;
	
	public AccountFileDownloadSchedule() {
		super();
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		logger.info("AccountFileDownloadSchedule begin...");
		try {
			 ...
			 if (jobs != null && !jobs.isEmpty()) {
				 for (JobExecutionContext job : jobs) {
					 if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						 ...
						 String auth = "smtp";
						 String subject = "行動裝置險-將安達提供的財務用銷帳回饋檔記錄到ACCOUNT_FILE資料表作業暫停執行";
			...
			runFetPolicyService.downloadAccountFile();
			logger.info("將安達提供的財務用銷帳回饋檔記錄到ACCOUNT_FILE資料表");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("將安達提供的財務用銷帳回饋檔記錄到ACCOUNT_FILE資料表作業失敗");
		...
		logger.info("AccountFileDownloadSchedule end...");
	}
}

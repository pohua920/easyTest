...
public class AssocToLiaRcvAnnounceJob extends QuartzJobBean {

	protected Logger logger = Logger.getLogger(AssocToLiaRcvAnnounceJob.class);

	public AssocToLiaRcvAnnounceJob() {
		super();
	}

	@Override
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		logger.info("AssocToLiaRcvAnnounceJob begin...");
		try {
			...
			if (jobs != null && !jobs.isEmpty()) {
				for (JobExecutionContext job : jobs) {
					if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						...
						String auth = "smtp";
						String subject = "ASSOC_RCV轉LIA_RCV排程重複執行(前一次未執行完)通知，請儘快處理～";
			...
			logger.info("傷害險收件通報轉檔系統作業(ASSOC轉中介)作業完成");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("傷害險收件通報轉檔系統作業(ASSOC轉中介)作業失敗" + DateUtils.getDateString(new Date()));
		...
		logger.info("AssocToLiaRcvAnnounceJob end...");
	}
}

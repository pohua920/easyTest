 ...
 *
 */
public class HasFetMatchFileDayJob implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		logger.info("HasFetMatchFileDayJob begin...");
		String backFileType = "1";//每日作業
		String userId = "SYSTEM";

		try {
			Calendar dataDate = Calendar.getInstance();
			dataDate.add(Calendar.DATE, -1);//系統日-1
			...
			if(result.getMessage() != null){
				...
				logger.info("遠傳旅平險姓名生日不一致比對作業(每日):"+result.getMessage().toString());
			}
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("遠傳旅平險姓名生日不一致比對作業(每日)失敗" + DateUtils.getDateString(new Date()));
		...
		logger.info("HasFetMatchFileDayJob end...");
	}
}

...
public class SendSmsJob extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		logger.info("SendSmsJob(簡訊web service 發送排程) begin...");
		try {
			List<JobExecutionContext> jobs = context.getScheduler().getCurrentlyExecutingJobs();
			 if (jobs != null && !jobs.isEmpty()) {
				 for (JobExecutionContext job : jobs) {
					 if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						 ...
						 String auth = "smtp";
						 String subject = "簡訊web service 發送排程重複執行(前一次未執行完)通知，請儘快處理～";
			 ...
			 smsService.batchSendMessage();
			 smsService.batchHandleExceMessage();
			 
			 logger.info("SendSmsJob(簡訊web service 發送排程)作業完成");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("SendSmsJob(簡訊web service 發送排程)作業失敗" + DateUtils.getDateString(new Date()));
		...
		logger.info("SendSmsJob(簡訊web service 發送排程) end...");
	}
}

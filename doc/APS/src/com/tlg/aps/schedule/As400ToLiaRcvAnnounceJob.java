...
public class As400ToLiaRcvAnnounceJob extends QuartzJobBean {

	protected Logger logger = Logger.getLogger(As400ToLiaRcvAnnounceJob.class);

	public As400ToLiaRcvAnnounceJob() {
		super();
	}

	@Override
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		logger.info("As400ToLiaRcvAnnounceJob begin...");
		try {
			 ...
			 if (jobs != null && !jobs.isEmpty()) {
				 for (JobExecutionContext job : jobs) {
					 if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						 ...
						 String auth = "smtp";
						 String subject = "AS400轉LIA_RCV排程重複執行(前一次未執行完)通知，請儘快處理～";
			...
			logger.info("傷害險收件通報轉檔系統作業(400轉中介)作業完成");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("傷害險收件通報轉檔系統作業(400轉中介)作業失敗" + DateUtils.getDateString(new Date()));
		...
		logger.info("As400ToLiaRcvAnnounceJob end...");
	}
}

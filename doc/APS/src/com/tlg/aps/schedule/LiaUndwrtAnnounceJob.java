...
public class LiaUndwrtAnnounceJob extends QuartzJobBean{

	protected Logger logger = Logger.getLogger(LiaUndwrtAnnounceJob.class);	
	
	public LiaUndwrtAnnounceJob() {
		super();
	}		
	
	 @Override
	 protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		logger.info("LiaUndwrtAnnounceJob begin...");
		try {
			 ...
			 if (jobs != null && !jobs.isEmpty()) {
				 for (JobExecutionContext job : jobs) {
					 if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						 ...
						 String auth = "smtp";
						 String subject = "LIA_UNDWRT呼叫CWP公會通報排程重複執行(前一次未執行完)通知，請儘快處理～";
			...
			logger.info("傳送至公會承保通報排程作業完成");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("傳送至公會承保通報排程作業失敗" + DateUtils.getDateString(new Date()));
		...
		logger.info("LiaUndwrtAnnounceJob end...");
	}
}

...
public class AcFileDownloadSchedule implements Job{

	protected Logger logger = Logger.getLogger(AcFileDownloadSchedule.class);	
	private RunFetPolicyService runFetPolicyService;
	
	public AcFileDownloadSchedule() {
		super();
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		logger.info("AcFileDownloadSchedule begin...");
		try {
			 ...
			 if (jobs != null && !jobs.isEmpty()) {
				 for (JobExecutionContext job : jobs) {
					 if (job.getTrigger().equals(context.getTrigger()) && !job.getJobInstance().equals(this)) {
						 ...
						 String auth = "smtp";
						 String subject = "行動裝置險-將安達提供的銷帳回饋檔記錄到AC_FILE資料表作業暫停執行";
			...
			runFetPolicyService.downloadAcFile("");/** mantis：MOB0026，處理人員：CE035，需求單編號：MOB0026 優化手機險對帳流程   ac檔補批單號 */
			logger.info("將安達提供的銷帳回饋檔記錄到AC_FILE資料表");
		} catch (SystemException e) {
			e.printStackTrace();
			logger.warn("將安達提供的銷帳回饋檔記錄到AC_FILE資料表作業失敗");
		...
		logger.info("AcFileDownloadSchedule end...");
	}
}

...
(function($) {
  ...
  $.fn.extend({
    ...
    twzipcode: function(options) {	  
      ...
      var i = 0, tpl = [];
      var opt = ['<option value=""></option>', '<option value="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</option>'];
      ...
      if(2 === countyAndarea.length){
        ...
        // -----------------------------------------------------------------------
        // 2011-05-20
        beforeCountyName=countyAndarea[0];
        beforeAreaName=countyAndarea[1];
        // -----------------------------------------------------------------------     
      ...
      sel.county.append( tpl.join('') ).val( o.countySel ).attr('selected', true).change(function(){
        ...
        if(sel.county.val() === ''){
          ...
          //Jessica Add opt[0]+tpl.join('') 原本只有tpl.join('') 為了加一個空白的預設選項
          sel.area.empty().append( opt[0]+tpl.join('') ).val( o.areaSel ).attr('selected', true).trigger('change');
          
          // 2011-05-20
          var beforeAddressName=$("#"+o.addressName).val();
          if (beforeAddressName!='' && beforeCountyName!='' ) {
        	  var nowAddressName=$.fn.twzipcode.replaceAll(beforeAddressName, beforeCountyName, sel.county.val() );
        	  $("#"+o.addressName).val(nowAddressName);
          }          
          
        }
        
        if (sel.county.val()!= '' ) {
        	beforeCountyName=sel.county.val(); // 2011-05-20
        }
      ...
      sel.area.change(function(){
        ...
        if(sel.county.val() !== '' && sel.area.val() !== ''){       
          ...
          // 2011-05-20          
          var beforeAddressName=$("#"+o.addressName).val();
          if (beforeAddressName!='' && beforeAreaName!='' ) {
        	  var nowAddressName=$.fn.twzipcode.replaceAll(beforeAddressName, beforeAreaName, sel.area.val() );
        	  $("#"+o.addressName).val(nowAddressName);
          } else {        	  
        	  if (!$.fn.twzipcode.searchFound($("#"+o.addressName).val(), sel.county.val(), sel.area.val()) ) {
        		  $("#"+o.addressName).val(sel.county.val()+sel.area.val()+$("#"+o.addressName).val() );
        	  }
          }
        ...
        if(sel.county.val() !== '' && sel.area.val() == ''){
        	sel.zip.val('');//Jessica Add
        }
        
        if (sel.area.val()!='' ) {
        	beforeAreaName=sel.area.val(); // 2011-05-20
        }
      ...
      sel.zip.change(function(){
         ...
         * 2011-05-20 需求變更
         * 郵遞區號處理：
         * 1.輸入郵遞區號帶出"城市","鄉鎮縣市"選單
         * 2.下拉選單異動時，搜尋詳細地址內起頭開始有無先相同"城市","鄉鎮縣市"內容的字串，如果有將其置換成最新的"城市","鄉鎮縣市"。
         * -----------------------------------------------------------------------
         * 
         * A. 找的到資料,取代
         * B. 找不到資料, 把 縣市/地區 放到開頭
         */
        var beforeCountyValue=sel.county.val();
        var beforeAreaValue=sel.area.val();
        var beforeAddressName=$("#"+o.addressName).val();        
        
        if(0 === val.length){
        	sel.county.val('').attr('selected', true).trigger('change');
            sel.area.val('').attr('selected', true);
            sel.zip.val('');
        ...
        if(2 === _countyAndarea.length){        	     	        	
          ...
          sel.zip.val( val );
      		 ...
      		 * -----------------------------------------------------------------------
      		 */          	          	
      		var nowCountyValue=_countyAndarea[0];
      		var nowAreaValue=_countyAndarea[1];
      		if (beforeCountyValue!='' && beforeAreaValue!='' && beforeAddressName!='' ) {
      			var nowAddressName=$.fn.twzipcode.replaceAll(beforeAddressName, beforeCountyValue, nowCountyValue);
      			nowAddressName=$.fn.twzipcode.replaceAll(nowAddressName, beforeAreaValue, nowAreaValue);
      			$("#"+o.addressName).val(nowAddressName);      			
      		}
      		if (beforeAddressName=='' ) {      		      			
      			$("#"+o.addressName).val(nowCountyValue+nowAreaValue);
      		} else {
      			if (!$.fn.twzipcode.searchFound($("#"+o.addressName).val(), nowCountyValue, nowAreaValue) ) {      				
      				$("#"+o.addressName).val(nowCountyValue+nowAreaValue+$("#"+o.addressName).val() );      				
      			}
      		}      		
      		
      		
            beforeCountyName=sel.county.val(); // 下拉change會用到 2011-05-20
            beforeAreaName=sel.area.val(); // 下拉change會用到 2011-05-20
	 ...
	 * -----------------------------------------------------------------------
	 */  
  	$.fn.twzipcode.searchFound = function(strOrg,countyName,areaName){
  		if (strOrg==null || ''==strOrg ) {
  			return false;
  		}  		
  		var found=false;
  		if (strOrg.indexOf(countyName)>-1 && strOrg.indexOf(areaName)>-1 ) {
  			found=true;
  		}
  		return found;
  	}
	$.fn.twzipcode.replaceAll = function(strOrg,strFind,strReplace){

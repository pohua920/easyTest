			...
			return this.each(
				function()
				{
					...
					if (!alreadyExists && s.createButton) {
						// create it!
						var addID = '';
						//建立小日曆圖案的ID(議題編號：0001739)
						if(s.imageID ==null || s.imageID ==''){
							s.imageID = $(this).attr('id')+"Image";
						}
						addID = ' id="'+s.imageID+'" ';
						
						controller.button = $('<a href="#" '+addID+' class="dp-choose-date" title="' + $.dpText.TEXT_CHOOSE_DATE + '">' + $.dpText.TEXT_CHOOSE_DATE + '</a>')
								.bind(
									'click',
									function()
									{
						...
						//小日曆圖案不需要tabindex(議題編號：0001739)
						$('#'+s.imageID).attr("tabindex", -1);
					}
					
					var types = s.dateType;
					...
					if (!alreadyExists && $this.is(':text')) {
						$this
							.bind(
								...
								{	
									this.value = selectedDate.asString(DateFormat,DateType);
								}
							).bind(
								'change',
								function()
								{
									if (this.value != '') {
										parseDate(this,DateType,DateFormat);
										//只要text發生改變會執行這裡~~
										var d = Date.fromString(this.value,DateType,DateFormat);
										if (d) {
											controller.setSelected(d, true, true);
										}
										//按小日曆圖案回傳日期後,tabindex回到小日曆圖案前的欄位(議題編號：0001739)
										$this.select();
									}
								}
							);
 ...
 **/
		dpSetSelected : function(d, v, m,dateType,dateFormat)
		{
			if (v == undefined) v=true;
			if (m == undefined) m=true;
			return _w.call(this, 'setSelected', Date.fromString(d,dateType,dateFormat), v, m, true);
		},
/**
		...
		{	
			...
			setStartDate : function(d)
			{
				if (d) {
					...
					this.startDate = Date.fromString(d,'ad','dd/mm/yyyy');
				}
				if (!this.startDate) {
			...
			{
				if (d) {
					this.endDate = Date.fromString(d,'ad','dd/mm/yyyy');
				}
				if (!this.endDate) {

...
<%
	...
	String title = "AML洗錢查詢作業-掃描結果明細";
	String image = path + "/" + "images/";
	String GAMID = "APS006E1";
	String mDescription = "AML洗錢查詢作業-掃描結果明細";
...
// 				}
			/* mantis：OTH0087，處理人員：BJ085，需求單編號：OTH0087 AML手動登錄 end */
		});
...
<s:if test="resultMainDevResults != null && 0 != resultMainDevResults.size">
<p align="center">AML回傳結果</p>
<table class="MainBodyColor" id="table3" cellpadding="0" cellspacing="0" width="970px" >
<s:iterator var="row" value="resultMainDevResults">
	<tr>
		<td width="110px" align="right">狀態：</td>
		<td width="285px" align="left">
			<c:if test="${screenCode == '0'}">成功</c:if>
			<c:if test="${screenCode == '1'}">格式錯誤</c:if>
			<c:if test="${screenCode == '2'}">系統錯誤</c:if>
			<c:if test="${screenCode == '3'}">Unikey重覆</c:if>
			<c:if test="${screenCode == '4'}">無此資料</c:if>
			<c:if test="${screenCode == '5'}">該客戶有未結案件</c:if>
		</td>
		<td width="110px" align="right">錯誤原因說明：</td>
		<td width="285px" align="left">
			<s:property value="screenMessage"/>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td width="285px" align="left">
			<s:property value="wlfScreenEntryid"/>
		</td>
		<td width="110px" align="right">掃描時間：</td>
		<td width="285px" align="left">
			<s:property value="wlfScreenDate"/>
		</td>
	</tr>
	<tr>
		<td width="110px" align="right">起案ID：</td>
		<td width="285px" align="left">
			<s:property value="wlfCaseId"/>
		</td>
		<td width="110px" align="right">工作狀態：</td>
		<td width="285px" align="left">
			<c:if test="${workStatus == '00'}">不執行</c:if>
			<c:if test="${workStatus == '01'}">待再查詢</c:if>
			<c:if test="${workStatus == '02'}">查詢中</c:if>
			<c:if test="${workStatus == '03'}">收到回覆拒保</c:if>
			<c:if test="${workStatus == '04'}">收到回覆可承保</c:if>
			<c:if test="${workStatus == '05'}">查詢異常</c:if>
			<c:if test="${workStatus == '06'}">查詢超時</c:if>
			<c:if test="${workStatus == '07'}">人工審核註記</c:if>
			<c:if test="${workStatus == '08'}">人工審核完成</c:if>
		</td>
	</tr>
	<tr>
		<td width="110px" align="right">案件目前處理單位：</td>
		<td width="285px" align="left">
			<s:property value="wlfCaseAffiliate"/>
		</td>
		<td width="110px" align="right">案件目前處理使用者：</td>
		<td width="285px" align="left">
			<s:property value="wlfCaseUser"/>
		</td>
	</tr>
	<tr>
		<td width="110px" align="right">案件調查進度：</td>
		<td width="285px" align="left">
			<c:if test="${wlfCaseStatus == 'L2'}">第 2 層</c:if>
			<c:if test="${wlfCaseStatus == 'L3'}">第 3 層</c:if>
			<c:if test="${wlfCaseStatus == 'C'}">案件結案</c:if>
		</td>
		<td width="110px" align="right">案件確認結果：</td>
		<td width="285px" align="left">
			<c:if test="${wlfCaseResult == '1'}">案件調查中</c:if>
			<c:if test="${wlfCaseResult == '2'}">可交易/放行交易</c:if>
			<c:if test="${wlfCaseResult == '10'}">拒絕交易</c:if>
			<c:if test="${wlfCaseResult == '11'}">取消交易</c:if>
			<c:if test="${wlfCaseResult == '12'}">不配合審查</c:if>
		</td>
	</tr>
	<tr>
		<td width="110px" align="right">制裁狀態：</td>
		<td width="285px" align="left">
			<c:if test="${refuseLimitInsurance == '00'}">拒限保未命中</c:if>
			<c:if test="${refuseLimitInsurance == '01'}">拒限保命中未判定</c:if>
			<c:if test="${refuseLimitInsurance == '02'}">拒限保命中已判定True</c:if>
			<c:if test="${refuseLimitInsurance == '03'}">拒限保命中已判定False</c:if>
		</td>
		<td width="110px" align="right">保單風險：</td>
		<td width="285px" align="left">
			<c:if test="${riskRating == '00' }">高風險未處理</c:if>
			<c:if test="${riskRating == '01' }">高風險已處理</c:if>
			<c:if test="${riskRating == '02' }">中風險未處理</c:if>
			<c:if test="${riskRating == '03' }">中風險已處理</c:if>
			<c:if test="${riskRating == '04' }">低風險</c:if>
		</td>
	</tr>
	<tr>
		<td width="110px" align="right">命中狀態：</td>
		<td width="285px" align="left">
			<c:if test="${listDetection == '01' }">名單檢測未命中</c:if>
			<c:if test="${listDetection == '02' }">名單檢測命中未判定</c:if>
			<c:if test="${listDetection == '03' }">名單檢測命中已判定</c:if>
		</td>
	...
	<s:hidden key="oid" id="oidResult" />
</s:iterator>
</table>
</s:if>
<s:elseif test="0 == resultMainDevResults.size">
...
<s:elseif test="resultDtlDevResults.size == 0 && resultMainDevResults.size!=0">

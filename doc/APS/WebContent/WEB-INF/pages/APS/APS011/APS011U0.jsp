...
$(document).ready(function(){
	...
	$("#hiddenIndex").val($(".addTable").eq(-1).index());
	changeDateToROC();
	//validate
	$("#mainForm").validate({
		isShowLabel:false,
	...
	function form_submit(method){
		if('update' === method && checkDtl() && confirm("是否確認修改此筆資料？")===true){
			changeDate();
	...
	function changeDateToROC(){
		var index = $("#hiddenIndex").val();
		for(var i=0;i<=index;i++){
			var birthday = $("[id='amlInsuredList["+i+"].birthday']");
			var estDate = $("[id='amlInsuredList["+i+"].estDate']");
			if(birthday.val()!="" && birthday.val()!= undefined){
				birthday.val((parseInt(birthday.val().substring(0,4))-parseInt(1911))+birthday.val().substring(4,10));
			}
			if(estDate.val()!="" && estDate.val() != undefined){
				estDate.val((parseInt(estDate.val().substring(0,4))-parseInt(1911))+estDate.val().substring(4,10));
			}
		}
	}
...
<s:form theme="simple" namespace="/aps/011" id="mainForm" name="mainForm">
	...
	<table class="MainBodyColor" id="table3" cellpadding="0" cellspacing="0" width="970px">
		...
		<tr>
			...
			<td width="300px" align="left">
				<s:textfield key="%{'amlInsuredListVo.businessNo'}" id="%{'amlInsuredListVo.businessNo'}" readonly="true" cssClass="txtLabel" size="20" maxlength="20" />
	...
	<div id="addTableDiv">
	<s:if test="amlInsuredList != null && 0 != amlInsuredList.size">
		<s:iterator var="row" value="amlInsuredList" status="stat">
		<div class="addTable">
			<table class="MainBodyColor" id="table3" cellpadding="0" cellspacing="0" width="970px">
				<tr>
					<td width="200px" align="right"><font color="#FF0000">*</font>序號：</td>
					<td width="300px" align="left">
					<s:textfield class="cserialNo" key="amlInsuredList[%{#stat.index}].serialNo" id="amlInsuredList[%{#stat.index}].serialNo" size="2" maxlength="2" readonly="true"/></td>
					<td colspan="5"></td>
				</tr>
				<tr>
					<td width="200px" align="right"><font color="#FF0000">*</font>身分別：</td>
					<td width="300px" align="left">
					<s:select list="#{'1':'1 - 自然人', '2':'2 - 法人'}" key="amlInsuredList[%{#stat.index}].insuredType" id="amlInsuredList[%{#stat.index}].insuredType"/></td>
				</tr>
				<tr>
					<td width="200px" align="right"><font color="#FF0000">*</font>身分類型：</td>
					<td width="300px" align="left">
					<s:select list="#{'2':'2 - 要保人', '1':'1 - 被保人', '8':'8 - 賠付對象', '9':'9 - 受益人', '3':'3 - 銀行', '4':'4 - 船名', '5':'5 - 飛機', '6':'6 - 國家', '7':'7 - 收貨人'}" key="insuredFlag" id="amlInsuredList['+parseInt(index+1)+'].insuredFlag"/></td>
				</tr>
				<tr>
					<td width="200px" align="right">身分證字號/統編：</td>
					<td width="300px" align="left">
					<s:textfield key="amlInsuredList[%{#stat.index}].id" id="amlInsuredList[%{#stat.index}].id" size="10" maxlength="50"/></td>
				</tr>
				<tr>
					<td width="200px" align="right"><font color="#FF0000">*</font>姓名：</td>
					<td width="300px" align="left">
					<s:textfield key="amlInsuredList[%{#stat.index}].name" id="amlInsuredList[%{#stat.index}].name" size="50" maxlength="200"/></td>
				</tr>
				
				<tr>
					<td width="200px" align="right">英文姓名：</td>
					<td width="300px" align="left">
					<s:textfield key="amlInsuredList[%{#stat.index}].enName" id="amlInsuredList[%{#stat.index}].enName" size="50" maxlength="200"/></td>
				</tr>
				
				<tr>
					<td width="200px" align="right">性別：</td>
					<td width="300px" align="left">
					<s:select list="#{'':'' , 'M':'M - 男', 'F':'F - 女'}" key="amlInsuredList[%{#stat.index}].gender" id="amlInsuredList[%{#stat.index}].gender"/>
					(法人或是理賠時不需填寫)</td>
				</tr>
				
				<tr>
					<td width="200px" align="right">生日yyy-MM-dd：</td>
					<td width="300px" align="left">
					<s:textfield key="amlInsuredList[%{#stat.index}].birthday" id="amlInsuredList[%{#stat.index}].birthday" size="10" maxlength="10"/>
					(法人或是理賠時不需填寫)</td>
				</tr>
				
				<tr>
					<td width="200px" align="right">國籍代碼：</td>
					<td width="300px" align="left">
					<s:textfield key="amlInsuredList[%{#stat.index}].nationCode" id="amlInsuredList[%{#stat.index}].nationCode" size="4" maxlength="4"/>
					例如：TW(理賠不需填寫)</td>
				</tr>
				...
				<tr>
					...
					<td width="300px" align="left">
					<s:select list="#{'':'' , 'Y':'Y - 是', 'N':'N - 否'}" key="amlInsuredList[%{#stat.index}].dangerOccupation" id="amlInsuredList[%{#stat.index}].dangerOccupation"/>
					(理賠不需填寫)</td>
				</tr>
				
				<tr>
					<td width="200px" align="right">公司成立日期yyy-MM-dd：</td>
					<td width="300px" align="left">
					<s:textfield key="amlInsuredList[%{#stat.index}].estDate" id="amlInsuredList[%{#stat.index}].amlInsuredVo.estDate" size="10" maxlength="10"/>
					(自然人、理賠不需填寫)</td>
				</tr>
				
				<tr>
					<td width="200px" align="right">上市櫃公司：</td>
					<td width="300px" align="left">
					<s:select list="#{'':'' , 'Y':'Y - 是', 'N':'N - 否'}" key="amlInsuredList[%{#stat.index}].listedCabinetCompany" id="amlInsuredList[%{#stat.index}].listedCabinetCompany"/>
					(自然人、理賠不需填寫)</td>
				</tr>
			</table>
			<input type="button" value="刪除" onClick="deleteDtlTable(this)" style="margin-left:467px"/>
		</div>
		</s:iterator>
	...
	<table width="970px">
		<tr>
			<td align="center"><input value="送出" type="button" onclick="javascript:form_submit('update');">&nbsp;&nbsp;&nbsp;&nbsp; 
							   <input value="取消" type="button" onclick="cancelUpdate()" />
			</td>
		</tr>
	</table>
</s:form>
...
<script type="text/javascript">
	...
	function addDtlTable(){
		var addTableIndex = $(".addTable").eq(-1).index();
		...
		var table = $('<div class="addTable"><table class="MainBodyColor" id="table3" cellpadding="0" cellspacing="0" width="970px">'
				...
				+'<td width="300px" align="left">'
				+'<s:textfield class="cserialNo" key="amlInsuredList['+parseInt(index+1)+'].serialNo" id="amlInsuredList['+parseInt(index+1)+'].serialNo" size="2" maxlength="2" value="'+parseInt(addTableIndex+2)+'" readonly="true"/></td>'
				+'<td colspan="5"></td></tr>'
				//身分別
				...
				+'<td width="300px" align="left">'
				+'<s:textfield key="amlInsuredList['+parseInt(index+1)+'].estDate" id="amlInsuredList['+parseInt(index+1)+'].amlInsuredVo.estDate" size="10" maxlength="10"/>'
				+'(自然人、理賠不需填寫)</td></tr>'
				//上市櫃公司

...
<body bgcolor="#FFFFFF" onload="showValue();">
 ...
 <form name="fm" method="post" action="CommonReinsSave.jsp">
	<table class="common" cellpadding="5" cellspacing="1" align="center">
        ...
        <tr>
        	<td colspan=6 class="input">
        		<%--臨分意向 --%>
        		<s:text name="undwrt.pages.undwrtDeal.partIntention"/>：</td>
        </tr>
        
        <tr>
            <td class=input colspan=6 align="center">
                <textarea class="big" wrap="hard" name="Remarks" rows="8" cols="80" maxLength="255" 
                	description="分保意向" value=""><s:property value="#request.feoEnquiryVO.remarks"/>
                </textarea>
            </td>
        </tr>
        ...
        <tr>
      		<td class="input">
            	...
            	<input type="hidden" name="CheckBoxSpecialFacShareHidden" value="<s:property value="#request.feoEnquiryVO.specialFacFlag"/>"/>
            ...
            <td class="text">
            	<input type="text" name="SpecialFacShare" 
            		value="<fmt:formatNumber value="${feoEnquiryVO.specialFacShare}" pattern="0.000000"/>"></td>
        </tr>
        
        <tr>
            <td class="input">
            	<input type="CheckBox" name="CheckBoxFacShare">
            	<input type="hidden" name="CheckBoxFacShareHidden" value="<s:property value="#request.feoEnquiryVO.facFlag"/>">
            ...
            <td class="text">
            	<input type="text" name="FacShare" 
            		value="<fmt:formatNumber value="${feoEnquiryVO.facShare}" pattern="0.000000"/>"></td>        
        </tr>
    </table>
    ...
    <!--条件UI显示表格-->
    <span style="display:">
    <table id="reinsReceiveDATA" name="reinsReceiveDATA" style="display:none">
		<tr>
    ...
    <table class="common" cellpadding="5" cellspacing="1" align="center" id="reinsReceiveUI" name="reinsReceiveUI">
        ...
        <s:if test="#request.feoEnquiryVO.verifyFlag!=3">
        <s:if test="#request.feoReinsReceiveList!=null">
		<s:iterator id="feoReinsReceive" status="statu" value="#request.feoReinsReceiveList">
			<tr>
				<td class="text" width="15%">
				...
				<td class="text" width="10%" > <input type="button" name="deleteReinsReceiveButton" 
					value="<s:text name='prompt.del'/>" class="button" alt="<s:text name='prompt.del'/>" 
					src="/undwrt/common/images/butDeleteBlue.gif" onclick="deleteRow('reinsReceiveUI',this,1,1);"></td>
			</tr>  
		</s:iterator>
		</s:if>
		</s:if>
     		...
     		<tr>
        		<td class="text" colspan=14 align="center">               
                	<Input name="addReinsReceive" class="button" type="button" value="<s:text name='undwrt.pages.undwrtDeal.addAccepter'/>" 
                		...
                		onclick="insertRow('reinsReceiveUI','reinsReceiveDATA');">
              	</td>
      		</tr>
		</tfoot>
 		<tbody></tbody>
  		...
  		<table class="common" cellpadding="5" cellspacing="0" align="center">
  			<tr>
      			<td class=button width=33% align="center">
      				<%--儲存 --%>
      				<Input name="butSave" class="button" type="button" value="<s:text name='undwrt.save'/>" 
  	...
  	<table align="center" class="common" cellpadding="5" cellspacing="1">
		<tr>
   			<td class="title">
   				<%--再保確認狀態 --%>
   				<s:text name="undwrt.pages.undwrtDeal.reinsuranceAffirmStates"/></td>
          	<s:if test="#request.feoEnquiryVO!=null">
         	<td colspan="7" class="text">
         		<input type="hidden" name="feoEnquiryDtoVerifyFlag" readonly value="<s:property value="#feoEnquiryVO.verifyFlag" />">
             	<s:if test="#request.feoEnquiryVO.verifyFlag==0">
             		<%--未提交再保 --%>
             		<s:text name="undwrt.pages.undwrtDeal.notSubmitReinsurance"/>
           		</s:if>
            	<s:if test="#request.feoEnquiryVO.verifyFlag==1">
                    <%--待再保确认 --%>
                    <s:text name="undwrt.pages.undwrtDeal.waitReinsuranceAffirm"/>
              	</s:if>
           		<s:if test="#request.feoEnquiryVO.verifyFlag==2">
                    <%--待再保通过 --%>
            	...
            	<s:if test="#request.feoEnquiryVO.verifyFlag==4">
                    <%--再保不通过 --%>
                    <s:text name="undwrt.pages.undwrtDeal.reinsuranceNotPass"/>
               	</s:if>
            	<s:if test="#request.feoEnquiryVO.verifyFlag==5">
                   <%--非临分 --%>
                   <s:text name="undwrt.pages.undwrtDeal.noPart"/>
              	</s:if>
               	<s:if test="#request.feoEnquiryVO.verifyFlag==9">
               		<%--注销 --%>
		        	...
		        	<thead>
		            	<tr class=listtitle>
		                	<td width="10%" align="center">
		                		<%--確認次數 --%>
		                		<s:text name="undwrt.pages.undwrtDeal.numberAffirm"/></td>
		                	<td width="80%" align="center">
		                		<%--再保意見 --%>
		                		<s:text name="undwrt.pages.undwrtDeal.reinsuranceOpinion"/></td>
		                	<td width="10%" align="center">
		                		<%--處理時間 --%>
		                		<s:text name="undwrt.pages.undwrtDeal.processingDate"/></td>
		            	</tr>
		        	</thead>
		        	<s:if test="#request.feoReinsVerifyList!=null">
		        	<s:iterator id="feoReinsVerify" status="statu" value="#request.feoReinsVerifyList">
				    	<tr>
				        	<td class="text" width="10%">
				        		<s:property value="#feoReinsVerify.serialNo" /></td>  
					    	<td class="text" width="80%">
					    		<textarea name="feoReinsVerify" rows="3" cols="100" readonly="true" styleClass="one">${verifyText }</textarea></td>
					    	<td class="text" width="10%"><rc:rcDate value = "${verifyDate }" format="yyyy-MM-dd"/></td>
				    	</tr>
					</s:iterator>
					</s:if>
		        	<tfoot></tfoot>
      	...
      	<tr>
            ...
            <td class="text">
            	<input type="text" name="EnquiryNo" readonly value="<s:property value="#request.feoEnquiryVO.enquiryNo" />"></td>
      	</tr>
  	</table>
</form>
</body>

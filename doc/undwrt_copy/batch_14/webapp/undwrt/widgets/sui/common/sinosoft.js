...
var globalQueryButton = null;
SINOSOFT.util.disableCurrentQueryButton=function(){
	var evt=event || window.event;   
	globalQueryButton = evt.srcElement;
	globalQueryButton.disabled=true;	
}

SINOSOFT.util.enableCurrentQueryButton=function(){
	if(globalQueryButton!=null){
		globalQueryButton.disabled=false;
		globalQueryButton=null;
	}
}
...
SINOSOFT.util.navigation = function(oRequest,oParsedResponse,oCallback,oCaller){ 
	...
	var totalShow = totalRecords;
	if(parseInt(totalRecords)>500){
		totalShow = i18n.navigator.more + " 500";
	}
    var markup = "["+ i18n.navigator.page +" "+pageNo +" / "+ pageCount +"]&nbsp;&nbsp;["+ totalShow +" "+i18n.navigator.records+"]&nbsp;&nbsp;";
    ...
    SINOSOFT.util.enableCurrentQueryButton();
}; 
...
SINOSOFT.util.extra_message = function(oRequest,oParsedResponse,oCallback,oCaller){ 
	var extra_message = document.getElementsByName(contentDataTable._elContainer.id+"_message")
    if(YAHOO.widget.DataTable.MESSAGE != null){
		var markup= YAHOO.widget.DataTable.MESSAGE; 
	    for(var i=0;i<extra_message.length;i++){
		    extra_message[i].innerHTML = markup; 
		}
	} else {
		for(var i=0;i<extra_message.length;i++){
		    extra_message[i].innerHTML = ""; 
		}
	}
}; 
...
function editRecord(url){
	...
	var newWindow = openWindow(url, "_blank", "alwaysRaised,channelmode,status,scrollbars,resizable");
}
...
//delete record
function deleteRecord(url,handleSuccess){
	if(!window.confirm("ÄúÈ·¶¨É¾³ýÂð£¿")){
		return;
	} 
...
function checkLength(field)
{
  ...
  if(count>length)
  {
    alert(i18n.messages.contentBeyond+"\n" +  i18n.messages.maxLength + length + i18n.messages.englishCode+"\n"+i18n.messages.inputAgain6);
    field.focus();
    field.select();
    return false;
  }

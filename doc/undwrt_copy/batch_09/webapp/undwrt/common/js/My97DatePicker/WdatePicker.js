 ...
 */
function getToRcDateValue(fm,field){
	var adValue = RcToAd(field.value);
	var index = getElementIndexInForm(fm,field)-1;
	var adfield = fm.elements[index];
	adfield.value = adValue;
}

function RcToAd(value){
	 var date = parseInt(value.split("-")[0])+1911;
	 return date+"-"+value.split("-")[1]+"-"+value.split("-")[2];
